...
contract SCRUB is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    SCRUBDividendTracker public dividendTracker;
    ...
    constructor() ERC20("SCRUB", "$SCRUB") {
        marketingWallet = payable(0x42F7FaF5716C6F937976bC61b61708eB41C5D764);
        devWallet = payable(0x42F7FaF5716C6F937976bC61b61708eB41C5D764);
        ...
        dividendTracker = new SCRUBDividendTracker(
            payable(this),
            router,
        ...
        uint256 totalTokenSupply = 369_369_369_369_369 * 10**18;
        _mint(owner(), totalTokenSupply); // only time internal mint function is ever called is to create supply
        maxWallet = 18_468_468_468 * 10**18;
        swapTokensAtAmount = 738_738_738 * 10**18; //0.2%
...
contract SCRUBDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        SCRUBContract = SCRUB(mainContract);
    ...
    function _transfer(
        ...
        require(false, "SCRUB_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "SCRUB_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SCRUBcontract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(SCRUBContract);

            uint256 prevBalance = SCRUBContract.balanceOf(address(this));
            ...
            {
                uint256 received = SCRUBContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    SCRUBContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
