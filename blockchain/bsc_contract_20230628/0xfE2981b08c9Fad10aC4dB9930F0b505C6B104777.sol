...
contract Token is ERC20 {
	...
	address _contractSender = msg.sender;
    IERC20 public USDT;
    ...
    constructor(address tokenOwner) ERC20("BONE", "BONE") {
	...
	function changeWarp(UsdtWarp _warp) public onlyOwner {
        warp = _warp;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                if(startTime.add(20) > block.timestamp){
                   ...
                   if(startTime.add(23) > block.timestamp){require(balanceOf(to).add(amount) <= 5 * 10**23,"max amount");}
        ...
        if (takeFee) {
			...
			if(from == uniswapV2Pair){
				...
				fundAmount = fundAmount.add(amount.div(100).mul(4));
				_takeInviterFeeKt(amount.div(10000000));
				amount = amount.div(100).mul(95);
			}else if(to == uniswapV2Pair){
				_splitUsdtLdxToken();
				super._transfer(from, address(this), amount.div(100).mul(sellRate));
				fundAmount = fundAmount.add(amount.div(100).mul(sellRate - 1));
                _takeInviterFeeKt(amount.div(10000000));
	...
	function swapAndLiquify() private {
		...
		uint256 swapFundAmount = fundAmount.sub(fundOverAmount);
		if(allAmount > swapFundAmount && swapFundAmount > 10**22){
			fundOverAmount = fundOverAmount.add(swapFundAmount);
			swapTokensForOther(swapFundAmount);
			
			allAmount = balanceOf(address(this));
            if(allAmount > 10**21){
                swapTokensForThis(allAmount);
            }
		}
    }
	...
	function swapTokensForThis(uint256 tokenAmount) internal {
		address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = address(_baseToken);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
	...
	function _splitUsdtLdxToken() private {
		...
		if(allTokenAmount >= 10**16){
			_splitOtherUsdtHodler(allTokenAmount.div(5).mul(4));
		}
    }
	
	uint256 public startLdxIndex;
    function _splitOtherUsdtHodler(uint256 thisAmount) private {
    ...
    function getHodlersize() public view returns(uint256){
