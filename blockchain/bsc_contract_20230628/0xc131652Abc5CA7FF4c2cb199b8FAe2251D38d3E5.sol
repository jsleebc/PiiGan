...
contract PepeF is ERC20, Ownable, ReentrancyGuard {
    ...
    event UpdateExcludeFromMaxWalletAmount(
        address indexed account,
        bool isExcluded
    );
    ...
    constructor() ERC20("Pepe Frenzy", "PepeF") {
        _transferOwnership(address(0xdCA5c9eAf816F1Ea3700aa5dC6ca9F7ed41C65df));
        
        _mint(owner(), 400_000_000_000 * (10 ** 18));

        marketingTaxBuy = 500;
        marketingTaxSell = 500;
        marketingTaxTransfer = 500;

        reflectionTaxBuy = 300;
        reflectionTaxSell = 300;
        ...
        marketingWallet = 0xdCA5c9eAf816F1Ea3700aa5dC6ca9F7ed41C65df;
        buybackWallet = 0xAdddaCCb92324E1c492fd400b63166A6DA3404A7;
        ...
        _isExcludedFromMaxWalletAmount[address(uniswapV2Router)] = true;
        _isExcludedFromMaxWalletAmount[address(uniswapV2Pair)] = true;
        _isExcludedFromMaxWalletAmount[address(owner())] = true;
        _isExcludedFromMaxWalletAmount[address(DEAD)] = true;
    ...
    function setBuyTax(
        ...
        emit UpdateBuyTax(_marketingTaxBuy, _reflectionTaxBuy, _buybackTaxBuy);
    }
    ...
    function setMaxWalletAmount(uint256 amount) external onlyOwner {
        require(
            ...
            "maxWalletAmount already on that amount"
        );
        require(
            amount >= 100 && amount <= 10_000,
            "maxWalletAmount cannot be below 1% of totalSupply (100) or more than 100% of totalSupply (10000)"
        );

        maxWalletAmount = amount;

        emit UpdateMaxWalletAmount(amount);
    }
    ...
    function setExcludeFromMaxWalletAmount(
        address account,
        bool excluded
    ) external onlyOwner {
        require(
        ...
        emit UpdateExcludeFromMaxWalletAmount(account, excluded);
    }

    function isExcludedFromMaxWalletAmount(
        address account
    ...
    function _transfer(
        ...
        if (!_isExcludedFromMaxWalletAmount[to]) {
            require(
                balanceOf(to) + amount <=
                    (totalSupply() * maxWalletAmount) / denominator,
                "Balance of receiver wallet cannot more than max wallet amount"
            );
        }
        ...
        if (takeFee) {
            ...
            if (_isAutomatedMarketMakerPair[from]) {
                if (isLiquidityAdded && block.number <= antiBotBlockEnd) {
                    fees = (amount * (antiBotTax)) / denominator;
                } else {
                    fees =
                        (amount *
                            (marketingTaxBuy +
    ...
    function swapBack() internal inSwap {
        ...
        uint256 marketingBNB = (newBalance *
            (marketingTaxBuy + marketingTaxSell + marketingTaxTransfer)) /
            totalTax;
        uint256 reflectionBNB = (newBalance *
            (reflectionTaxBuy + reflectionTaxSell + reflectionTaxTransfer)) /
            totalTax;
        uint256 buybackBNB = newBalance - marketingBNB - reflectionBNB;
        ...
        if (reflectionBNB > 0) {
            sendBNB(address(dividendTracker), reflectionBNB);
            dividendTracker.distributeDividends(reflectionBNB);
            emit SendDividends(reflectionBNB);
        }

        if (buybackBNB > 0) {
            sendBNB(buybackWallet, buybackBNB);
        }
    }
