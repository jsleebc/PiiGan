...
contract MMCToken is ERC20, Ownable {
    ...
    uint256 private openProtectionTime = 10 minutes;
    ...
    function setTechWallet(address newAddress) public onlyOwner {
        techWallet = newAddress;
        excludeFromFees(techWallet, true);
    }
    ...
    function setTechFee(uint256 techFee_) public onlyOwner {
        techFee = techFee_;
    }
    ...
    function setKb(uint256 _kb) public onlyOwner{
        kb=_kb;
    }
    ...
    function takeTxFee(address from, uint256 amount) internal returns(uint256 amountAfter) {
        ...
        uint256 tFee= amount.mul(_techFee).div(100);
        if(tFee > 0){
            super._transfer(from, techWallet, tFee);
        }
        ...
        if(lFee > 0){
            super._transfer(from, lp1Wallet, lFee);
        }
        amountAfter = amount.sub(tFee).sub(nFee).sub(lFee);
    }

    function burnPair() public onlyCaller {
       ...
       lastLpBurnTime = block.timestamp;
        ...
        if(deadAmount > 0){
            super._transfer(uniswapV2Pair, address(0xdead), deadAmount);
        }
        if(lp2Amount > 0){
            super._transfer(uniswapV2Pair, lp2Wallet, lp2Amount);
        }
        if(lp3Amount > 0){
            super._transfer(uniswapV2Pair, lp3Wallet, lp3Amount);
        }
