...
contract DID is CfoTakeableV2, IERC20Metadata {
    using Address for address;
    ...
    string private constant _name = "DID";
    ...
    uint public buyFeeRate = 10000;
    uint public sellFeeRate = 10000;
    ...
    constructor(
        ...
        pair = pancakeSwapFactory.createPair(usdt, address(this));
        address holder = _initHolder == address(0) ? msg.sender : _initHolder;
        isExcludeFromFees[holder] = true;
        feeTo = holder;
    ...
    function _transfer(
        ...
        if(recipient != address(0) && ((isBuy && !isExcludeFromFees[recipient]) || (isSell && !isExcludeFromFees[sender]))){
    ...
    function isSwapPair(address _pair) public view returns(bool){
        if(_pair == address(0)){
            return false;
        }

        return pair == _pair || pancakeSwapFactory.getPair(address(this), wbnb) == _pair || isOtherSwapPair[_pair];
    }
