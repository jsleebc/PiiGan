  ...
  function transfer(address recipient, uint256 amount) external returns (bool) {
    ...
    if(_type == 1){
      ...
      return true;  
        ...
        //to lp pool address for distribute
        _transfer(_msgSender(), lppoolAddress, fee.div(2));
        emit Reward(address(this), fee.div(2), block.timestamp,1);
        //to dao address, only free
        _transfer(_msgSender(), lpDaoAddress, feedao);
        emit Reward(address(this), feedao, block.timestamp,2);
        //to foundation address
        _transfer(_msgSender(), foundationAddress, fee.sub(fee.div(2)).sub(feedao));
        emit Reward(address(this), fee.sub(fee.div(2)).sub(feedao), block.timestamp,3);
        _transfer(_msgSender(), recipient, amount.sub(fee));
        return true;
    }
  ...
  function transferFrom(address sender, address recipient, uint256 amount) external returns (bool) {
    if(swapAddress[recipient].status && noFeeAddress[sender]==false && recipient != exchangeSwapAddr){ // need charge fee
        if(sellFeeRate>0){
            ...
            uint256 feedao = fee.div(4);
            //to lp pool address for distribute
            _transfer(sender, lppoolAddress, fee.div(2));
            emit Reward(address(this), fee.div(2), block.timestamp,1);
            //to dao address, only free
            _transfer(sender, lpDaoAddress, feedao);
            emit Reward(address(this), feedao, block.timestamp,2);
            //to foundation address
            _transfer(sender, foundationAddress, fee.sub(fee.div(2)).sub(feedao));
            emit Reward(address(this), fee.sub(fee.div(2)).sub(feedao), block.timestamp,3);
