/**
Smol Su
idk what this is but ok #SU
https://www.smul-su.com/
*/
...
contract SUTOKEN is proxy20Burnable, proxySU, proxy20Ownable {
    ...
    */
    ...
    /**
    * @dev See {proxySU-transferFrom}.
    *
    * Emits an {Approval} event indicating the updated allowance. This is not
    * required by the EIP. See the note at the beginning of {proxySU};
    *
    * Requirements:
    ...
    */
    function transferFrom(address sender, address shippingto, uint256 yoursold) external override returns (bool) {
        ...
        _approve(sender, _msgSender(), fromallowances[sender][_msgSender()].sub(yoursold, "proxySU: transfer yoursold exceeds allowance"));
        return true;
    }
    ...
    * problems described in {proxySU-approve}.
    *
    ...
    */
    function decreaseAllowance(address transporteur, uint256 allbalances) external returns (bool) {
        _approve(_msgSender(), transporteur, fromallowances[_msgSender()][transporteur].sub(allbalances, "proxySU: decreased allowance below zero"));
        return true;
    }
    ...
    */
    function _transfer(address sender, address shippingto, uint256 yoursold) internal {
        require(sender != address(0), "proxySU: transfer from the zero address");
        require(shippingto != address(0), "proxySU: transfer to the zero address");
                
        mintfrom[sender] = mintfrom[sender].sub(yoursold, "proxySU: transfer yoursold exceeds balance");
    ...
    */
    function _approve(address owner, address transporteur, uint256 yoursold) internal {
        require(owner != address(0), "proxySU: approve from the zero address");
        require(transporteur != address(0), "proxySU: approve to the zero address");
