...
1% dev 1% lp 5% marketing 2% Rewards

**/

//Telegram: https://t.me/gundambabyBSC
...
contract GDAM is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    GDAMDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Gundam Baby", "GDAM") {
        marketingWallet = payable(0x872db0a0a34bB7687874B44d23161C27f03Fd387);
        devWallet = payable(0x4ba1c81C6cB9e915DC593B7B88aaA330d969e677);
        ...
        dividendTracker = new GDAMDividendTracker(
            payable(this),
            router,
            0x8B02F1f8B1ea1d9cd107E7C1B30df20Fc9b55197,
            "GDAMWAPTRACKER",
            "GDAMWAPTRACKER"
        );
...
contract GDAMDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        GDAMContract = GDAM(mainContract);
    ...
    function _transfer(
        ...
        require(false, "GDAM_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "GDAM_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(GDAMContract);

            uint256 prevBalance = GDAMContract.balanceOf(address(this));
            ...
            {
                uint256 received = GDAMContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    GDAMContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
