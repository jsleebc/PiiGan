...
contract Rozi is BEP20, Ownable {
    ...
    bool public contestEnabled = true;

    uint256 public tokenLiquidityThreshold = 5e11 * 10 ** 18;

    uint256 public genesis_block;
    uint256 private deadline = 2;
    uint256 private launchtax = 99;

    address public marketingWallet = 0x51aFdbC5972506d8a6E7455D4E32EC4FDaE6e603;
    address public leaderBoardContract =
        0x98f0dEea8199ce10b6C1558dd7C59E0A4fc441b6;
    address public developmentWallet =
        0x49540fe97A89c2F0fB9FbafcbD37d909e0d1a3Fb;
    ...
    Taxes public buyTaxes = Taxes(3, 1, 1, 0);
    Taxes public sellTaxes = Taxes(3, 1, 1, 0);
    ...
    constructor() BEP20("Rozi Inu", "ROZI") {
        _tokengeneration(msg.sender, 5e14 * 10 ** decimals());
        ...
        exemptFee[leaderBoardContract] = true;
    ...
    function _transfer(
        ...
        // contest leaderboard tracking
        if (providingLiquidity && contestEnabled) {
            if (sender == pair && !exemptFee[recipient]) {
                ITracking(leaderBoardContract).trackTransfer(
                    recipient,
                    amount,
                    true
                );
    ...
    function Liquify(
        ...
        if (contractBalance >= tokenLiquidityThreshold) {
            ...
            uint256 leaderboardAmt = unitBalance * 2 * swapTaxes.leaderboard;
            if (leaderboardAmt > 0) {
                payable(leaderBoardContract).sendValue(leaderboardAmt);
            }
        }
    }
    ...
    function enableContest(bool enable) external onlyOwner {
        contestEnabled = enable;
    }

    function updateLiquidityTreshhold(uint256 new_amount) external onlyOwner {
        require(
            new_amount >= 5e10,
            "Swap threshold amount should be greater or equal to 0.01% of tokens"
        );
        require(
            new_amount <= 5e12,
    ...
    function updateMarketingWallet(address newWallet) external onlyOwner {
        require(
            ...
            "Fee Address cannot be Contract Address"
        );
        require(newWallet != address(0), "Fee Address cannot be zero address");
        marketingWallet = newWallet;
    }

    function updateleaderBoardContract(address newWallet) external onlyOwner {
        require(
            newWallet != address(this),
            "Fee Address cannot be Contract Address"
        );
        require(newWallet != address(0), "Fee Address cannot be zero address");
        leaderBoardContract = newWallet;
    }
