...
contract AIO is ERC20, Ownable, Pausable {
    uint256 private initialSupply;

    uint256 private constant denominator = 100;

    uint256 private swapThreshold = 0.0000005 ether; // The contract will only swap to BNB, once the fee tokens reach the specified threshold
    ...
    IUniswapV2Router02 private constant PANCAKESWAPV2ROUTER02 =
        IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    IUniswapV2Factory private constant PANCAKESWAPV2FACTORY =
        IUniswapV2Factory(0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73);
    IUniswapV2Pair private immutable PANCAKESWAPV2PAIR;

    constructor(
        string memory _tokenName,
        string memory _tokenSymbol
    ) ERC20(_tokenName, _tokenSymbol) {
        initialSupply = 100_000_000 * (10 ** 9);
        _setOwner(msg.sender);
        PANCAKESWAPV2PAIR = IUniswapV2Pair(
            PANCAKESWAPV2FACTORY.createPair(
                address(this),
                PANCAKESWAPV2ROUTER02.WETH()
            )
        );
    ...
    function handleTax(
        ...
        sellPath[1] = PANCAKESWAPV2ROUTER02.WETH();
        ...
        if (!isExcluded(from) && !isExcluded(to)) {
            ...
            if (from == address(PANCAKESWAPV2PAIR)) {
            ...
            } else if (to == address(PANCAKESWAPV2PAIR)) {
                ...
                uint256 ethValue = PANCAKESWAPV2ROUTER02.getAmountsOut(
                    taxSum,
                    sellPath
                )[1];
                ...
                if (ethValue >= swapThreshold) {
                    ...
                    _approve(
                        address(this),
                        address(PANCAKESWAPV2ROUTER02),
                        toSell
                    );

                    PANCAKESWAPV2ROUTER02.swapExactTokensForETH(
                        toSell,
                        0,
                        sellPath,
                        address(this),
                    ...
                    uint256 teamETH = (ethGained *
                        ((teamTokens * 10 ** 18) / taxSum)) / 10 ** 18;
                    ...
                    (uint amountToken, , ) = PANCAKESWAPV2ROUTER02
                    ...
                    payable(taxWallets["development"]).transfer(devETH);
                    payable(taxWallets["team"]).transfer(teamETH);

                    if (
                        ethGained -
                            (marketingETH + devETH + liquidityETH + teamETH) >
                        0
                    ) {
                        payable(taxWallets["marketing"]).transfer(
                            ethGained -
                                (marketingETH + devETH + liquidityETH + teamETH)
                        );
                    }
    ...
    function triggerTax() public onlyOwner {
        handleTax(address(0), address(PANCAKESWAPV2PAIR), 0);
    }
    ...
    function exclude(address account) public onlyOwner {
        require(!isExcluded(account), "AIO: Account is already excluded");
    ...
    function removeExclude(address account) public onlyOwner {
        require(isExcluded(account), "AIO: Account is not excluded");
