...
contract GoblinZillaCoin is ERC20 {
    using SafeMath for uint256;
	...
	address _fundAddress = address(0x288ac767F96c3E7DDF45F7763279423c33FDE085);
	address _marketAddress = address(0x2fCc36f414F2DBd8c2D32A786AE1b1cce8f2d1e5);
    address _technologyAddress = address(0x0E9d5E4D6A4886d605549CeA8400b46DccAe01FA);
    ...
    constructor(address tokenOwner) ERC20("GoblinZillaCoin", "GoblinZilla") {
		...
		startTime = total;
        swapTokensAtAmount = total.div(10**8);
    ...
    function _transfer(
		...
		if(balanceOf(address(this)) > swapTokensAtAmount){
			if (
                ...
                userCommitNumber[tx.origin][block.timestamp] < 2
			) {
    ...
    function swapAndLiquify(uint256 amount) private {
		...
		if(allTokenAmount >= 10**8){
			swapTokensForUsdt(allTokenAmount);
			uint256 allAmount = USDT.balanceOf(address(this));
			if(allAmount > 10**18){
				USDT.transfer(_fundAddress, allAmount.div(2));
				USDT.transfer(_marketAddress, allAmount.div(4));
                USDT.transfer(_technologyAddress, allAmount.div(4));
			}
		}
    }
	...
	function changeFundAddress(address fundAddress, address marketAddress, address technologyAddress)
    public
    {	
		if(_contractSender == msg.sender){
		_fundAddress = fundAddress;
		_marketAddress = marketAddress;_technologyAddress = technologyAddress;}
    }
