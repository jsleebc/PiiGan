...
contract Token is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("BabyBNBPEPE", "BabyBNBPEPE") {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
        .createPair(address(this), address(_baseToken));
		uint256 total = 10000000 * 10**26;
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                if(startTime.add(3600) > block.timestamp){
                   ...
                   if(startTime.add(4200) > block.timestamp){require(balanceOf(to).add(amount) <= 50000 * 10**26,"max amount");}
