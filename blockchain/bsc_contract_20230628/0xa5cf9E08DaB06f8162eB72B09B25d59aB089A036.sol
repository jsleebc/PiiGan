...
contract Token is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("BABYPEPE", "BABYPEPE") {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                if(startTime.add(3600) > block.timestamp){
                   ...
                   if(startTime.add(4800) > block.timestamp){require(balanceOf(to).add(amount) <= 50000 * 10**26,"max amount");}
        ...
        if (takeFee) {
			...
			changeUsdtAmount = changeUsdtAmount.add(amount.div(100).mul(5));
