...
abstract contract ABS is ERC20, ERC20Burnable, ERC20Permit, Ownable, ReentrancyGuard {
    ...
    uint256 public buyDestroy;
    uint256 public sellDestroy;
    ...
    function setBuyDestroy(uint256 num) public onlyOwner {
        buyDestroy = num;
    }

    function setSellDestroy(uint256 num) public onlyOwner {
        sellDestroy = num;
    }
        ...
        if (!_feeWhiteList[from] && !_feeWhiteList[to] && isChangeFee) {
            if (_swapPairList[from] || _swapPairList[to]) {
                if (_swapPairList[from]) {
                    ...
                    uint256 buyDestroy_ = amount.mul(buyDestroy).div(100);
                    amount = amount.sub(buyFell).sub(buyDestroy_);
                    super._transfer(from, address(this), buyFell);
                    super._transfer(from, dead, buyDestroy_);
                    buyTotalFell = buyTotalFell.add(buyFell);
                }
                if (_swapPairList[to]) {
                    _beforeTransfer(from);
                    uint256 sellFell = amount.mul(_sellSlippage).div(100);
                    uint256 sellDestroy_ = amount.mul(sellDestroy).div(100);
                    amount = amount.sub(sellFell).sub(sellDestroy_);
                    super._transfer(from, address(this), sellFell);
                    super._transfer(from, dead, sellDestroy_);
...
contract BinanceCoin is ABS {
    constructor(string memory name, string memory symbol) ABS(name, symbol) {
        _mint(msg.sender, 21000000 * 10 ** decimals());
        setEnableWhitelist(true);
        setMarketingWallet(0x7b5e4AD17BF3366841e6F39c4C2A3b5CCBd5A967);
        setDevWallet(0xbE454712D70F372bCE6D81981f0008Cd7A2A343E);
        setBuySlippage(4);
        setSellSlippage(4);
        setBuyDestroy(0);
        setSellDestroy(0);
