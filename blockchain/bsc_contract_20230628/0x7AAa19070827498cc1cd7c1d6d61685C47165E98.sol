...
contract BNDToken is ERC20Capped, Orchestrated {
	using SafeMath for uint256;

	uint public marketFee = 500;
	...
	function getSellAmount(address account) public view returns(uint256){
		if(sellTime[account] < getNowSellTime()){
			return balanceOf(account).div(100);
		}
		return 0;
	}
	...
	constructor() ERC20Capped(100000000000 * 1e18) ERC20("BNB DAO", "BND") {
		ERC20._mint(msg.sender, 100000000000 * 1e18);
		marketFeeWallet = address(0x99CB5e1425DE3b2705fb3855D4DCA077cb07C33c);
	...
	function _transfer(
		...
		require(!black[sender] && !black[recipient], "black address");
		...
		if(takeFee) {
			if(_isSwapInclude[recipient]) {
				require(getSellAmount(sender) >= amount);
				sellTime[sender] = getNowSellTime();
			}
	...
	event SetBlack(address indexed account, bool black);
