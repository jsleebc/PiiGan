...
contract FlokitaCEO is Context, IERC20, IERC20Metadata, Ownable {
    ...
    constructor() {
        ...
        */
        ...
        swapTokens.swapTokensAt = _totalSupply.div(1000);
        swapTokens.minToSend = 100 ether;
    ...
    */
    function setTaxAddress(address _nativeTokenAddress, address _farmAddress) external onlyOwner {
    ...
    */
    ...
    function swapTokensForBUSD(uint256 _amount) internal {
    ...
    function manualswapTokensForBUSD(uint256 _amount) external onlyOwner {
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        path[2] = address(BUSD);
    ...
    function manualSendBUSD() external onlyOwner {
        sendBUSD();
    }
 
    function sendBUSD() private {
        uint256 busdBalance = IERC20(BUSD).balanceOf(address(this));
        IERC20(BUSD).transfer(nativeTokenAddress, busdBalance.div(2));
        IERC20(BUSD).transfer(farmAddress, busdBalance.div(2));
    }
        ...
        if(hasFees && (to == uniswapV2Pair || from == uniswapV2Pair)) {
            ...
            if (
                ...
                if(contractTokenBalance > swapTokens.swapTokensAt.mul(1100).div(1000)) {
                    swapTokensForBUSD(swapTokens.swapTokensAt);
                } else {
                ...
                uint256 contractBUSDBalance = IERC20(BUSD).balanceOf(address(this));
 
                if(contractBUSDBalance > swapTokens.minToSend) {
                    sendBUSD();
                }
            }
        }
