...
contract GruInu is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        uint256 totalSupply_ = 420690 * (10**9) * (10**18);
    ...
    function _transfer(
        ...
        } else if (starting == true && block.number > (startBlock + 1)) {
            buyFee = 10;
            sellFee = 10;
            starting = false;
        }
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
            ...
            if (startBlock == 0 || block.number < (startBlock + 3)) {
                require(amount <= maxTransaction, "max transaction in first 10s");
            }
        }
