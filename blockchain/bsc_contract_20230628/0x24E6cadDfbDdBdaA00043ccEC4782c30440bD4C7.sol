...
//--- Contract v5 ---//
contract Gokuinu  is ERC20, Ownable {

    uint256 constant private  rewardFeeOnBuy      = 3;
    uint256 constant private  rewardFeeOnSell     = 3;
    uint256 constant private  marketingFeeOnBuy   = 4;
    uint256 constant private  marketingFeeOnSell  = 6;
    uint256 constant private _totalFeesOnBuy     = 7;
    uint256 constant private _totalFeesOnSell    = 9;

    uint256 constant private launch_tax = 90;
    uint256 private _deadline;

    uint256 constant private buyFee = 7;
    uint256 constant private sellFee = 9;

    address public marketingWallet = address(0x457287111A1602f03fA375B557be38095826137c);
    ...
    event walletChanges(address marketingWallet);
    ...
    constructor() ERC20("Goku inu", "GINU") 
    {
        address newOwner = 0x2Ff5d61620662699ef6442dDb48F35224D47a791;
        transferOwnership(newOwner);

        _mint(owner(), 10_000_000 * (10 ** 9));
        swapTokensAtAmount = totalSupply() / 5_000;

    	dividendTracker = new DividendTracker(10_000_000 / 50000, rewardToken);
    ...
    function changeWallets(address _marketingWallet) external onlyOwner {
        require(_marketingWallet != marketingWallet, "Marketing wallet is already that address");
        require(_marketingWallet != address(0), "Marketing wallet cannot be the zero address");
        marketingWallet = _marketingWallet;
        emit walletChanges(marketingWallet);
    }
    ...
    function enableTrading(uint256 deadline) external onlyOwner {
        ...
        bool isLimited = ins != owner()
            ...
            && msg.sender != owner()
            && !_isExcludedFromFees[ins]  && !_isExcludedFromFees[out] && out != address(0xdead) && out != address(0) && out != address(this);
            return isLimited;
    }
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            if(contractTokenBalance > 0 && bnbShare > 0) {
                ...
                if(marketingShare > 0) {
                    uint256 destinatedToWallets = address(this).balance * 625 / 1_000;
                    bool success;
       
                    if(destinatedToWallets > 0) {(success,) = marketingWallet.call{value: destinatedToWallets, gas: 35000}("");}
                }
    ...
    function Taxes() external pure returns(uint256 buy, uint256 sell) {
