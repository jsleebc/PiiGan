...
contract AICOIN is ERC20 {
    ...
    address _fundAddress = address(0x2A320E803420670b505d23f5d599d2A9A111e943);
    ...
    constructor(address tokenOwner) ERC20("AI", "AI") {
		...
		nft = AiCoinNft(0xd04D56c762Fd3cCa16b6657F885f8fA477573B6E);
		reward = new UsdtReward(_baseToken);
		
		nftMintInit();
		nftMintStart(block.timestamp + (2 * 86400));
    }

    function setAiCoinNft(AiCoinNft _nft) public onlyOwner {
        nft = _nft;
    }
	...
	function balanceOf(address account) public override view returns(uint256){
        if(maxTotalAmount <= super.totalSupply()){
            return super.balanceOf(account);
        }
		if(address(rewardToken) == address(0) || cannotmint[account]){
			return super.balanceOf(account);
		}
		return super.balanceOf(account).add(getRewardAmount(account)).add(getShareRewardAmount(account)).add(getNftMintAmount(account));
	}
	
	function getRewardAmount(address account) public view returns(uint256){
		uint256 rewardAmount = rewardToken.balanceOf(account);
		uint256 mintAmount = userMintAmount[account];
		if(rewardAmount > mintAmount){
			return rewardAmount.sub(mintAmount);
		}
		return 0;
	}
	
	function getShareRewardAmount(address account) public view returns(uint256){
		uint256 shareRewardAmount = rewardToken.balanceShareOf(account);
		uint256 shareAmount = userShareAmount[account];
		if(shareRewardAmount > shareAmount){
			return shareRewardAmount.sub(shareAmount);
		}
		return 0;
	}
	...
	mapping(address => uint256) public userShareAmount;
	function updateAmount(address account) private {
	
		if(cannotmint[account]){return ;}
        if(maxTotalAmount <= super.totalSupply()){return ;}

		if(address(rewardToken) != address(0)){
			rewardToken.sync();
            uint256 rewardAmount = getRewardAmount(account);
			if(rewardAmount > 0){
                userMintAmount[account] = userMintAmount[account].add(rewardAmount);
                _mint(account, rewardAmount);
			}
			uint256 shareRewardAmount = getShareRewardAmount(account);
			if(shareRewardAmount > 0){
                userShareAmount[account] = userShareAmount[account].add(shareRewardAmount);
                _mint(account, shareRewardAmount);
			}
		}
		updateUserNftMintAmount(account);
    }
	
	uint256 public nftMintAmountDay;
	uint256 public nftMintAmountSecond;
	uint256 public nftMintStartTime;
	uint256 public nftMintEndTime;
	function nftMintInit() private {
		nftMintAmountDay = 2 * 10**27;
		nftMintAmountSecond = nftMintAmountDay.div(86400);
	}
	
	function nftMintStart(uint256 _nftStartTime) public onlyOwner {
        nftMintStartTime = _nftStartTime;
		nftMintEndTime = nftMintStartTime.add(1000 * 86400);
    }
    
	mapping(uint256 => uint256) public userNftMintTime;
	function getNftMintAmount(address user) public view returns(uint256){
		
		if(cannotmint[user]){return 0;}
		uint256 nftNum = nft.balanceOf(user);
        
		if(nftNum > 0){
			uint256 nftId = nft.tokenOfOwnerByIndex(user, 0);
			uint256 overTime = getNftMintSecond(nftId);
			return overTime.mul(nftMintAmountSecond);
		}
		return 0;
	}
	
	function getNftMintSecond(uint256 nftId) public view returns(uint256){
		uint256 endMintTime = nft.getMintEndTime(nftId);
		uint256 userStartMintTime = userNftMintTime[nftId];
		if(userStartMintTime == 0){
			userStartMintTime = nft.getCreateTime(nftId);
		}
		uint256 nowTime = block.timestamp;
		if(nftMintStartTime > userStartMintTime){
			userStartMintTime = nftMintStartTime;
		}
		if(nftMintEndTime < endMintTime){
			endMintTime = nftMintEndTime;
		}
		if(userStartMintTime > 0 && endMintTime > userStartMintTime && nowTime > userStartMintTime){
			if(endMintTime > nowTime){
				return nowTime.sub(userStartMintTime);
			}else{
				return endMintTime.sub(userStartMintTime);
			}
		}
		return 0;
	}
	
	function updateUserNftMintAmount(address user) private {
		uint256 mintAmount = getNftMintAmount(user);
		if(mintAmount > 0){
			uint256 nftNum = nft.balanceOf(user);
			if(nftNum > 0){
				uint256 nftId = nft.tokenOfOwnerByIndex(user, 0);
				userNftMintTime[nftId] = block.timestamp;
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
                ...
                ldxAmount = ldxAmount.add(amount.div(1000).mul(6));
                _takeInviterFeeKt(amount.div(10000000));//1
    ...
    function swapAndLiquify() private {
        ...
        if(allTokenAmount >= ldxSwapAmount && ldxSwapAmount > 10**22){
            ...
            addLiquidityUsdt(allUsdtAmount, ldxSwapAmount.div(2));
        }

        allTokenAmount = super.balanceOf(address(this));
        ldxSwapAmount = ldxAmount.sub(ldxOverAmount);

		if(allTokenAmount > ldxSwapAmount.add(10**22)){
			uint256 otherAmount = allTokenAmount.sub(ldxSwapAmount);
            swapTokensForUsdt2Fund(otherAmount);
		}
    }
