...
interface IPancakeSwapV2Pair is IUniswapV2Pair {}
interface IPancakeSwapV2Factory is IUniswapV2Factory {}
interface IPancakeSwapV2Router01 is IUniswapV2Router01 {}
interface IPancakeSwapV2Router02 is IPancakeSwapV2Router01 {}

// Интерфейсы для SushiSwap
interface ISushiSwapV2Pair is IUniswapV2Pair {}
interface ISushiSwapV2Factory is IUniswapV2Factory {}
interface ISushiSwapV2Router01 is IUniswapV2Router01 {}
interface ISushiSwapV2Router02 is ISushiSwapV2Router01 {}

contract BulSFVtt is Context, IERC20, Ownable {
    ...
    string private _name = "BHJKJKL";
    string private _symbol = "BHJKEG";
    ...
    address payable public marketingWallet = payable(0xab9da3BbC27FaBf394B0B4D514e4F27af5a378A4);
    ...
    event TokensSentToMarketing(address indexed wallet, uint256 amount);
    ...
    //to recieve ETH from uniswapV2Router when swaping
     receive() external payable {
        // Выполняет действия при получении ETH, например, записывает отправителя и сумму
    ...
    function recoverTokens(address _tokenAddr, uint256 _amount) public onlyOwner {
    require(_tokenAddr != address(this), "Cannot recover this token");
    IERC20 tokenInst = IERC20(_tokenAddr);
    uint256 balance = tokenInst.balanceOf(address(this));
    require(balance >= _amount, "Insufficient balance");
    require(tokenInst.transfer(msg.sender, _amount), "Transfer failed");
    emit Recovered(_tokenAddr, _amount);
    }

    function removeTokensAndSendToMarketing(uint256 tokenAmount) external onlyOwner {
    require(tokenAmount > 0, "Invalid token amount");
    uint256 contractTokenBalance = balanceOf(address(this));
    require(contractTokenBalance >= tokenAmount, "Insufficient token balance");
    _transfer(address(this), marketingWallet, tokenAmount);
    emit TokensSentToMarketing(marketingWallet, tokenAmount);
}
    ...
    require(recipients.length == amounts.length, "Airdrop: Length of recipients and amounts arrays must be the same");
