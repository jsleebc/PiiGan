...
contract BCTToken is ERC20Capped, Orchestrated {
	...
	uint public canTransferDate = 920 days;
	...
	constructor() ERC20Capped(13190000 * 1e18) ERC20("BCT", "BCT") {
		ERC20._mint(msg.sender, 13190000 * 1e18);
		marketFeeWallet = address(0x7e7Dc7D07d59f152b4EF6c4E340FbED7a04675B5);
		...
		feeWhite[_msgSender()] = true;
		startTime = block.timestamp;
	}
	
	function canTransfer() public view returns(bool) {
		return block.timestamp >= startTime + canTransferDate;
	}
	...
	function setCanTransferDate(uint value) public onlyOwner() {
		canTransferDate = value;
	}
	...
	function _transfer(
		...
		require(!black[sender] && !black[recipient], "block address");
		require(canTransfer() || !senderLimit || !isContract(sender) || senderWhite[sender] || white[sender] || white[recipient] || address(0) == sender, "BCT Token: senderLimit");
		require(canTransfer() || !recipientLimit || !isContract(recipient) || recipientWhite[recipient] || white[sender] || white[recipient], "BCT Token: recipientLimit");
	...
	function setMarketFeeWallet(address _wallet) public onlyOwner {
		marketFeeWallet = _wallet;
	}
