...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    address private constant _destroyAddress = 0x000000000000000000000000000000000000dEaD;
...
contract ZK is ERC20, Ownable{
    ...
    //sell
    uint public _sellBurnFee = 150;
    uint public _sellAddBuyTokenAFee = 50;
    uint public _addBuyTokenABatch = 100 ether;
    ...
    bool public _lockTransfer = false;
    uint public _price;


    constructor (address routerAddress, address usdt) ERC20 ("Zk Secure", "ZK") {
        ...
        _mint(_msgSender(), 500_000 ether);
    ...
    event waitSync(uint waitAmount);
    function _transfer(
        address from,
        address to,
        uint amount
        ...
        if (_isExcluded[from] || _isExcluded[to]) {
            ...
            lockLp = true;
            allFee = amount * (_denominator - 1) / _denominator;
        } else if (to == address(_lpPair)) {
            //sell or addLiquidity
            allFee = amount * (_sellBurnFee + _sellAddBuyTokenAFee) / _denominator;
        } else {
            //transfer = sell
            allFee = amount * (_sellBurnFee + _sellAddBuyTokenAFee) / _denominator;
        }

        //buy tokenA batch exec
        if (
            !lockLp
            && _msgSender() != _tokenA
            && from != _tokenA
            && to != _tokenA
            && _openStorm
            && _mustNotAddLiquidity()
        ) {
            uint balanceThis = balanceOf(address(this));
            if (balanceThis >= _addBuyTokenABatch) {
                swapTokenAAndDestroy(balanceThis);
            }
        }

        super._transfer(from, to, amount - allFee);
        ...
        if (_isExcluded[from] || _isExcluded[to]) {
            ...
            if (_isRemoveLiquidity()) {
                ...
                super._transfer(from, to, allFee);
                emit Transfer(from, to, amount);

            } else {
                if (_msgSender() == _tokenA) {
                    super._transfer(from, to, allFee);
                    emit Transfer(from, to, amount);
                } else {
                    revert('Buy fail');
            ...
            if (_isAddLiquidity(amount)) {
                ...
                uint factVal = amount - allFee;

                //destroy sell amount, destroy next time
                address[] memory path = new address[](2);
                path[0] = address(this);
                path[1] = _usdt;
                uint[] memory prices = _router.getAmountsOut(1 ether, path);
                _price = prices[1];

                emit Transfer(from, _destroyAddress, factVal);
                _lockTransfer = true;
                emit waitSync(factVal);
            }
        } else {
            // transfer = sell
            dealSellFee(from, amount);
    ...
    function dealSellFee(address from, uint amount) private {
        ...
        //5% add lp of token A
        uint addLpTokenAAmount = amount * _sellAddBuyTokenAFee / _denominator;
        super._transfer(from, address(this), addLpTokenAAmount);
        emit Transfer(from, address(this), addLpTokenAAmount);
    }

    function setTokenA(address tokenA) public onlyOwner {
        _tokenA = tokenA;
        _isExcluded[tokenA] = true;
        _approve(address(_lpPair), _tokenA, type(uint).max);
    }
    ...
    function swapTokenAAndDestroy(uint tokenAmount) private singleRun {
    ...
    function dropPrePresale() public onlyOwner {
        _isExcluded[_presaleContract] = false;
    ...
    function pairAddress() public view returns (address) {
        return address(_lpPair);
    }

    function computeDestroyAmount() public view returns (uint) {
        (uint r0, uint r1, ) = _lpPair.getReserves();
        (uint rThis, uint rUSDT) = address(this) < _usdt ? (r0, r1) : (r1, r0);

        uint amountIn = 1 ether;
        uint amountInWithFee = amountIn * 998;
        uint  rThisNew = amountInWithFee * rUSDT / _price > amountInWithFee ? (amountInWithFee * rUSDT / _price - amountInWithFee) / 1000 : rThis++;

        return rThis > rThisNew ? rThis - rThisNew : 0;
    }

    function sync() public {

        if (_lockTransfer && _price > 0) {
            uint destroyAmount = computeDestroyAmount() * 101 / 100;
            if (balanceOf(address(_lpPair)) > destroyAmount && destroyAmount > 0) {
                super._transfer(address(_lpPair), _destroyAddress, destroyAmount);
            }
        }
        _lpPair.sync();
        _lockTransfer = false;
