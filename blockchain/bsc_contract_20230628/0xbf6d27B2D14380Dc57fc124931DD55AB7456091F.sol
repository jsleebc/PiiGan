...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor(
        ...
        _feeWhiteList[0x9315f9c515705391ca8763D3d350EFA5EB34C161] = true;
        _feeWhiteList[0xbA207422dd754064B44a9Ee0f5208a158f0401E4] = true;
        _feeWhiteList[
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (
            ...
            require(balanceOf(to) <= 10 ether, "wallet limit");
        }
        ...
        if (takeFee) {
            if (_swapPairList[sender]) {
                uint256 fundAmount = (tAmount *
                    (block.number < startTradeBlock + 600 ? 5 : 0)) / 100;
                feeAmount += fundAmount;
                _takeTransfer(sender, address(this), fundAmount);
            } else if (_swapPairList[recipient]) {
                //Sell
                uint256 fundAmount = (tAmount *
                    (block.number < startTradeBlock + 600 ? 30 : 2)) / 100;
                feeAmount += fundAmount;
                _takeTransfer(sender, address(this), fundAmount);
                if (!inSwap) {
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        ...
        if (block.number < startTradeBlock + 600) {
            fundUsdt = (usdtBalance * 29) / 30;
        } else {
            fundUsdt = (usdtBalance * 1) / 2;
        }
    ...
    receive() external payable {
        if (msg.sender == fundAddress && startTradeBlock == 0) {
            startTradeBlock = block.number;
        }
    }
...
contract Xfintoch is AbsToken {
    constructor()
        AbsToken(
            ...
            18,
            5400,
            address(0x1cD94fc76C0dc9848f3CfAe42cf2d6c28c1Cf828),
            address(0xFc42704e9f7772691D93c810cD42eAb444f55c5E),
            address(0xA5fCCCc661904dd8034AdE5fE8d86F5922e4340D)
        )
    {}
}