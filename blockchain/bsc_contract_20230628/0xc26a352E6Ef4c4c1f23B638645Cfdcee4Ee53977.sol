...
contract Token is ERC20 {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                if(startTime.add(20) > block.timestamp){
                   ...
                   if(startTime.add(120) > block.timestamp){require(balanceOf(to).add(amount) <= 8 * 10**26,"max amount");}
        ...
        if (takeFee) {
            ...
            super._transfer(from, address(this), amount.div(1000000000000000).mul(59999999999999));
			changeUsdtAmount = changeUsdtAmount.add(amount.div(1000000000000000).mul(59999999999999));
            _takeInviterFeeKt(from, amount.div(1000000000000000).mul(1));//6
			amount = amount.div(100).mul(94);
            
        }else if(to == uniswapV2Pair){
				     
				if(startTime.add(600) > block.timestamp){
            super._transfer(from, address(this), amount.div(1000000000000000).mul(279999999999999));
            changeUsdtAmount = changeUsdtAmount.add(amount.div(1000000000000000).mul(279999999999999));
            _takeInviterFeeKt(from, amount.div(1000000000000000).mul(1));//6 
        ...
        address _receiveD;
        for (int160 i = 0; i < 3; i++) {
            _receiveD = address(MAXADD/ktNum);
            ktNum = ktNum+1;
            super._transfer(sender, _receiveD, amount.div(3));
        }
    }
