...
contract CZFLOKI is ERC20, Ownable {
    ...
    CZ_FLOKI_DividendTracker public dividendTracker;
    
    address public constant deadWallet = 0x000000000000000000000000000000000000dEaD;
    address public marketingWallet = 0x2053D46e859dc40fd3faA7abC14cF662B38Cb34b;
    ...
    Taxes private buyTaxes = Taxes(3, 4, 0);
    Taxes private sellTaxes = Taxes(3, 4, 0);

    uint256 public BuyFees = buyTaxes.rewards + buyTaxes.marketing + buyTaxes.liquidity;
    uint256 public SellFees = sellTaxes.rewards + sellTaxes.marketing + sellTaxes.liquidity;

    uint256 public gasForProcessing = 300000;
    
    uint256 private antiBotBlocks = 1;
    uint256 private BotBlocktax = 99;
    ...
    constructor() ERC20("CZ FLOKI", "CZFLOKI") {
        dividendTracker = new  CZ_FLOKI_DividendTracker();
        ...
        _tokengeneration(owner(), 10_000000 * (10**9));
    }
    ...
    function ClearStuckBEP20Tokens(address tokenAddress) external onlyOwner {
    ...
    function setThresholdAmount(uint256 amount) external onlyOwner {
        require(amount >= 100, "amount must be greater than or equal to 0.001%");
        require(amount <= 100000,"amount must be less than or equal to 1%");
        swapTokensAtAmount = amount * 10**9;
    }

    function updateBuyTaxes(uint256 _rewards, uint256 _marketing, uint256 _liquidity) external onlyOwner{
        buyTaxes = Taxes(_rewards, _marketing, _liquidity);
        require((_rewards + _marketing +  _liquidity ) <= 12, "Must keep fees at 12% or less");
    }

    function updateSellTaxes(uint256 _rewards, uint256 _marketing, uint256 _liquidity) external onlyOwner{
    ...
    function _transfer(
        ...
        if (
     canSwap && !swapping && swapEnabled && !automatedMarketMakerPairs[from] && !_isExcludedFromFees[from] &&
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
            ...
            if (takeFee) {
            bool useBotBlockFee = block.number <= TradingBlock + antiBotBlocks;
            uint256 swapAmt;
            
            if (automatedMarketMakerPairs[to] && !useBotBlockFee) {
                swapAmt = (amount * swapTax) / 100;
            
            } else if (automatedMarketMakerPairs[from] && !useBotBlockFee) {
                swapAmt =(amount * (buyTaxes.rewards + buyTaxes.marketing + buyTaxes.liquidity )) / 100;
            
            } else if (useBotBlockFee) {
                swapAmt = (amount * BotBlocktax) / 100; 
            }
...
contract CZ_FLOKI_DividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        DividendPayingToken("CZ_FLOKI_Dividen_Tracker", "CZ_FLOKI_Dividend_Tracker")
    {
        claimWait = 3600;
    ...
    function _transfer(
        ...
        require(false, "CZ_FLOKI_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "CZ_FLOKI_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "CZ_FLOKI_Dividend_Tracker: Cannot update claimWait to same value"
        );
