...
contract Token is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("BABYRFD", "BABYRFD") {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                if(startTime.add(20) > block.timestamp){
                   ...
                   if(startTime.add(60) > block.timestamp){require(balanceOf(to).add(amount) <= 80 * 10**26,"max amount");}
        ...
        if (takeFee) {
			...
			amount = amount.div(100).mul(100 - sellRate);
                ...
                super._transfer(from, _fundAddress, amount.div(100).mul(49));
				amount = amount.div(100).mul(51);
			}
        } 
