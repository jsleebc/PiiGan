...
contract BIDA is ERC20, Ownable {
    ...
    uint256 public maxSellTransactionAmount = 10_000_000 * (10**18); // @note Equals 1e+16 | 10,000,000

    uint256 public swapTokensAtAmount = 1_000_000 * (10**18); // @note Equals 1e+14 | 100,000
    ...
    uint256 public totalFees = SellFee.add(liquidityFee);
    ...
    event SetTrading(bool indexed value); // SetTrading
    ...
    event FeesUpdated(uint256 indexed newSaleFee, uint256 indexed newLiquidityFee);
    ...
    function updateFees(uint256 salePerc, uint256 liquidityPerc) external onlyOwner {
        emit FeesUpdated(salePerc, liquidityPerc);
        SellFee = salePerc;
        liquidityFee = liquidityPerc;
        totalFees = SellFee.add(liquidityFee);
    }
    ...
    function _transfer(
        ...
        if (
            !swapping &&
            tradingEnabled && // @note tradingIsEnabled
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        (bool success,) = address(operationsWallet).call{value: dividends}("");
    ...
    function withdrawIERC20(address _token, address recipient) external onlyOwner {
        require(_token != address(this));
        uint256 _tokenBalance = IERC20(_token).balanceOf(address(this));        
        IERC20(_token).transfer(recipient, _tokenBalance);
    }
}