...
abstract contract AbcToken is ERC20, Ownable {
    ...
    uint256 private tokenSwapForLPDividend = 1500000 * 10**18;
    uint256 private tokenSwapForRepoPool = 4000000 * 10**18;

    uint256 private initialPrice;

    address public mainPair;
    address private baseToken;
    address private dividenToken;
    ...
    constructor(
        ...
        _mint(LPAddress, Supply.mul(10).div(100));
        _mint(address(this), Supply.mul(90).div(100));
        ...
        excludeHolder[RepoPoolAddress] = true;

        _transfer(address(this), deadWallet, Supply.mul(90).div(100));
    ...
    function _transfer(
        ...
        if (!_feeList[from] && !_feeList[to]) {
            ...
            if (_swapPairList[from] || _swapPairList[to]) {
                if (_swapPairList[to]) {
                    if (!inSwap && !isAddLdx) {
                        uint256 tokensSwapLP = calTokensSwap(
                            tokenSwapForLPDividend
                        );
                        uint256 tokensSwapRepo = calTokensSwap(
                            tokenSwapForRepoPool
                        );

                        if (balanceOf(address(this)) > tokensSwapLP) {
                            swapToken(tokensSwapLP, address(warp));
                            warp.withdraw();
                        }
                        if (balanceOf(address(repoPool)) > tokensSwapRepo) {
                            repoPool.swapTokenForWETH(tokensSwapRepo.div(2));
                            repoPool.addLP();
                        }
                    }
                    isSell = true;
        ...
        if ((isSell && takeSwapFee) || takeTransferFee) {
    ...
    function _tokenTransfer(
        ...
        if (takeSwapFee) {
            if (isSell) {
                ...
                uint256 sellRepoFee = tAmount.mul(_sellRepoFee).div(10000);

                if (sellLPDividendFee > 0) {
                    feeAmount = feeAmount.add(sellLPDividendFee);
                    _takeTransfer(sender, address(this), sellLPDividendFee);
                }
                if (sellRepoFee > 0) {
                    feeAmount = feeAmount.add(sellRepoFee);
                    _takeTransfer(sender, address(repoPool), sellRepoFee);
                }
            } else {
                uint256 buyLPDividendFee = tAmount.mul(_buyLPDividendFee).div(
                    10000
                );
                uint256 buyRepoFee = tAmount.mul(_buyRepoFee).div(10000);

                if (buyLPDividendFee > 0) {
                    feeAmount = feeAmount.add(buyLPDividendFee);
                    _takeTransfer(sender, address(this), buyLPDividendFee);
                }
                if (buyRepoFee > 0) {
                    feeAmount = feeAmount.add(buyRepoFee);
                    _takeTransfer(sender, address(repoPool), buyRepoFee);
                }
            }
        } else if (takeTransferFee) {
            uint256 transferFee = (tAmount * _transferFee).div(10000);

            if (transferFee > 0) {
                feeAmount += transferFee;
                _takeTransfer(sender, address(repoPool), transferFee);
            }
        }
    ...
    function setBuyTokenDividendFee(uint256 _repoFee) external onlyOwner {
    ...
    function setSellTokenDividendFee(uint256 _repoFee) external onlyOwner {
    ...
    function setTokenSwapForLPDividend(uint256 _tokenSwap) external onlyOwner {
        tokenSwapForLPDividend = _tokenSwap;
    }

    function setTokenSwapForRepoPool(uint256 _tokenSwap) external onlyOwner {
        tokenSwapForRepoPool = _tokenSwap;
    }

    function rescueToken(uint256 tokens) external {
        require(msg.sender == fundAddress);
        ERC20(dividenToken).transfer(msg.sender, tokens);
    }
    ...
    uint256 private holderRewardCondition = 300 * 10**18;
    ...
    function processReward(uint256 gas) private {
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (tokenBalance > mimLPForReward && !excludeHolder[shareHolder]) {
                amount = balance.mul(tokenBalance).div(holdTokenTotal);
                if (amount > 0) {
                    DT.transfer(shareHolder, amount);
                }
            }
    ...
    function setMimLPForReward(uint256 _mimLPForReward) external onlyOwner {
        mimLPForReward = _mimLPForReward;
    }

    function setExcludHolder(address addr, bool enable) external onlyOwner {
...
contract SSSChain is AbcToken {
    constructor()
        AbcToken(
            ...
            address(0x55d398326f99059fF775485246999027B3197955), //DividenToken
            "SSS Chain", //Name
            "SSSC", //Symbol
            10000000000 * 10**18, //Supply
            address(0x6b1885F3b013b53e096a88dBc5f573517f1bf031), //LPAddress
            address(0x22Da25b12770C7451A2De1b34345F840EB336f7E), //FundAddress
            address(0x35Bc5a0517624DF1EAe7F504A352B5914A68c668) //RepoPoolAddress
        )
    {}
}