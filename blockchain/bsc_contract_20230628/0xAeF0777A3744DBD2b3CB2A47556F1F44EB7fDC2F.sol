...
contract BABYPEPE is ERC20, Ownable {
    ...
    constructor(
        ...
        require(
            ...
            'BABYPEPE: fee account from the zero address'
        );
        require(feePercent <= PRECISION, 'BABYPEPE: incorrect fee percent');
        require(owner != address(0), 'BABYPEPE: incorrect owner address');
        require(mintToAddr != address(0), 'BABYPEPE: incorrect mintToAddr address');
        if (_msgSender() != owner) {
    ...
    function _transfer(
        ...
        require(sender != address(0), 'BABYPEPE: transfer from the zero address');
        require(recipient != address(0), 'BABYPEPE: transfer to the zero address');
