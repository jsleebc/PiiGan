...
contract PegMartik {
    ...
    string constant _name = "Peg-Martik";
    string constant _symbol = "MTKP";
    ...
    function toPegMartik(uint256 amount) external {
        require(migrate);
...
function setFees(
    ...
    require(
        ...
        _reflectionFee +
        _burnFee <= 1000,
        "Buy fees cannot exceed 10%"
    );
    require(
        _sellReflectionFee +
        _sellBurnFee +
        _sellMarketingFee <= 1000,
        "Sell fees cannot exceed 10%"
    );
