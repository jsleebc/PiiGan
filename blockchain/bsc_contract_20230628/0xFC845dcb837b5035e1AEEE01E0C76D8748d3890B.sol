...
contract AstrospacesBurnToken is Context, IERC20, Ownable {
    ...
    constructor (
        ...
        address router_,
        uint16 declare_tax_fee,
        uint16 declare_liquidity_fee,
        uint16 declare_burn_fee,
        ...
        numTokensSellToAddToLiquidity = _tTotal.div(1000);

        _taxFee = declare_tax_fee;
        _liquidityFee = declare_liquidity_fee;
        _burnFee = declare_burn_fee;

        require(
            _taxFee + _burnFee + _liquidityFee <= 25,
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
        require(maxTxPercent >= 1, "Max. txn size must be higher than or equal to 1%.");
        _maxTxAmount = _tTotal.mul(maxTxPercent).div(100);
    }

    function setMaxWalletPercent(uint256 maxWalletPercent) external onlyOwner() {
        require(maxWalletPercent >= 1, "Max. wallet size must be higher than or equal to 1%.");
    ...
    function _transfer(
        ...
        if (
            ...
            require(heldTokens + amount <= _maxWalletAmount, "Max wallet limited.");
        }
