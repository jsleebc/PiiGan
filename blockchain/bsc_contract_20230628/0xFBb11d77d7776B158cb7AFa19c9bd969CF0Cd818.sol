        ...
        if (!_feeWhiteList[from] && !_feeWhiteList[to] && isChangeFee) {
            if (_swapPairList[from] || _swapPairList[to]) {
                if (_swapPairList[from]) {
                    _beforeTransfer(to);
                    uint256 buyFell = amount.mul(_buySlippage).div(10000);
                ...
                if (_swapPairList[to]) {
                    ...
                    uint256 sellFell = amount.mul(_sellSlippage).div(10000);
...
contract BitcoinKingToken is ABS {
    constructor(string memory name, string memory symbol) ABS(name, symbol) {
        _mint(msg.sender, 21000000 * 10 ** decimals());
        setEnableWhitelist(true);
        setMarketingWallet(0x5604F94BAA87AaDD41860dC02Db8C078d9D3CB25);
        setDevWallet(0x5E5e5ABE7957CB256cd4AEF9b0Ca75636A311240);
        setBuySlippage(300);
        setSellSlippage(300);
