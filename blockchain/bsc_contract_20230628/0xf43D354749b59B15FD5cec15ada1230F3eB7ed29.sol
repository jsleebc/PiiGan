...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            if (_swapPairList[sender]) {//Buy
                uint256 fundAmount = tAmount * (_buyFundFee + _buyFundFee2 + _buyLPFee) / 10000;
                if (fundAmount > 0) {
                    feeAmount += fundAmount;
                    _takeTransfer(sender, address(this), fundAmount);
                }
            } else if (_swapPairList[recipient]) {//Sell
                uint256 fundAmount = tAmount * (_sellFundFee + _sellFundFee2 + _sellLPFee) / 10000;
                if (fundAmount > 0) {
                    feeAmount += fundAmount;
                    _takeTransfer(sender, address(this), fundAmount);
                }
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        ...
        uint256 totalFee = fundFee + fundFee2 + lpFee;
        totalFee += totalFee;
    ...
    function setBuyFee(
        uint256 buyFundFee, uint256 buyFundFee2, uint256 lpFee
    ) external onlyOwner {
    ...
    function setSellFee(
        uint256 sellFundFee, uint256 sellFundFee2, uint256 lpFee
    ) external onlyOwner {
...
contract PepeKing is AbsToken {
    ...
    //
        10000000000000,
    //Fund
        address(0x6175194082B2709631D7E7bE6535a3698A569988),
    //Fund2
        address(0xE514c38422181144Ed7567EC412656Aa9CC4560c),
    //Received
        address(0xcD1b685179D60608cF5a288903211b126F4B72aB)
    ){
