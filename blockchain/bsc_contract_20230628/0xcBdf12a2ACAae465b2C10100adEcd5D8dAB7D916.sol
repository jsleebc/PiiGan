...
contract PepeCash is ERC20, Ownable {
    ...
    PepeCashDividendTracker public dividendTracker;
    ...
    constructor() ERC20("PepeCash", "PEPECASH") {
        marketingWallet = payable(0x6aD3C08646c608Cc4B257433402C7C78DB944ff6);
        devWallet = payable(0xb671bB45182a04C3F0E4750F24514748b685d3B9);
        ...
        dividendTracker = new PepeCashDividendTracker(
            payable(this),
            router,
        ...
        maxWallet = totalTokenSupply / 200000000000; // 0.005%
        swapTokensAtAmount = totalTokenSupply / 200000000000; // 0.05%;
...
contract PepeCashDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    PepeCash public PepeCashContract;
    ...
    constructor(
        ...
        defaultToken = token;
        PepeCashContract = PepeCash(mainContract);
    ...
    function _transfer(
        ...
        require(false, "PepeCash_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "PepeCash_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PepeCash contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(PepeCashContract);

            uint256 prevBalance = PepeCashContract.balanceOf(address(this));
            ...
            {
                uint256 received = PepeCashContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    PepeCashContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
