...
Website: https://oddpaws.site/
TG: https://t.me/OddPawsBSC
Twitter: https://twitter.com/OddPawsBSC
...
contract OddPaws is IERC20, Ownable {
    ...
    string private constant _symbol = 'OPAWS';
    uint8 private constant _decimals = 9;
    uint256 private _totalSupply = 33 * 10**6 * (10 ** _decimals);
    ...
    function updateAddresses(address development, address marketing, address lp) external onlyOwner {
        development_receiver = development; 
        marketing_receiver = marketing;
        liquidity_receiver = lp;
    }
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap {
        uint256 _denominator = uint256(40).add(uint256(30)).mul(uint256(2));
        uint256 tokensToAddLiquidityWith = tokens.mul(uint256(0)).div(_denominator);
        uint256 toSwap = tokens.sub(tokensToAddLiquidityWith);
        uint256 initialBalance = address(this).balance;
        swapTokensForETH(toSwap);
        uint256 deltaBalance = address(this).balance.sub(initialBalance);
        uint256 unitBalance= deltaBalance.div(_denominator.sub(uint256(0)));
        uint256 ETHToAddLiquidityWith = unitBalance.mul(uint256(0));
        if(ETHToAddLiquidityWith > uint256(0)){addLiquidity(tokensToAddLiquidityWith, ETHToAddLiquidityWith); }
        uint256 marketingAmt = unitBalance.mul(2).mul(uint256(40));
