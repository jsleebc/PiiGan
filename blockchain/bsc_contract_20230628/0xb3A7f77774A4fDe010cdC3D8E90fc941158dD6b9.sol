...
contract BasedBeans is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BasedBeansDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Based Beans", "BASED") {
        marketingWallet = payable(0xc06Ca75eb2c227b2b374549FA667D5dB68aDbaD5);
        devWallet = payable(0x25cCFa8f1AbB07d1a86C1D1c621cD583aA91aaC0);
        ...
        dividendTracker = new BasedBeansDividendTracker(
            payable(this),
            router,
...
contract BasedBeansDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    BasedBeans public BasedBeansContract;
    ...
    constructor(
        ...
        defaultToken = token;
        BasedBeansContract = BasedBeans(mainContract);
    ...
    function _transfer(
        ...
        require(false, "BasedBeans_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "BasedBeans_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BasedBeans contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BasedBeansContract);

            uint256 prevBalance = BasedBeansContract.balanceOf(address(this));
            ...
            {
                uint256 received = BasedBeansContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BasedBeansContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
