...
contract PEPETH is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    PEPETHDividendTracker public dividendTracker;
    ...
    constructor() ERC20("PEPETH", "PEPETH") {
        marketingWallet = payable(0xB4E3554e0f41c3eAFfE15581BA1dd42665B988a0);
        devWallet = payable(0x15442a7e5d1De8549323F814DD74739b12ee5cEd);
        ...
        dividendTracker = new PEPETHDividendTracker(
            payable(this),
            router,
...
contract PEPETHDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        PEPETHContract = PEPETH(mainContract);
    ...
    function _transfer(
        ...
        require(false, "PEPETH_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "PEPETH_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PEPETH contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(PEPETHContract);

            uint256 prevBalance = PEPETHContract.balanceOf(address(this));
            ...
            {
                uint256 received = PEPETHContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    PEPETHContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
