...
contract FuckPepe is ERC20, Ownable {
    ...
    FuckPepeDividendTracker public dividendTracker;
    ...
    constructor() ERC20("FuckPepe", "FPEPE") {
        marketingWallet = payable(0x1F7a029F04c3Fde8A9aD387689ED3cAc100C14F2);
        devWallet = payable(0x5A7A4Dc3789dbC0BE43F9D27664e32aE0e9070F8);
        ...
        dividendTracker = new FuckPepeDividendTracker(
            payable(this),
            router,
        ...
        uint256 totalTokenSupply = (420_000_000_000) * (10**18);
        _mint(owner(), totalTokenSupply); // only time internal mint function is ever called is to create supply
        maxWallet = totalTokenSupply / 42000000000;
        swapTokensAtAmount = totalTokenSupply / 42000000000;
    ...
    function setmaxWallet(uint256 value) external onlyOwner {
        value = value * (10**18);
        require(value >= _totalSupply / 42000000000, "max wallet cannot be set to less than 0.05%");
        maxWallet = value;
    }
...
contract FuckPepeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    FuckPepe public FuckPepeContract;
    ...
    constructor(
        ...
        defaultToken = token;
        FuckPepeContract = FuckPepe(mainContract);
    ...
    function _transfer(
        ...
        require(false, "FuckPepe_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "FuckPepe_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FuckPepe contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(FuckPepeContract);

            uint256 prevBalance = FuckPepeContract.balanceOf(address(this));
            ...
            {
                uint256 received = FuckPepeContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    FuckPepeContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
