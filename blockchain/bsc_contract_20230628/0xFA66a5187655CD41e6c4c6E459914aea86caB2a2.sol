...
contract SevenC is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 2800000 * 10**18;

    uint256 public _tTaxFeeTotal;

    string private _name = "SevenC";
    string private _symbol = "SevenC";
    ...
    address private developer =
        address(0xF4314c54b5B38A3265EB3FaD21F0f819E49AFFa3); //2%
    address private market =
        address(0x21A99FeA7F0aBAad97dc6Cdb32349339143F2e2b); //1%
    ...
    function setBlacks(address account, bool _b) public onlyOwner {
    ...
    function _transfer(
        ...
        require(!blacks[from], "send is blacklist!");
        require(!blacks[to], "receiver is blacklist!");
    ...
    function _transferStandard(
        ...
        (uint256 tTransferAmount, uint256 tDev, uint256 tFee) = _getTValues(
            tAmount
        );
    ...
    function _takeTaxMarketing(uint256 tFee) private {
        _tOwned[market] = _tOwned[market].add(tFee);
    }
        ...
        )
    {
        ...
        tDev = tAmount.mul(2).div(100); //foundation
        tFee = tAmount.mul(1).div(100); //marketing
        tTransferAmount = tAmount.sub(tDev).sub(tFee);
        return (tTransferAmount, tDev, tFee);
    }
