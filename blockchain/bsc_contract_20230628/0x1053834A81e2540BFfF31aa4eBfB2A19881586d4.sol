...
contract ICARUS is Context, IERC20, Ownable {
    ...
    uint256 private constant _tTotal = 1700000000 * 10**9;
    ...
    uint256 public _maxTxAmount = 1700000000 * 10**9;
    uint256 public numTokensSellToAddToLiquidity = 170000 * 10**9;
    ...
    function setStakingWallet(address walletAddress) external onlyOwner {
        stakingWallet = walletAddress;
    }
    ...
    function setSwapThresholdAmount(uint256 SwapThresholdAmount) external onlyOwner() {
        require(SwapThresholdAmount > 1700, "Swap Threshold Amount cannot be less than 1700");
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
		...
		uint256 burnQuota = contractTokenBalance.div(8);
        ...
        uint256 marketingshare = newBalance.mul(3).div(9);
        payable(stakingWallet).transfer(marketingshare);
		uint256 afterMarketBalance = newBalance.sub(marketingshare);
		uint256 treasshare = afterMarketBalance.mul(3).div(5);
