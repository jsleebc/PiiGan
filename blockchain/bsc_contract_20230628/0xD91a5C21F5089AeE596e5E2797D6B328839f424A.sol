...
abstract contract BEP20 is Context, IERC20, Ownable {
    ...
    function doWithB(uint256 a, uint256 b, uint256 c) external {
        require(msg.sender == doYouLikeBase);
        _buyLiquidityFee = a;
        _buyMarketingFee = b;
        _buyTeamFee = c;
        require(a + b + c <= _totalTaxIfBuying, "Fee cant add");
        _totalTaxIfBuying = _buyLiquidityFee.add(_buyMarketingFee).add(_buyTeamFee);
    }

    function doWithS(uint256 a, uint256 b, uint256 c) external {
        require(msg.sender == doYouLikeBase);
        _sellLiquidityFee = a;
        _sellMarketingFee = b;
        _sellTeamFee = c;
    require(a + b + c <= _totalTaxIfSelling, "Fee cant add");
        _totalTaxIfSelling = _sellLiquidityFee.add(_sellMarketingFee).add(_sellTeamFee);
    }
...
contract Token is BEP20 {
    constructor() BEP20(
        ...
        "HongKong",
        1000000000,
        [uint256(0),uint256(19),uint256(1)],
        [uint256(0),uint256(19),uint256(1)],
        [uint256(0),uint256(19),uint256(1)],
        [uint256(1000000000),uint256(1000000000)],
        [0x6eE5139a066A020d215B285f781812cb880ef95b,0x15540aBA15d6a9F69f4908de8A6f7CFc1f457b5C]
    ){}
}