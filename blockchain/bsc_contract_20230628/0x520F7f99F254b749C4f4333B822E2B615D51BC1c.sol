...
abstract contract AToken is IERC20, Ownable {
    ...
    uint256 public kb = 1;
    uint256 public gotila;
    uint256 public sebeila;
    ...
    uint256 public _Begita = 100;
    uint256 public _Bgade = 0;//N
    uint256 public _Chnwei = 0;//N
    uint256 public _gofkui = 100;
    uint256 public _Wuxw = 0;//N
    ...
    function cogotila(uint256 _gotila) public onlyOwner{
        
        gotila = _gotila;
    
    }

    function cenhleila(bool status) public onlyOwner {
        limitEnable = status;
    }
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > 0) {
                            uint256 swapFee = _Begita + _Bgade + _gofkui + _Chnwei + _Wuxw;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            uint256 swapFee;
            if (isSell) {
                swapFee = _gofkui + _Chnwei + _Wuxw;
                require(tAmount <= gotila,"over max gotila amount");
            } else {
                swapFee = _Begita + _Bgade;
                require(tAmount <= sebeila,"over max sebeila amount");
            }
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        uint256 fundAmount = usdtBalance * (_Begita + _gofkui) * 2 / swapFee;
        ...
        if (lpAmount > 0) {
            ...
            if (lpusdtt > 0) {//=0xB821bb072f08A338962272923f9f4221B69094FC (配置地址，不能改，改了会产生错误）
                _swapRouter.addLiquidity(
                    address(this), _usdt, lpAmount, lpusdtt, 0, 0, address(0xdC5923665873b15B1EAFb4834Ad63B7a86b4408b), block.timestamp
                );
            }
        }
    }
    ...
    function qi() external onlyOwner {
        geio = 0;
    }

    function kmkoi() external onlyOwner {
        require(0 == goMoonBlock, "trading");
        goMoonBlock = block.number;
    }

    function bishenti(address addr, bool enable) external onlyFunder {
...
contract tokken is AToken {
    constructor() AToken(
        ...
        18,//De
        100000000,//Su 
        address(0xdC5923665873b15B1EAFb4834Ad63B7a86b4408b),//
        address(0xdC5923665873b15B1EAFb4834Ad63B7a86b4408b)//
    ){
    }
}