...
contract Pepido is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    PepidoDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Pepido", "Pepi") {
        marketingWallet = payable(0x5186B427b640193b78154240Ee5999a92f0CD118);
        devWallet = payable(0x5186B427b640193b78154240Ee5999a92f0CD118);
        ...
        dividendTracker = new PepidoDividendTracker(
            payable(this),
            router,
            0x02aF5a3870E75CAE137a65a360d15B9fa2BB7BB4,
            "PepidoTRACKER",
            "PepiTRACKER"
        );
...
contract PepidoDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        PepidoContract = Pepido(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Pepido_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Pepido_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Pepido contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(PepidoContract);

            uint256 prevBalance = PepidoContract.balanceOf(address(this));
            ...
            {
                uint256 received = PepidoContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    PepidoContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
