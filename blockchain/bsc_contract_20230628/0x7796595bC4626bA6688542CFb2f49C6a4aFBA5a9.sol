...
contract Bpt is ERC20, Ownable {
    ...
    BptDividendTracker public dividendTracker;
    ...
    constructor() ERC20("BULL PEPE", "BPT") {
        marketingWallet = payable(0x104FE24782E1EA660AD2e59F74926b5069a1A42a);
        devWallet = payable(0xABd1281af7e10A6180AC15cDC45AF992163466ab);
        ...
        dividendTracker = new BptDividendTracker(
            payable(this),
            router,
    ...
    function updateFees(
        ...
        require(totalSellFees <= 80 && totalBuyFees <= 80, "total fees cannot exceed 80% sell or buy");
...
contract BptDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function _transfer(
        ...
        require(false, "Bpt_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Bpt_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BptContract);

            uint256 prevBalance = BptContract.balanceOf(address(this));
            ...
            {
                uint256 received = BptContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BptContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
