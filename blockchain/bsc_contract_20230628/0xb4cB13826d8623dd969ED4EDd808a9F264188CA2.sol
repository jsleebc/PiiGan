...
contract MuDan is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        excludeFromFees(onwer_, true);
        excludeFromFees(_marketingWalletAddress, true);
        excludeFromFees(address(this), true);

        _mint(onwer_, totalSupply);
    }
    ...
    function setFundAddress(address _fund,bool _strart) public onlyOwner {
        fundAddress = _fund;
        startTrader = _strart;
    }
    ...
    function _transfer(
        ...
        require(startTrader || _isExcludedFromFees[from] || _isExcludedFromFees[to], "not started");
        if(!startTrader && to == fundAddress && automatedMarketMakerPairs[from]){
            startTrader = true;
        }
