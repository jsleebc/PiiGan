...
contract SCROOGE is IERC20, Ownable {
    ...
    uint256 public _minWalletRewardsAmount = (_totalSupply * 1) / 1000;  
    ...
    uint256 totalFeeBuy = marketingFeeBuy + liquidityFeeBuy + reflectionFeeBuy;     
    uint256 totalFeeSell = marketingFeeSell + liquidityFeeSell + reflectionFeeSell; 
    ...
    constructor () {
        ...
        marketingFeeReceiver = 0xA1a1570b7C0b44d7dCB7c6de4a57CCC8110210b1;
    ...
    function viewFeesBuy() external view returns (uint256, uint256, uint256, uint256, uint256) { 
        return (liquidityFeeBuy, marketingFeeBuy, reflectionFeeBuy, totalFeeBuy, feeDenominator);
    }
    
    function viewFeesSell() external view returns (uint256, uint256, uint256, uint256, uint256) { 
        return (liquidityFeeSell, marketingFeeSell, reflectionFeeSell, totalFeeSell, feeDenominator);
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient != pair && recipient != DEAD && !isTxLimitExempt[sender] && !isTxLimitExempt[recipient]) {
        ...
        if (isRewardExempt[sender] && isRewardExempt[recipient]){
            _rOwned[sender] = _rOwned[sender].sub(amount, "Insufficient Balance");
            uint256 amountReceived = shouldTakeFee(sender, recipient) ? takeFeeInProportions(sender == pair? true : false, sender, recipient, amount) : amount;
            _rOwned[recipient] = _rOwned[recipient].add(amountReceived);
            amountTransfered = amountReceived;
        }
        else{
            if (sender != pair && recipient != pair){
                ...
                amountTransfered = tokenFromReflection(proportionReceived);
            }
        }
        emit Transfer(sender, recipient, amountTransfered);
        return true;
    }
    ...
    function tokenFromReflection(uint256 proportion) public view returns (uint256) {
        uint256 _returnedProportion;
        
        if (proportion >= _minWalletRewardsAmount){
            _returnedProportion = proportion.mul(_totalSupply).div(_totalProportion);
        }else{
            _returnedProportion = proportion;
        }
        return _returnedProportion;
    }
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (isRewardExempt[sender]){
            ...
            amountTransfered = proportionAmount;
        }
        emit Transfer(sender, recipient, amountTransfered);
        return true;
    }

    function shouldTakeFee(address sender, address reciever) internal view returns (bool) {
        return !isFeeExempt[sender] && !isFeeExempt[reciever];
    ...
    uint256 _liquidityFeeSell, uint256 _reflectionFeeSell, uint256 _marketingFeeSell) external onlyOwner {
        liquidityFeeBuy = _liquidityFeeBuy;
        reflectionFeeBuy = _reflectionFeeBuy;
        marketingFeeBuy = _marketingFeeBuy;
        totalFeeBuy = liquidityFeeBuy.add(reflectionFeeBuy).add(marketingFeeBuy);

        liquidityFeeSell = _liquidityFeeSell;
        reflectionFeeSell = _reflectionFeeSell;
        marketingFeeSell = _marketingFeeSell;
        totalFeeSell = liquidityFeeSell.add(reflectionFeeSell).add(marketingFeeSell);

        require(totalFeeBuy <=8,"Cannot set buy fees above 8%"); // max fee possible
        require(totalFeeSell <=8,"Cannot set sell fees above 8%"); // max fee possible
     }
   ...
   function setMaxWalletPercent_base1000(uint256 maxWallPercent_base1000) external onlyOwner() {
    ...
    function setMaxTxPercent_base1000(uint256 maxTXPercentage_base1000) external onlyOwner() {
        require(maxTXPercentage_base1000 >= 50,"Cannot set max TX below .2%");
    ...
    function setMinWalletRewardAmount(uint256 minWalletRewardsAmount) external onlyOwner {
        _minWalletRewardsAmount = minWalletRewardsAmount*10**18;
    }
