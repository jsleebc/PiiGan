...
contract WAYAWA is ERC20, Ownable {
    ...
    constructor(
        ...
        require(tradeStartTime_ > 0);
        _lastSwapTime = tradeStartTime = tradeStartTime_;
    ...
    function extraFeeRate() public view returns (uint256) {
        if (block.timestamp < tradeStartTime) return 0;
        uint256 result = block.timestamp.sub(tradeStartTime).div(1 minutes);
        return result >= 7 ? 0 : uint256(7).sub(result);
    }
    ...
    function _transfer(
        ...
        require(_isTradeStart(from, to), "WAYAWA: transaction not started");
        if (_shouldSwapFee(from, to)) _swapFee();
        if (_shouldTakeFee(from, to)) amount = _takeFee(from, amount);
        super._transfer(from, to, amount);
    }

    function _isTradeStart(
        address _from,
        address _to
        ...
        return
            ...
            (_from == presaleAddress || _to == presaleAddress) ||
    ...
    function _takeFee(
        ...
        uint256 MFee1 = _amount.mul(marketingFeeRate1.add(extraFeeRate())).div(
            100
        );
        AmountMarketingFee1 += MFee1;

        uint256 MFee2 = _amount.mul(marketingFeeRate2).div(100);
        AmountMarketingFee2 += MFee2;

        uint256 TFee = MFee1.add(MFee2);
        result = result.sub(TFee);
        if (TFee > 0) super._transfer(_from, address(this), TFee);
    }

    function _shouldSwapFee(
        address from,
        address to
    ) private view returns (bool) {
        return
    ...
    function _swapFee() private lockTheSwap {
        if (AmountMarketingFee1 > 0) {
            _swapTokensForUsdt(AmountMarketingFee1, marketingWallet1);
            AmountMarketingFee1 = 0;
        }
        if (AmountMarketingFee2 > 0) {
            _swapTokensForUsdt(AmountMarketingFee2, marketingWallet2);
    ...
    function setTradeStartTime(uint256 _tradeStartTime) external onlyOwner {
        require(_tradeStartTime > 0);
        _lastSwapTime = tradeStartTime = _tradeStartTime;
    }
    ...
    function setMarketingWallet1(address _marketingWallet1) external onlyOwner {
        marketingWallet1 = _marketingWallet1;
    }

    function setMarketingWallet2(address _marketingWallet2) external onlyOwner {
