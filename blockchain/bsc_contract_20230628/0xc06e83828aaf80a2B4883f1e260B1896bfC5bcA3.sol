...
contract HuDex is ERC20, Ownable {
    ...
    {   
        ...
        marketingWallet = 0x8F8B82E2BE225708C1Add1Ce139dfd86Ab7418b0;
    ...
    function excludeFromFees(address _account, bool _excluded) external onlyOwner{
        require(_isExcludedFromFees[_account] != _excluded,"Account is already the value of 'excluded'");
    ...
    function updateFees(uint256 _marketingFeeOnBuy, uint256 _marketingFeeOnSell) external onlyOwner {
        ...
        require(marketingFeeOnBuy <= 5 && marketingFeeOnSell <= 5, "Total Fees cannot exceed the maximum");

        emit UpdateFees(marketingFeeOnBuy, marketingFeeOnSell);
    }

    function changeMarketingWallet(address _marketingWallet) external onlyOwner{
        require(_marketingWallet != marketingWallet,"Marketing wallet is already that address");
        require(_marketingWallet != address(0),"Marketing wallet cannot be the zero address");
        require(!payable(_marketingWallet).isContract() && (msg.sender == tx.origin), "Marketing wallet cannot be a contract");
    ...
    function setSwapTokensAtAmount(uint256 _newAmount) external onlyOwner{
        require(_newAmount > totalSupply() / 1_000_000, "SwapTokensAtAmount must be greater than 0.0001% of total supply");
