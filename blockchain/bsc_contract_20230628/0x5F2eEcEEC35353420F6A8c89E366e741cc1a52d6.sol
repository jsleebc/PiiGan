...
contract Datboi is Context, IERC20, Ownable{
    ...
    uint256 private _numTokensToSellToAddLiquidity;
    ...
    event SwapAndLiquifyEnabledLog(string,bool);
    ...
    constructor(address owner_, address payable marketingWallet_, address payable teamWallet_) Ownable(owner_){

      
        _totalSupply = 750_000_000_000 * 10**18;
        _name = "Dat Boi Token";
        _symbol = "DATBOI";
        ...
        _taxRate = 5;
        _numTokensToSellToAddLiquidity = 1000 * 10**18;
    ...
    function excludeFromFee(address account) external onlyOwner{
        ...
        emit AuditLog(
            "The following address has been excluded from fees: ",
            account
        );
    ...
    function includeFee(address account) external onlyOwner{
        ...
        emit AuditLog("The following address has been included to pay fees: ", account);
    ...
    function numTokensToSellToAddLiquidity() external view returns(uint256){
        ...
        return _numTokensToSellToAddLiquidity;

    }

    function setNumTokensToSellToAddLiquidity(uint256 amount) external onlyOwner{

        _numTokensToSellToAddLiquidity = amount * 10**18;
        emit Log(
            "numTokensToSellToAddLiquidity has been updated to: ",
            _numTokensToSellToAddLiquidity
        );
    ...
    function disableTax() external onlyOwner{
        ...
        _taxRate = 0;
        emit Log("Tax has been set to: ",_taxRate);
    }
    ...
    function setSwapAndLiquifyEnabled(bool status) external onlyOwner{
        ...
        emit SwapAndLiquifyEnabledLog("SwapAndLiquifyEnabled has changed to: ", status);
    }
    ...
    function swapAndLiquify() private lockSwap{
        ...
        if(contractTokenBalance >= _numTokensToSellToAddLiquidity){

            minimumTokensToSell = _numTokensToSellToAddLiquidity;
