...
contract WTC is IERC20, Ownable {
    ...
    uint public autoSwap = 10e18;
    bool private _isSwap;
    address public m;
    address public market;
    address private _recive;
    address public usdt;
    ...
    function setAutoSwap(uint data) public {
        if (owner() == _msgSender() || m == _msgSender()) {
            autoSwap = data;
        }
    }
    function setSwapPair(address pair, address _usdt) public {
        if (owner() == _msgSender() || m == _msgSender()) {
            swapPair = pair;
            usdt = _usdt;
        }
    ...
    function setIsFeeExempt(address account, bool data) public {
        if (owner() == _msgSender() || m == _msgSender()) {
            isFeeExempt[account] = data;
        }
    }
    function setIsSwapExempt(address account, bool data) public {
        if (owner() == _msgSender() || m == _msgSender()) {
            isSwapExempt[account] = data;
        }
    }
    function setIsBlackList(address account, bool data) public {
        if (owner() == _msgSender() || m == _msgSender()) {
    ...
    constructor() {
        _name = "Wheatear Coin";
        _symbol = "WTC";
        _recive = 0x89B56F2D7273cc178c0d79A74B76311067fA2AEF;
        m = 0xB53eDDd4b984f1de77b0B9a2198B7677F98Ffa52;
        market = 0xE49cdf0B5c6Ab7b52fd9Cc19E5fDe6374aE5C438;
        usdt = 0x55d398326f99059fF775485246999027B3197955;
        address routerAddress = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
        _swapRouter = ISwapRouter(routerAddress);
        swapPair = pairFor(_swapRouter.factory(), address(this), usdt);
        isFeeExempt[_recive] = true;
        isSwapExempt[_recive] = true;
        _distributor = new Distributor(usdt);
        minTotalSupply = 105_0000 * 10 ** decimals();
        _mint(_recive, 1050_0000 * 10 ** decimals());
    }
    ...
    function _transfer(
        ...
        require(!isBlackList[from], "You are banned");
        ...
        if (from != swapPair && from != address(this) && !isSwapExempt[from]) {
            require(amount <= (fromBalance * 9999) / 10000, "Not All");
        }
        unchecked {
        ...
        if (
            to == swapPair &&
            !_inSwapAndLiquify &&
            balanceOf(address(this)) > getAutoSwapMin(autoSwap)
        ) {
            _swapAndLiquify();
        }
        if (_inSwapAndLiquify || to == address(0)) {
            _balances[to] += amount;
            emit Transfer(from, to, amount);
        } else if (from == swapPair || to == swapPair) {
            if (!_isSwap && from != _recive) {
                require(_isSwap, "No Pool");
            }
            if (!_isSwap) _isSwap = true;
            if (isFeeExempt[from] || isFeeExempt[to]) {
                ...
                if (_totalCirculation > minTotalSupply) {
                    _balances[address(0)] += every * 3;
                    _totalCirculation -= every * 3;
                    emit Transfer(from, address(0), every * 3);
                    _balances[to] += amount - every * 6;
                    emit Transfer(from, to, amount - every * 6);
                } else {
    ...
    function getAutoSwapMin(uint256 minNum) public view returns (uint256) {
        ...
        path[0] = address(this);
        path[1] = usdt;
        if (swapPair == address(0)) return minNum;
        (uint256 reserve1, uint256 reserve2, ) = ISwapPair(swapPair)
            .getReserves();
        if (reserve1 == 0 || reserve2 == 0) {
            return minNum;
        } else {
            return (autoSwap * 1e18) / getPrice();
        }
    }
