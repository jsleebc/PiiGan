...
contract JFH is ERC20, Ownable {
    ...
    address private constant GSD = 0xB12E8Eb6b1F24e14381514d2f3B75e7c61487016;
    address public hole = address(0xdead);
    address public lpOwner = address(0xdead);
    address public tokenOwner = 0x36e45492a122F4A019A190ba8F4188D83582236d;

    uint256 public numTokensSellToSwap = 8 * 1e18;
    ...
    constructor() ERC20("JFH", "JFH") {
        ...
        IERC20(GSD).approve(pancakeRouterAddr, type(uint).max);

        /*
            _mint is an internal function in ERC20.sol that is only called here,
            and CANNOT be called ever again
        */
        _mint(tokenOwner, 8800 * 1e18);
    }
    ...
    function setHoleAddr(address _hole) external onlyOwner { 
        hole = _hole;
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        if(takeFee) {
            uint256 feeToThis;
            uint256 feeToReceiver;
            uint256 originalAmount = amount;
            if(_isPair[sender]) { //buy
                feeToThis = buyBurnFee + sellLiquifyFee;
                feeToReceiver = buyLpFee;
            } else if (_isPair[recipient]) {
                feeToThis = sellBurnFee + sellLiquifyFee;
                feeToReceiver = sellLpFee;
            }
            ...
            if (feeToReceiver > 0) {
                uint256 feeAmount = originalAmount * feeToReceiver / 100;
                super._transfer(sender, address(dividend), feeAmount);
                amount -= feeAmount;
            }
        }
    ...
    function swapAndDividend(uint256 tokenAmount) private lockTheSwap {
        uint totalBuyShare = buyBurnFee + sellLiquifyFee;
        uint totalSellShare = sellBurnFee + sellLiquifyFee;
    ...
    function dividendToLPHolders(uint256 gas) private {
        ...
        uint256 totalRewards = balanceOf(address(dividend));
        if (totalRewards < 1 * 1e16) {
            return;
        }
        ...
        while (gasUsed < gas && iterations < numberOfTokenHolders) {
            ...
            if (LPAmount >= minAmountForLPDividend) {
                ...
                if (dividendAmount > 0) {
                    uint256 balanceOfThis = balanceOf(address(dividend));
                    if (balanceOfThis < dividendAmount)
                        return;
                    _balances[address(dividend)] -= dividendAmount;
                    _balances[cur] += dividendAmount;
                    emit Transfer(address(dividend), cur, dividendAmount);
                }
    ...
    function swapAndLiquify(uint256 tokens) private {
       // split the contract balance into halves
        ...
        uint256 initialBalance = IERC20(GSD).balanceOf(address(tokenReceiver));
        ...
        uint256 newBalance = IERC20(GSD).balanceOf(address(tokenReceiver)) -initialBalance;
        IERC20(GSD).transferFrom(address(tokenReceiver), address(this), newBalance);
