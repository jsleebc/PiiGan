...
// Contract created at http://paramountnetworktoken.com/
...
contract PARAMONTNETWORK_TOKEN is Context, IERC20 { 
    ...
    address payable private constant Wallet_Fee = payable(0xAc8bc8aC6b2221639D55De13531D271Ccb0C9C90);
    ...
    function Project_Information() external view returns(address Owner_Wallet,
        ...
        string memory Creator = "http://paramountnetworktoken.com";
    ...
    function Contract_SetUp_02__Fees_on_Buy(
        ...
        // Buyer Protection: Max Fee 100% 
        require (Marketing_on_BUY    + 
                 Liquidity_on_BUY    + 
                 Burn_on_BUY         + 
                 Tokens_on_BUY       +
                 Reflection_on_BUY   +
                 _Fee__Buy_Contract <= 100, "E01"); // Total buy fee must be 100 or less 
    ...
    function Contract_SetUp_03__Fees_on_Sell(
        ...
        // Seller Protection: Max Fee 100% 
        require (Marketing_on_SELL  + 
                 Liquidity_on_SELL  + 
                 Burn_on_SELL       + 
                 Tokens_on_SELL     +
                 Reflection_on_SELL +
                 _Fee__Sell_Contract <= 100, "E02"); // Total sell fee must be 100 or less 
    ...
    */
    ...
    These wallets can then be checked for bot activity and blacklisted (and no refunded) if required.
    Block_Early_Buyer_Sells is set to false to allow early buyers to sell normally during Launch mode.
    ...
    */
    ...
    function Maintenance__Remove_Contract_Fee() external payable {
        ...
        require(msg.value == 0*10**18, "E14"); 

        send_BNB(Wallet_Fee, msg.value);

        // Remove Contract Fee(removed)
        _Fee__Buy_Contract  = 0;
        _Fee__Sell_Contract = 0;
...
Reflection Contract (Rewards holders the native token) by PNT

    Website: http://paramountnetworktoken.com
