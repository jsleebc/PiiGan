...
contract PINGDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("PING_Dividend_Tracker", "PING_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "PING_Dividend_Tracker: No transfers allowed");
    }


    function withdrawDividend() public pure override {
        require(false, "PING_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PING contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "PING_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "PING_Dividend_Tracker: Cannot update claimWait to same value");
...
contract PING is ERC20, Ownable, SafeToken {
    using SafeMath for uint256;
    ...
    PINGDividendTracker public dividendTracker;
    ...
    constructor() ERC20("PING", "PING")
    {
        ...
        marketingWallet = payable(0x87461D8a18E9Ace1585485690323fdd9dBfe1054);
        devWallet = payable(0x9Bf624c181EbC641f17eA20DEf292490Aa52774b);


        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(devFee); // total fee transfer and buy


        dividendTracker = new PINGDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "PING: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "PING: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "PING: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }


    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "PING: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "PING: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "PING: Cannot update gasForProcessing to same value");
