...
contract KINGPEPE is Context, IERC20, Ownable {
    ...
    event BulkExcludeFee(address[] indexed accounts, bool state);
    ...
    function bulkExcludeFee(address[] memory accounts, bool state) external onlyOwner {
        ...
        emit BulkExcludeFee(accounts, state);
    }

    function updateMarketingWallet(address newWallet) external onlyOwner {
        require(newWallet != address(0),"Fee Address cannot be zero address");
        marketingWallet = newWallet;
        emit UpdateMarketingWallet(newWallet);
    }
    ...
    function rescueAnyBEP20Tokens(address _tokenAddr) public {
        ...
        IERC20(_tokenAddr).safeTransfer(msg.sender, caBalance);
    }
