...
contract PumbAndDump is ERC20, Ownable {
    ...
    mapping(address => bool) private excludedMaxTxAmount;

    constructor() ERC20("Pumb And Dump", unicode"P&D") {
        ...
        excludedMaxTxAmount[address(_uniswapV2Router)] = true;
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (limitsInEffect == true && from != owner() && to != owner() && to != address(0) && to != address(0xdead)) {
            if (uniswapV2Pair == from && !excludedMaxTxAmount[to]) {
                require(amount <= maxTxAmount, "ERC20: Buy transfer amount exceeds the max transaction amount.");
            } else if (uniswapV2Pair == to && !excludedMaxTxAmount[from]) {
