...
Keyboard Cat: Welcome to the zaniest litter box on the internet! Expect purr-fect 
memes, pawsome puns, and a catnip-fueled crypto journey. Hop in, fur real
...
// Website: https://kcat.io
// Telegram: https://t.me/kcatofficial
...
contract KeyboardCat is IERC20, Ownable {       
    string private constant _name = "Keyboard Cat";
    string private constant _symbol = "KCAT";
    uint8 private constant _decimals = 9;
    uint256 private constant _totalSupply = 777_777_777_777 * 10**_decimals;
    ...
    uint256 public constant maxWalletAmount = 17_258_888_890 * 10**_decimals;
    uint256 private constant contractSwapLimit = 17_258_888_890 * 10**_decimals;
    uint256 private constant contractSwapMax = 8_158_888_890 * 10**_decimals;
    ...
    address payable private constant PianoFund = payable(0xc90b582f29bE129515d880C86f3AfE437E12d661);
    ...
    modifier tradingLock(address sender) {
        require(tradingOpen || sender == deployerAddress || sender == PianoFund);
        _;
    }
    ...
    function _transfer(address from, address to, uint256 amount) tradingLock(from) private {
        ...
        if (from != address(this) && from != PianoFund && to != PianoFund && to != deployerAddress) {
    ...
    function swapback(uint256 tokenAmount) private swapLock {
        ...
        if(contractETHBalance > 0) {
            PianoFund.transfer(contractETHBalance);
        }
    }
