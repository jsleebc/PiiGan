// https://tomchingcheng.com/
// https://t.me/tomchingcheng_erc
...
contract TomChingCheng is ERC20, Ownable {
    ...
    constructor() ERC20("Tom Ching Cheng", "HANJI") {
        ...
        uint256 totalSupply = 8000000000 * 10**decimals();

        maxWallet = maxTxAmount = totalSupply.mul(2).div(100);
        swapTokensAtAmount = totalSupply.mul(1).div(10000);

        developmentWallet = _msgSender();

        buyTax = Taxes(5, 0, 5);
        sellTax = Taxes(35, 0, 35);
    ...
    function updateBuyTaxes(uint256 _development, uint256 _liquidity) external onlyOwner {
        uint256 _total = _development + _liquidity;
        buyTax = Taxes(_development, _liquidity, _total);
    }

    function updateSellTaxes(uint256 _development, uint256 _liquidity) external onlyOwner {
        uint256 _total = _development + _liquidity;
        sellTax = Taxes(_development, _liquidity, _total);
    }
