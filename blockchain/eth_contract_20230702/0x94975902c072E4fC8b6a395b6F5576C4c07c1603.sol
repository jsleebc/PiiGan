...
contract SHIBA275 is IERC20, Ownable {
    using SafeMath for uint256;

    uint256 private _totalSupply;

    string private _name = "SHIBA2.75";
    string private _symbol = "INU2.75";
    ...
    constructor(uint256 feeRewardPct, address feeRewardAddress, address uniRouter, address uniPairFactory) {
        ...
        require(uniPairFactory != address(0));

        uint256 mintAmount = 10000000000 * 10**decimals();
    ...
    function setMaxFeeSwap(uint256 newMaxFeeSwap) external onlyOwner {
        uint256 maxLimit = _totalSupply.div(100); 
        require(newMaxFeeSwap <= maxLimit);
