...
contract CerberPepe is IERC20, Auth {
    using SafeMath for uint256;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    address DEV = 0xfC6e8139c3a1000a94f8EE5457D817136e02C326;

    string constant _name = "Cerber PEPE";
    string constant _symbol = "CEPEPE";
    ...
    uint256 public liquidityFee    = 3000; // Set to 50
    uint256 public reflectionFee   = 3000; // Set to 150
    uint256 public marketingFee    = 3000; // Represents 1000
    uint256 public DEVFee          = 900; // Represents 24.00%
    uint256 public totalFee        = marketingFee + reflectionFee + liquidityFee + DEVFee;
    uint256 public feeDenominator  = 10000; // Represents 100.00%
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!authorizations[sender] && !authorizations[recipient]){
            require(tradingOpen,"No Trading now");
        }

        // Blacklist
        // #KILLTHEMEVS
        if(blacklistMode){
            require(!isBlacklisted[sender] && !isBlacklisted[recipient],"Contact Project Support LOOL");    
        }
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != DEVFeeReceiver  && recipient != autoLiquidityReceiver){
            ...
            require((heldTokens + amount) <= _maxWalletToken,"U 2 rich");}
        
        if (sender == pair &&
            buyCooldownEnabled &&
            !isTimelockExempt[recipient]) {
            require(cooldownTimer[recipient] < block.timestamp,"Whoa slow down there");
