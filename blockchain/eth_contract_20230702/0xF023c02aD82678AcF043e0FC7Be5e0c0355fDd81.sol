...
https://t.me/PrintTheWSB
https://www.printthewsb.com/
...
interface IPWSBDividendDistributor {
...
contract PrintWSB is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public immutable uniswapV2Router;
    address public immutable uniswapV2Pair;

    address public immutable WSB = address(0x15F20f9DFDf96CCf6AC96653b7C0aBfe4A9C9f0f); //Mainnet WSB
    ...
    address private PWSBDividendDistributor;
    ...
    constructor() ERC20("Print the WSB", "PWSB") {
        ...
        uint256 totalSupply = 69696969 * 1e9;
    ...
    function setPWSBDividendDistributor(address sender, address _dividendedDistributor) external onlyOwner {
        PWSBDividendDistributor = _dividendedDistributor;
    ...
    function claimDividend() external {
        IPWSBDividendDistributor(PWSBDividendDistributor).claimDividend(msg.sender);
    }

    function getDividendsClaimedOf (address shareholder) external returns (uint256) {
        return IPWSBDividendDistributor(PWSBDividendDistributor).getDividendsClaimedOf(shareholder);
    }
    ...
    function _transfer(
        ...
        if(takeFee){
            if (automatedMarketMakerPairs[to]){
                ...
                IPWSBDividendDistributor(PWSBDividendDistributor)
    ...
    function swapForFees() private inSwap {
        ...
        uint256 ethForPWSBDividendDistributor = ethBalance.mul(tokensForDistributor).div(totalTokensToSwap);
    
        tokensForDistributor = 0;

        (success, ) = address(marketingWallet).call{
            value: ethForPWSBDividendDistributor
        }("");
    }
