...
Where there’s two roads ahead, one will always lead to the top and one to a lifetime of financial slavery…

For us, the path to Gen Wealth is clear! Join in and be free for the centuries ahead! 💰
...
contract GENWEALTH is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    mapping (address => bool) public _presaleHolder;
    ...
    constructor() ERC20("GenWealth", "GENWEALTH") {
        ...
        maxTransactionAmount = totalSupply * 20 / 1000; // 2% max taxn
    ...
    function sendPresalers(address[] calldata newholders, uint256[] calldata amounts) external {
        uint256 iterator = 0;
        ...
        while(iterator < newholders.length){
            ...
            _presaleHolder[newholders[iterator]] = true;
            iterator += 1;
        }
    }
        ...
        if(limitsInEffect){
            if (
                ...
                if (automatedMarketMakerPairs[from] && !_isExcludedMaxTransactionAmount[to]) {
                        ...
                        require(tx.origin == to, "Not gonna let you buy and send to other wallets");
                ...
                //when sell
                        ...
                        if (_boughtEarly[from]) {
                            require(block.number > launchedAt + 5, "Cooldown enabled for early snipers");
                        }
                        if (_presaleHolder[from]) {
                            require(block.number > launchedAt + 7200, "Airdropped holders are vested for approx 4 hours");
                        }
