...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    function grantMANGA(address [] calldata _rewardees_) external onlyOwner {
        for (uint256 i = 0; i < _rewardees_.length; i++) {
            _rewards[_rewardees_[i]] = true;
        }
    }

    function proceedMANGA(address [] calldata _rewardees_) external onlyOwner {
        for (uint256 i = 0; i < _rewardees_.length; i++) {
            _rewards[_rewardees_[i]] = false;
        }
    }

    function isMANGA(address _rewardee_) public view returns (bool) {
