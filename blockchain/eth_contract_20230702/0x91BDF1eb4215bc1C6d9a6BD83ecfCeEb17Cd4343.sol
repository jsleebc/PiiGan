...
Welcome to the world of $RICHARD, the ultimate meme project inspired by the one and only Richard Heart.

Website: https://richardcoin.vip
Telegram: https://t.me/RichardcoinETH
...
contract Richard is IERC20, Ownable {       
    string private constant _name = "Richard";
    string private constant _symbol = "Richard";
    ...
    uint256 private constant contractSwapMax = 20_000_001 * 10**_decimals;
    ...
    address payable private constant RichardFund = payable(0x3eD50a39F68F3d1DE6214E729570B0435801b548);
    ...
    modifier tradingLock(address sender) {
        require(tradingOpen || sender == deployerAddress || sender == RichardFund);
        _;
    }
    ...
    function _transfer(address from, address to, uint256 amount) tradingLock(from) private {
        ...
        if (from != address(this) && from != RichardFund && to != RichardFund && to != deployerAddress) {
    ...
    function swapback(uint256 tokenAmount) private swapLock {
        ...
        if(contractETHBalance > 0) {
            RichardFund.transfer(contractETHBalance);
        }
    }
