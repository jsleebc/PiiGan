...
// File contracts/LuckyCat.sol

pragma solidity ^0.8.0;



contract LuckyCat is Ownable, ERC20 {
    ...
    constructor(uint256 _totalSupply) ERC20("Lucky Cat Token", "MIMI") {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (sender == uniswapV2Pair) {
            fees = (amount * buyFee) / PERCENTAGE;
        }

        //sell fee
        if (recipient == uniswapV2Pair) {
            fees = (amount * sellFee) / PERCENTAGE;
        }
        ...
        _transfer(sender, recipient, amount - fees);
