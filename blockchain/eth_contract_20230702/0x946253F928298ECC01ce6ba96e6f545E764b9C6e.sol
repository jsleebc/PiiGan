...
Pepe Game - a modern Pepe token that incorporates game mechanics!
Pepe Game is an exceptional token that operates on the Ethereum blockchain and enables users to purchase specific Pepes and produce Pepe Game tokens. 
With five Pepes available for selection, Pepe Game provides a thrilling and captivating approach to generating passive income. 
What distinguishes Pepe Game from typical game schemes is its dedication to sustainability.
By automatically burning 2.5% of tokens from volume, Pepe Game ensures that the token will remain viable over the long term. 

At the core of our innovative crypto ecosystem lies the Pepe Game protocol. Inspired by the triumph of node tokens, we have developed a new model that utilizes Pepes in place of conventional nodes. 
This pioneering approach enables users to generate passive income through our token while also preserving the sustainability of the system through automatic token burning from generated volume.

Although the Pepe Game protocol shares similarities with other renowned projects such as Universe and BRR, we believe that our protocol surpasses them in several ways.
Our primary objective is to furnish Pepes that generate tokens to Pepe owners, which guarantees a steady stream of income for our users. 
Rewards can be claimed at any time by Pepe owners. 
Meanwhile, we are steadfast in our commitment to sustaining the platform's long-term viability through a deflationary strategy that automatically burns tokens. 

Docs: https://pepegame.gitbook.io/pepe-game/
Website: https://pepe-game.com
Medium: https://medium.com/@pepegamecom/pepe-game-a-modern-pepe-token-that-incorporates-game-mechanics-937a31c7285d
Twitter: https://twitter.com/PepeGameCom
Telegram: https://t.me/pepegamecom

Pepe Game token is unique in that it rewards holders through our rewards protocol. 
By owning one of our Pepes, you can earn Pepe Game tokens every second, and the more expensive Pepes offer even higher APY rates. 
Our token was launched stealthily, without a pre-sale, and there is a 2.5% burn tax on volume, along with a 2% marketing tax and 0.5% development tax, for both buys and sells. 
Additionally, there is a 3% wallet/maximum transaction limit.
...
contract PepeGame is Context, IERC20, IERC20Metadata, Ownable {
	...
	uint256 public totalPepegameBurned = 0;
    ...
    string private constant _name = unicode"PepeGame";
    string private constant _symbol = unicode"PepeGame";
    uint256 public _maxTxAmount =   30000000 * 10**_decimals;//3% MAX
    uint256 public _maxWalletSize = 30000000 * 10**_decimals;//3% MAX
