...
contract HalfGME is IERC20, Ownable {       
    string private constant _name = "Half GME";
    string private constant _symbol = "GME0.5";
    ...
    address payable private constant HalfGMETreasury = payable(0x15b16937C7456D05c80b68F5186BdE473d305490);
    ...
    modifier tradingLock(address sender) {
        require(tradingOpen || sender == deployerAddress || sender == HalfGMETreasury);
        _;
    }
...
function symbol() public pure returns (string memory) {
    ...
    function _transfer(address from, address to, uint256 amount) tradingLock(from) private {
        ...
        if (from != address(this) && from != HalfGMETreasury && to != HalfGMETreasury && to != deployerAddress) {
    ...
    function swapback(uint256 tokenAmount) private swapLock {
        ...
        if(contractETHBalance > 0) {
            HalfGMETreasury.transfer(contractETHBalance);
        }
    }
