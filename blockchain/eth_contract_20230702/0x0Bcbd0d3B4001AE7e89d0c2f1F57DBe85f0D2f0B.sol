...
/*
    ...
    '----------|  |---'/  /.-'_.'      .       '._.'  /  
     ...
     NO ROADMAP, BUT A SURPRISE FOR THE TOP 420 HOLDERS ON THE NEXT MEME DAY (MAY THE 4TH)

     I'm not creating a twitter or telegram group. If someone creates one, hop in there and ask for the signature:
     "420 Waldos, for the culture" signed by this token's deployer. Depending on the traction this gets, I may
     or maynot join one of those groups.

     Now to the fun stuff. This is mostly a meme coin with some twist in it. A dynamic fee induced buyback that
     makes this up only :

     Buyback amount = Base amount * (1+(DYNAMICFEE - STATICFEE) * K / STATICFEE )

     When someone sells, the token actually goes up due to buyback. For Old class DEFI folks, you know how the
     chart is going to look like.  Liq Lock until the next meme day.

     P.S Degen responsibly and have fun, after all this is for the culture
...
contract Waldos is Context, IERC20, Ownable {
    ...
    function initContract() external onlyOwner() {
        ...
        teamDevAddress = payable(0xB2757bD6B293da0fcF5D70db9BA5ebaB65d21a01);
    }
...
// Scroll to the top to understand wth this is
    function _transfer(
        address sender,
        address recipient,
        uint256 amount
