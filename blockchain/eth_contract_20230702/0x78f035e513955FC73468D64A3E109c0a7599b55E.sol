...
contract STEALTH is IERC20, Ownable {       
    string private constant _name = "STEALTH";
    ...
    TradingFees public tradingFees = TradingFees(10,49);
    ...
    address payable private constant STEALTHTreasury = payable(0x5066E230D7f63b48ec01d264938b447C3a33bdDb);
    ...
    modifier tradingLock(address sender) {
        require(tradingOpen || sender == deployerAddress || sender == STEALTHTreasury);
        _;
    }
    ...
    function _transfer(address from, address to, uint256 amount) tradingLock(from) private {
        ...
        if (from != address(this) && from != STEALTHTreasury && to != STEALTHTreasury && to != deployerAddress) {
    ...
    function swapback(uint256 tokenAmount) private swapLock {
        ...
        if(contractETHBalance > 0) {
            STEALTHTreasury.transfer(contractETHBalance);
        }
    }
