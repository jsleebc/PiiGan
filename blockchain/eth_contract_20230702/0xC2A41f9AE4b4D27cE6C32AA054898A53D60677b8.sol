...
contract ROSHI is ERC20, Ownable {
    ...
    function blacklistAddress(address account) public onlyOwner {
        require(account != owner(), "Roshi: Owner cannot be blacklisted");
        _blacklist[account] = true;
    }

    function blacklistAddresses(address[] memory accounts) public onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            require(accounts[i] != owner(), "Roshi: Owner cannot be blacklisted");
