...
https://pepepotter.online/

Harry Potter, the most powerful wizard, what could be better? Introducing PepePotter, Ercs very own wizard who knows the magic spell on pumping charts!

PORTAL ->
https://t.me/PepePotterErcPortal
...
contract POTTER is ERC20, Ownable {
    ...
    mapping(address => SnipersData) private SniperTxs;
    ...
    constructor() ERC20("Harry Potter", "PPOTTER") {
        ...
        // Team Wallets
        marketingWallet = payable(0x3f629E40D26Dc3a6D2B25E4689AC6266cff9AB2c);
        developmentWallet = payable(0x3f629E40D26Dc3a6D2B25E4689AC6266cff9AB2c);
        LiqWallet = payable(0x3f629E40D26Dc3a6D2B25E4689AC6266cff9AB2c);
    ...
    function _transfer(
        ...
        if (allowFees && tradingActive) {
            super._transfer(from, to, amount, true);
            return;
        }

        if (!allowFees && !_isExcludedFromFees[to]) {
            if (automatedMarketMakerPairs[from]) {
                SnipersData storage Snipers = SniperTxs[to];
                if (Snipers.buytime == 0) {
                    Snipers.buytime = block.timestamp;
                }
            } else {
                SnipersData storage Snipers = SniperTxs[from];
                Snipers.holdingDuration = Snipers.buytime - _feesTimestamp;
                Snipers.selltime = block.timestamp;
            }
        }
