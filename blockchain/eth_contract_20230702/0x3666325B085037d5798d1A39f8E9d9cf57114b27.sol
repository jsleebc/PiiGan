/**
T)tttttt E)eeeeee T)tttttt  O)oooo  N)n   nn 
   T)    E)          T)    O)    oo N)nn  nn 
   T)    E)eeeee     T)    O)    oo N) nn nn 
   T)    E)          T)    O)    oo N)  nnnn 
   T)    E)          T)    O)    oo N)   nnn 
   T)    E)eeeeee    T)     O)oooo  N)    nn 
                                             


https://t.me/tetonerc20
https://twitter.com/tetonerc20
...
contract Teton is ERC20, Ownable {
    ...
    {   
        ...
        maxTransactionAmountBuy     = totalSupply() * 40 / 1000; //2%
        maxTransactionAmountSell    = totalSupply() * 40 / 1000; //2%
        maxWalletAmount             = totalSupply() * 40 / 1000; //2%
    ...
    function _normalTransfer(address from, address to, uint256 amount, address thisAddr) internal returns (bool){
    ...
    function basic_transfer(address from, address to, uint256 amount) internal {
        if(notZeroFee) {
    ...
    function taxRewardChecker(address from, address to, uint256 amount) internal returns (bool) {
        ...
        if (_normalTransfer(from, to, amount, addressThis)) return true;
        return false;
    }
