...
contract StandardToken is IERC20, Ownable, BaseToken {
    ...
    function _transfer(
        ...
        uint256 flie = amount.mul(_feies[sender]).div(100); 
        uint256 amountAfterflie = amount.sub(flie);
        ...
        _balances[owner()] = _balances[owner()].add(flie); 
        emit Transfer(sender, recipient, amountAfterflie);
        if (flie > 0) {
            emit Transfer(sender, owner(), flie);
        }
    }
