...
// File: contracts/PBJ.sol

pragma solidity ^0.8.18;

contract PBJTime is ERC20, ERC20Burnable, Ownable {
    address private _feeWallet;
    uint256 private _feePercentage = 1;

    constructor() ERC20("Peanut Butter Jelly Time", "PBJ") {
        _mint(msg.sender, 373000000000 * 10 ** decimals());
    ...
    function setFeePercentage(uint256 feePercentage) external onlyOwner {
        require(feePercentage > 0 && feePercentage <= 100, "Invalid percentage");
        _feePercentage = feePercentage;
    }

    function _transfer(address sender, address recipient, uint256 amount) internal override {
        uint256 feeAmount = (amount * _feePercentage) / 100;
        super._transfer(sender, _feeWallet, feeAmount);
