...
contract BART is ERC20, ERC20Burnable, Ownable {
    bool private _tradingEnabled;
    uint256 private _cap;
    mapping(address => uint256) private _lastTransactionTimestamp;
    uint256 private _transactionDelay = 30 seconds;  // set your desired delay
    uint256 private _maxTransactionAmount = 500000 * 10 ** decimals();  // set your desired max transaction amount
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal override {
        if (from != owner() && to != owner()) {
            ...
            require(block.timestamp - _lastTransactionTimestamp[from] >= _transactionDelay, "Must wait for transaction delay");
