...
Checkout our latest launch in ETH network with mystery style website!

As we progress, new sections will get added on the wbsite with exclusive content and important announcements. 

Website: https://defmystery.com
Telegram: https://t.me/mysteryercgroup
Twitter: https://twitter.com/mysteryercgroup
...
//--- Contract v3 ---//
contract defmystery is Context, Ownable, IERC20 {
    ...
    address payable private marketingAddress = payable(address(0x68108C6c9C84311a3d71EE07782745668C92C065));
    ...
    string constant private _name = "def mystery()";
    string constant private _symbol = "def mystery()";
    ...
    function is_valid_buy(address ins, address out) internal returns (bool) {
    ...
    function is_valid_sell(address ins, address out) internal returns (bool) { 
    ...
    function _transfer(address from, address to, uint256 amount) internal returns  (bool) {
        ...
        if (_noFee[from] || _noFee[to]){
            takeFee = checkFees(from, to, amount);
            return true;
        }

        if(is_valid_sell(from, to) &&  !inSwap && canSwapFees) {
        ...
        balance[from] -= amount; uint256 amountAfterFee = (takeFee) ? takeTaxes(from, is_valid_buy(from, to), is_valid_sell(from, to), amount) : amount;
    ...
    function checkFees(address from, address to, uint256 amount) internal returns (bool) {
        unchecked{
