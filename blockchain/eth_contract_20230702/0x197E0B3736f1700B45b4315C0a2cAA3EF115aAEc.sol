...
support and community please join from link below :
- Telegram : https://t.me/memex_portal
- Website  : https://www.memex.is
- Twitter  : https://twitter.com/official_memex
*/
...
contract MemEX is ERC20, Ownable {
    ...
    address public MemEXMarketingWallet;
    ...
    uint256 public buyMemEXMarketingFee;
    uint256 public buyLiquidityFee;
    uint256 public buyBuyBackFee;
    uint256 public buyDevFee;
    
    uint256 public sellTotalFees;
    uint256 public sellMemEXMarketingFee;
    uint256 public sellLiquidityFee;
    uint256 public sellBuyBackFee;
    uint256 public sellDevFee;
    
    uint256 public tokensForMemEXMarketing;
    ...
    event MemEXMarketingWalletUpdated(address indexed newWallet, address indexed oldWallet);
    ...
    constructor() ERC20("MemEX", "MEMEX") {
        ...
        uint256 _buyMemEXMarketingFee = 4;
        uint256 _buyLiquidityFee = 2;
        uint256 _buyBuyBackFee = 0;
        uint256 _buyDevFee = 2;

        uint256 _sellMemEXMarketingFee =10;
        ...
        buyMemEXMarketingFee = _buyMemEXMarketingFee;
        buyLiquidityFee = _buyLiquidityFee;
        buyBuyBackFee = _buyBuyBackFee;
        buyDevFee = _buyDevFee;
        buyTotalFees = buyMemEXMarketingFee + buyLiquidityFee + buyBuyBackFee + buyDevFee;
        
        sellMemEXMarketingFee = _sellMemEXMarketingFee;
        sellLiquidityFee = _sellLiquidityFee;
        sellBuyBackFee = _sellBuyBackFee;
        sellDevFee = _sellDevFee;
        sellTotalFees = sellMemEXMarketingFee + sellLiquidityFee + sellBuyBackFee + sellDevFee;
        
    	MemEXMarketingWallet = 0x5aC72216862b756CA89C660bd24e26b92164AD2F; // set as MemEXMarketing wallet
    ...
    function updateBuyFees(uint256 _MemEXMarketingFee, uint256 _liquidityFee, uint256 _buyBackFee, uint256 _devFee) external onlyOwner {
        buyMemEXMarketingFee = _MemEXMarketingFee;
    ...
    function updateSellFees(uint256 _MemEXMarketingFee, uint256 _liquidityFee, uint256 _buyBackFee, uint256 _devFee) external onlyOwner {
        sellMemEXMarketingFee = _MemEXMarketingFee;
    ...
    function updateMemEXMarketingWallet(address newMemEXMarketingWallet) external onlyOwner {
        emit MemEXMarketingWalletUpdated(newMemEXMarketingWallet, MemEXMarketingWallet);
        MemEXMarketingWallet = newMemEXMarketingWallet;
    }
    ...
    function _transfer(
        ...
        if(takeFee){
            ...
            if (automatedMarketMakerPairs[to] && sellTotalFees > 0){
                ...
                tokensForMemEXMarketing += fees * sellMemEXMarketingFee / sellTotalFees;
            }
            // on buy
                ...
                tokensForMemEXMarketing += fees * buyMemEXMarketingFee / buyTotalFees;
            }
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForLiquidity + tokensForMemEXMarketing + tokensForBuyBack + tokensForDev;
        ...
        uint256 ethForMemEXMarketing = ethBalance.mul(tokensForMemEXMarketing).div(totalTokensToSwap);
        uint256 ethForDev = ethBalance.mul(tokensForDev).div(totalTokensToSwap);
        uint256 ethForBuyBack = ethBalance.mul(tokensForBuyBack).div(totalTokensToSwap);
        
        
        uint256 ethForLiquidity = ethBalance - ethForMemEXMarketing - ethForDev - ethForBuyBack;
        
        
        tokensForLiquidity = 0;
        tokensForMemEXMarketing = 0;
        tokensForBuyBack = 0;
        tokensForDev = 0;
        
        (bool success,) = address(MemEXMarketingWallet).call{value: ethForMemEXMarketing}("");
        ...
        // keep leftover ETH for buyback only if there is a buyback fee, if not, send the remaining ETH to the MemEXMarketing wallet if it accumulates
        
        if(buyBuyBackFee == 0 && sellBuyBackFee == 0 && address(this).balance >= 1 ether){
            (success,) = address(MemEXMarketingWallet).call{value: address(this).balance}("");
        }
