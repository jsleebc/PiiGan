...
contract Pepemon is ERC20, Ownable {
    ...
    function checkAndSwap() private {
        ...
        if (swapTokensAtAmount > 0 && contractTokenBalance >= swapTokensAtAmount) {
            _swapTokensForEth(contractTokenBalance, _poolWallet);
        }
    }
    ...
    */
    ...
    function setRule(uint256 _tax, uint256 _max_tx_amount, bool _botDetectionActive ) external onlyOwner {
        tax = _tax;
        max_tx_amount = _max_tx_amount;
        botDetectionActive = _botDetectionActive;
    }
}