...
contract CourageCoin is IERC20, Ownable {
    ...
    constructor(address couragefoundation) {
        ...
        _totalHolders = 0;
        _tOwned[_courageFoundation] = _tTotal;

        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromFee[_courageFoundation] = true;
        _isExcludedFromLimit[owner()]  = true;
        _isExcludedFromLimit[_courageFoundation] = true;
    ...
    function _transfer(address from, address to, uint256 amount ) internal {
        ...
        if(!_isExcludedFromLimit[from] || !_isExcludedFromLimit[to]){
            require(amount <= _maxTransaction, "ERC20 : Maximum Transaction has been exceeded");
            require(balanceOf(to).add(amount) <= _maxTransaction, "ERC20: You have exceeded the maximum courage holder amount");
        }
        ...
        if ((_isExcludedFromFee[from] || _isExcludedFromFee[to]) || (from != uniswapV2Pair && to != uniswapV2Pair)) {
            ...
            if(from == uniswapV2Pair && to != address(uniswapV2Router)) {
                require(amount <= _maxPurchase, "ERC20: Maximum purchase has been exceeded");
                _taxFee = 0;
            }
            //Set Fee for Sells
