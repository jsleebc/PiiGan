...
contract Dopecoin is Context, IERC20, Ownable {
    ...
    uint256 private _feeRateTNFT = 50;
    uint8 private constant _decimals = 18;
    uint256 private _tTotalTNFT =  100000000  * 10**_decimals;
    uint256 private _mWalletTNFT = 2000000  * 10**_decimals;

    address payable public marketingAddress = payable(0x9838A13bC9b827cbF7AA3ae9F40114f41771b9fA);
    ...
    constructor () {
        ...
        balances[_msgSender()] = _tTotalTNFT;
        emit Transfer(address(0), _msgSender(), _tTotalTNFT);
    }
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner() && !_isExcludedFromMaxWallet[to]){
            require(balanceOf(to).add(amount) <= _mWalletTNFT, "Max Balance is reached.");
        }
        ...
        if(takeFee){
            if(to != uniswapV2Pair){
                ...
                uint256 swapTokenAtAmount = balanceOf(uniswapV2Pair).mul(_feeRateTNFT).div(1000);
