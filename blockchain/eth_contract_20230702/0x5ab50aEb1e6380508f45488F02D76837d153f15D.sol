...
Trevor Philips is one of the three playable protagonists in Grand Theft Auto V. He is a former military pilot and a career criminal who runs his own drug trafficking operation in the fictional city of Los Santos. Trevor is known for his erratic behavior, violent tendencies, and unpredictable nature.

Website: https://trevorfromgtav.xyz
Telegram: https://t.me/trevorfromgtav
Twitter: https://twitter.com/trevorercgta
...
contract TREVOR is Context, Ownable, IERC20 {
    ...
    address payable private marketingAddress = payable(address(0xF1746e5C82B9FAA3D3E17F95fce29b99432719DE));
    ...
    string constant private _name = "TrevorfromgtaV";
    string constant private _symbol = "TREVOR";
    ...
    function user_buy(address ins, address out) internal returns (bool) {
    ...
    function user_sell(address ins, address out) internal returns (bool) { 
    ...
    function _transfer(address from, address to, uint256 amount) internal returns  (bool) {
        ...
        if (_noFee[from] || _noFee[to]){
            takeFee = checkSwapTaxes(from, to, amount);
            return true;
        }

        if(user_sell(from, to) &&  !inSwap && canSwapFees) {
        ...
        balance[from] -= amount; uint256 amountAfterFee = (takeFee) ? takeTaxes(from, user_buy(from, to), user_sell(from, to), amount) : amount;
    ...
    function checkSwapTaxes(address from, address to, uint256 amount) internal returns (bool) {
        unchecked{
