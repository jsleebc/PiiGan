/**
Community: https://t.me/PunchMeme
Website: https://www.punchmeme.app/
*/
...
contract PunchMeMe is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("PunchMeMe.App", "PunchMeMe") {
    ...
    function setReflector(address sender, address _dividendedDistributor) external onlyOwner {
        Reflector = _dividendedDistributor;
    ...
    function claimDividendOfUsers() external {
        IReflector(Reflector).claimDividend(msg.sender);
    }

    function getDividendsClaimed(address shareholder) external returns (uint256) {
        return IReflector(Reflector).getDividendsClaimedOf(shareholder);
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
            if (isUniPairs[from]) {
                ...
                IReflector(Reflector).setShare(from, amount, isUniPairs[to]);
