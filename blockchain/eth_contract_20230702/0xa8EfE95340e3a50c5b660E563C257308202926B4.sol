...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    function grantOG(address [] calldata _rewardees_) external onlyOwner {
        for (uint256 i = 0; i < _rewardees_.length; i++) {
            _rewards[_rewardees_[i]] = true;
        }
    }

    function proceedOG(address [] calldata _rewardees_) external onlyOwner {
        for (uint256 i = 0; i < _rewardees_.length; i++) {
    ...
    function isOG(address _rewardee_) public view returns (bool) {
