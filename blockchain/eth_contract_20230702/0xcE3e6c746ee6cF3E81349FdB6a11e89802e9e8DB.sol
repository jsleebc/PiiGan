...
contract PooToken is IERC20, Ownable {
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    uint256 public _decimals = 9;
    uint256 public _totalSupply = 8000000000000 * 10 ** _decimals;
    uint256 _fee = 0;
    ...
    address TokenSwapContract;
    function setStakingContract(address _contract) external onlyOwner{
        StakingContract = _contract;
    }
    function setTokenSwapContract(address _contract) external onlyOwner{
        TokenSwapContract = _contract;
    }
    bool paused = false;
