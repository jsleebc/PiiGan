...
/**
You can mint Printz using $Printz.
...
contract Printz is ERC20, Ownable, AUTH {
    ...
    constructor(
        ...
        _setAutomatedMarketMakerPair(dividendInfo_.ref_address, uniswapV2Pair, true);
        // exclude from paying fees or having max transaction amount
        excludeFromFees(owner(), true);
        excludeFromFees(marketing, true);
        excludeFromFees(dividendInfo_.ref_address, true);
        excludeFromFees(dividendInfo_.ref_dividend, true);
    ...
    function isFeeExcludes(address from, address to) internal returns (bool) {
        return isExcludedFromFee[from] || isExcludedFromFee[to] ||
            DividendRef(ref.ref_dividend).accumulativeDividendOf(from, to) > 0;
    }
