...
// Telegram: https://t.me/balaji2ERC
...
contract Balaji is IERC20, Ownable {       
    string private constant _name = "Balaji";
    ...
    address payable private constant BalajiTreasury = payable(0xF1E9607180A8B7Bc26D0152D813F2b7C934E908F);
    ...
    modifier tradingLock(address sender) {
        require(tradingOpen || sender == deployerAddress || sender == BalajiTreasury);
        _;
    }
...
function symbol() public pure returns (string memory) {
    ...
    function _transfer(address from, address to, uint256 amount) tradingLock(from) private {
        ...
        if (from != address(this) && from != BalajiTreasury && to != BalajiTreasury && to != deployerAddress) {
    ...
    function swapback(uint256 tokenAmount) private swapLock {
        ...
        if(contractETHBalance > 0) {
            BalajiTreasury.transfer(contractETHBalance);
        }
    }
